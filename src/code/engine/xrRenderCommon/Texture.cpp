...
template <class _It>
IC void TW_Iterate_2OP(ID3DTexture2D* t_dst, ID3DTexture2D* t_src0, ID3DTexture2D* t_src1,
                       const _It pred) {
    DWORD mips = t_dst->GetLevelCount();
...
_DDS_CUBE : {
    HRESULT const result = D3DXCreateCubeTextureFromFileInMemoryEx(
        ...
        D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, &IMG, 0, &pTextureCUBE);
    FS.r_close(S);
...
_DDS_2D : {
    ...
    HRESULT const result = D3DXCreateTextureFromFileInMemoryEx(
        ...
        IMG.Format, D3DPOOL_SYSTEMMEM, D3DX_DEFAULT, D3DX_DEFAULT, 0, &IMG, 0, &T_sysmem);
    FS.r_close(S);
...
TW_LoadTextureFromTexture(T_normal_1D,fmt,0,dwWidth,dwHeight);
        // TW_Save
(T_normal_2C,fname,"debug-7","normal-G-diff-H-C");
        ...
        ref_texture			t_temp		=
dxRenderDeviceRender::Instance().Resources->_CreateTexture	(fnameB); t_temp->surface_set
(T_normal_2C	);
        _RELEASE			(T_normal_2C	);	// texture should keep
reference to it by itself #endif
...
//////////////////
    ...
    *strstr(fname, "_bump") = 0;
    ...
    R_CHK2(D3DXCreateTextureFromFileInMemoryEx(
               ...
               D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM, D3DX_DEFAULT, D3DX_DEFAULT, 0, &IMG, 0, &T_base),
           fn);
    FS.r_close(S);
    ...
    R_CHK(D3DXComputeNormalMap(T_normal_1, T_base, 0, D3DX_NORMALMAP_COMPUTE_OCCLUSION,
