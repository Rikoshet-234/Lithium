...
public:
    struct _spatial {
        ...
        _spatial()
            : type(0) {} // safe way to enhure type is zero before any contstructors takes place
    } spatial;
...
public:
    ...
    bool _empty() {
        return items.empty() &&
               (0 == (intptr_t(children[0]) | intptr_t(children[1]) | intptr_t(children[2]) |
                      intptr_t(children[3]) | intptr_t(children[4]) | intptr_t(children[5]) |
