...
CALifeUpdateManager::CALifeUpdateManager(xrServer* server, LPCSTR section)
    : CALifeSwitchManager(server, section), CALifeSurgeManager(server, section),
      CALifeStorageManager(server, section), CALifeSimulatorBase(server, section) {
...
void CALifeUpdateManager::set_process_time(int microseconds) {
    graph().set_process_time(float(microseconds) -
                             float(microseconds) * update_monster_factor() / 1000000.f);
}
...
{
    g_pGamePersistent->LoadTitleOld("st_creating_new_game");
...
void CALifeUpdateManager::load(LPCSTR game_name, bool no_assert, bool new_only) {
    g_pGamePersistent->LoadTitleOld("st_loading_alife_simulator");
    g_pGamePersistent->LoadTitle();
	...
	g_pGamePersistent->LoadTitleOld("st_server_connecting");
...
void CALifeUpdateManager::jump_to_level(LPCSTR level_name) const {
    ...
    bool failed = !ai().graph_engine().search(ai().game_graph(), graph().actor()->m_tGraphID,
                                              GameGraph::_GRAPH_ID(-1), 0, evaluator);
    if (failed) {
#ifndef MASTER_GOLD
        Msg("! Cannot build path via game graph from the current level to the level %s!",
            level_name);
        ...
        if (!ai().game_graph().vertex(dest)) {
            Msg("! There is no game vertices on the level %s, cannot jump to the specified level",
                level_name);
            return;
        }
    } else
...
void CALifeUpdateManager::teleport_object(ALife::_OBJECT_ID id, GameGraph::_GRAPH_ID game_vertex_id,
    ...
    if (psAI_Flags.test(aiALife)) {
        Msg("[LSS] teleporting object [%s][%s][%d] from level [%s], position [%f][%f][%f] to level "
            "[%s], position [%f][%f][%f]",
            ...
            (ai().game_graph()
                  .header()
                  .level(ai().game_graph().vertex(game_vertex_id)->level_id())
                  .name()),
...
void CALifeUpdateManager::add_restriction(
    ALife::_OBJECT_ID id, ALife::_OBJECT_ID restriction_id,
    const RestrictionSpace::ERestrictorTypes& restriction_type) {
    CSE_ALifeDynamicObject* object = objects().object(id, true);
    if (!object) {
        Msg("! cannot add restriction with id %d to the entity with id %d, because there is no "
            "creature with the specified id",
            restriction_id, id);
        return;
    }
    ...
    if (!object_restrictor) {
        Msg("! cannot add restriction with id %d to the entity with id %d, because there is no "
            "space restrictor with the specified id",
            restriction_id, id);
        return;
    }
    ...
    if (!creature) {
        Msg("! cannot add restriction with id %d to the entity with id %d, because there is an "
            "object with the specified id, but it is not a creature",
            restriction_id, id);
        return;
    }
    ...
    if (!restrictor) {
        Msg("! cannot add restriction with id %d to the entity with id %d, because there is an "
            "object with the specified id, but it is not a space restrictor",
            restriction_id, id);
        return;
    }
    ...
    case RestrictionSpace::eRestrictorTypeOut: {
        ...
        if (std::find(creature->m_dynamic_out_restrictions.begin(),
            ...
            Msg("! cannot add out-restriction with id %d, name %s to the entity with id %d, name "
                "%s, because it is already added",
    ...
    case RestrictionSpace::eRestrictorTypeIn: {
        ...
        if (std::find(creature->m_dynamic_in_restrictions.begin(),
            ...
            Msg("! cannot add in-restriction with id %d, name %s to the entity with id %d, name "
                "%s, because it is already added",
                restriction_id, restrictor->name_replace(), id, creature->name_replace());
...
void CALifeUpdateManager::remove_restriction(
    ...
    CSE_ALifeDynamicObject* object = objects().object(id, true);
    if (!object) {
        Msg("! cannot remove restriction with id %d to the entity with id %d, because there is no "
            "creature with the specified id",
            restriction_id, id);
        return;
    }
    ...
    CSE_ALifeCreatureAbstract* creature = smart_cast<CSE_ALifeCreatureAbstract*>(object);
    if (!creature) {
        Msg("! cannot remove restriction with id %d to the entity with id %d, because there is an "
            "object with the specified id, but it is not a creature",
            restriction_id, id);
        return;
    }
    ...
    case RestrictionSpace::eRestrictorTypeOut: {
        xr_vector<ALife::_OBJECT_ID>::iterator I =
            std::find(creature->m_dynamic_out_restrictions.begin(),
                      creature->m_dynamic_out_restrictions.end(), restriction_id);
        if (I == creature->m_dynamic_out_restrictions.end()) {
            Msg("~ cannot remove restriction with id [%d][%s] to the entity with id [%d][%s], "
    ...
    case RestrictionSpace::eRestrictorTypeIn: {
        xr_vector<ALife::_OBJECT_ID>::iterator I =
            std::find(creature->m_dynamic_in_restrictions.begin(),
...
void CALifeUpdateManager::remove_all_restrictions(
    ...
    CSE_ALifeDynamicObject* object = objects().object(id, true);
    if (!object) {
        Msg("! cannot remove restrictions to the entity with id %d, because there is no creature "
    ...
    CSE_ALifeCreatureAbstract* creature = smart_cast<CSE_ALifeCreatureAbstract*>(object);
    if (!creature) {
        Msg("! cannot remove restriction to the entity with id %d, because there is an object with "
            "the specified id, but it is not a creature",
            id);
        return;
    }
