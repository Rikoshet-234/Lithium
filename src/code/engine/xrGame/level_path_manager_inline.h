...
#define CLevelManagerTemplate \
    CBasePathManager<CLevelGraph, _VertexEvaluator, _vertex_id_type, _index_type>
...
IC bool CLevelManagerTemplate::actual() const {
    return (
        inherited::actual(this->m_object->object().ai_location().level_vertex_id(), this->dest_vertex_id()));
}

TEMPLATE_SPECIALIZATION
IC void CLevelManagerTemplate::build_path(const _vertex_id_type start_vertex_id,
