...
void check_path(const CBaseMonster* monster, const CPatrolPath* path) {
    VERIFY2(
        ai().game_graph()
                .vertex(path->vertices().begin()->second->data().game_vertex_id())
        ...
        make_string(
            ...
            *path->m_name, *monster->cName(),
            ai().game_graph()
                 .header()
                 .level(
                     ai().game_graph().vertex(monster->ai_location().game_vertex_id())->level_id())
                 .name(),
            ai().game_graph()
                 .header()
                 .level(ai().game_graph()
...
void CMonsterHome::load(LPCSTR line) {
    ...
    if (m_object->spawn_ini() && m_object->spawn_ini()->section_exist(line)) {
        ...
        VERIFY3(m_radius_max > m_radius_min,
                "Error: Wrong home point radius specified for monster ", *m_object->cName());
...
void CMonsterHome::setup(u32 lv_ID, float min_radius, float max_radius, bool aggressive,
                         float middle_radius) {
    m_path = 0;
...
u32 CMonsterHome::get_place_in_mid_home() {
    ...
    if (!at_mid_home(m_object->Position()) || at_min_home(m_object->Position())) {
        ...
        m_object->control().path_builder().get_node_in_radius(input_node, m_radius_min,
                                                              m_radius_middle, 5, result);
    } else {
        int i = 0;
        ...
        if (ai().level_graph().valid_vertex_position(m_dest_direction)) {
            ...
            input_node = m_object->ai_location().level_vertex_id();
            m_object->control().path_builder().get_node_in_radius(input_node, m_res_dist - 1,
                                                                  m_res_dist, 5, result);
        }
    }
...
u32 CMonsterHome::get_place_in_max_home_to_direction(Fvector to_direction) {
    ...
    float mAngle;
    Fvector m_dest_direction;
    float m_res_dist = m_radius_middle + (m_radius_max - m_radius_middle) / 2 -
...
u32 CMonsterHome::get_place_in_max_home() {
    ...
    m_object->control().path_builder().get_node_in_radius(input_node, m_radius_min, m_radius_max, 5,
                                                          result);
...
u32 CMonsterHome::get_place() {
    ...
    if (!m_path) {
        if (ai().level_graph().valid_vertex_id(m_level_vertex_id))
            m_object->control().path_builder().get_node_in_radius(
                m_level_vertex_id, m_radius_min, m_radius_min + (m_radius_max - m_radius_min) / 2,
                5, result);
