...
BOOL game_sv_Single::OnTouch(u16 eid_who, u16 eid_what, BOOL bForced) {
    ...
    if (ai().get_alife()) {
        ...
        if (l_tpALifeInventoryItem && l_tpDynamicObject &&
            ...
            alife().graph().attach(*e_who, l_tpALifeInventoryItem, l_tpDynamicObject->m_tGraphID,
                                   false, false);
#ifdef DEBUG
        else if (psAI_Flags.test(aiALife)) {
            Msg("Cannot attach object [%s][%s][%d] to object [%s][%s][%d]", e_what->name_replace(),
                *e_what->s_name, e_what->ID, e_who->name_replace(), *e_who->s_name, e_who->ID);
        }
#endif
    }
    return TRUE;
...
void game_sv_Single::OnDetach(u16 eid_who, u16 eid_what) {
    if (ai().get_alife()) {
        ...
        if (ai().alife().objects().object(e_who->ID, true) &&
            !ai().alife().graph().level().object(l_tpALifeInventoryItem->base()->ID, true) &&
            ai().alife().objects().object(e_what->ID, true))
            alife().graph().detach(*e_who, l_tpALifeInventoryItem, l_tpDynamicObject->m_tGraphID,
                                   false, false);
        else {
            if (!ai().alife().objects().object(e_what->ID, true)) {
                ...
                Msg("Cannot detach object [%s][%s][%d] from object [%s][%s][%d]",
                    ...
                    *l_tpALifeInventoryItem->base()->s_name, l_tpALifeInventoryItem->base()->ID,
                    l_tpDynamicObject->base()->name_replace(), l_tpDynamicObject->base()->s_name,
...
void game_sv_Single::restart_simulator(LPCSTR saved_game_name) {
	...
	g_pGamePersistent->LoadTitleOld("st_client_synchronising");
