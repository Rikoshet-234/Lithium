...
bool CUIGameSP::IR_UIOnKeyboardPress(int dik) {
    ...
    case kACTIVE_JOBS: {
        if (!pActor->inventory_disabled() && !pActor->pda_disabled())
            ShowPdaMenu();
        break;
    }
    ...
    case kSCORES:
        if (!pActor->inventory_disabled()) {
            ...
            m_game_objective->m_static->TextItemControl()->SetTextST((t1) ? t1->m_Title.c_str()
                                                                          : "st_no_active_task");
...
void CUIGameSP::ChangeLevel(GameGraph::_GRAPH_ID game_vert_id, u32 level_vert_id, Fvector pos,
                            Fvector ang, Fvector pos2, Fvector ang2, bool b_use_position_cancel,
                            const shared_str& message_str, bool b_allow_change_level) {
...
void CChangeLevelWnd::Show() {
    m_messageBox->InitMessageBox(m_b_allow_change_level ? "message_box_change_level"
