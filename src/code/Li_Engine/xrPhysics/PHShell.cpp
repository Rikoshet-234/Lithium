...
void CPHShell::MassAddBox(float mass, const Fvector& full_size) {
    dMass m;
    dMassSetBox(&m, mass, full_size.x, full_size.y,
...
CPhysicsElement* CPHShell::get_Element(u16 bone_id) {
    ...
    for (; e != i; ++i)
        if ((*i)->m_SelfID == bone_id)
            return static_cast<CPhysicsElement*>(*i);
    return nullptr;
}
...
void CPHShell::build_FromKinematics(IKinematics* K, BONE_P_MAP* p_geting_map) {
    ...
    // R_ASSERT2((*elements.begin())->numberOfGeoms(),"No physics shapes was assigned for model or
    // no shapes in main root bone!!!");  SetCallbacks(BonesCallback);
...
void CPHShell::AddElementRecursive(CPhysicsElement* root_e, u16 id, Fmatrix global_parent,
                                   u16 element_number, bool* vis_check) {
    ...
    if (no_visible) {
        ...
        for (u16 i = 0; i < num_children; ++i)
            AddElementRecursive(root_e, bone_data.GetChild(i).GetSelfID(), fm_position,
                                element_number, &lvis_check);
        ...
        if (joint_data.type == jtRigid && root_e) //
        {
            ...
            if (breakable) {
                ...
                root_e->add_Mass(bone_data.get_shape(), vs_root_position,
                                 bone_data.get_center_of_mass(), bone_data.get_mass(), &fracture);

                fracture_num = E->setGeomFracturable(fracture);
            } else {
                root_e->add_Shape(bone_data.get_shape(), vs_root_position);
                root_e->add_Mass(bone_data.get_shape(), vs_root_position,
                                 bone_data.get_center_of_mass(), bone_data.get_mass());
            }
    ...
    u16 num_children = bone_data.GetNumChildren();
    for (u16 i = 0; i < num_children; ++i)
        AddElementRecursive(E, bone_data.GetChild(i).GetSelfID(), fm_position, element_number,
                            arg_check);
    /////////////////////////////////////////////////////////////////////////////////////
    if (breakable) {
...
void CPHShell::SetCallbacksRecursive(u16 id, u16 element) {
    ...
    mask.assign(m_pKinematics->LL_GetBonesVisible());
    if (mask.is(1ui64 << (u64)id)) {
        if ((no_physics_shape(bone_data.get_shape()) || joint_data.type == jtRigid) &&
            element != u16(-1)) {
            B.set_callback(bctPhysics, 0, cast_PhysicsElement(elements[element]));
        } else {
...
void CPHShell::setEndElementSplitter() {
    ...
    if (!elements.back()->FracturesHolder()) // adding fracture for element supposed before adding
                                             // splitter. Need only one splitter for an element
...
void CPHShell::setEndJointSplitter() {
    if (!joints.back()->JointDestroyInfo()) // setting joint breacable supposed before adding
                                            // splitter. Need only one splitter for a joint
...
void CPHShell::get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
                              float& hi_ext) const {
...
void CPHShell::dbg_draw_geometry(float scale, u32 color,
