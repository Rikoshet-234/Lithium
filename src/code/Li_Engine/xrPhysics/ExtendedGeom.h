...
XRPHYSICS_API void get_user_data(dxGeomUserData*& gd1, dxGeomUserData*& gd2, bool bo1,
...
IC void dGeomCreateUserData(dxGeom* geom) {
    if (!geom)
        return;
    dGeomSetData(geom, (void*)xr_new<dxGeomUserData>());
...
IC void dGeomUserDataSetObjectContactCallback(dxGeom* geom,
...
IC void dGeomUserDataRemoveObjectContactCallback(dxGeom* geom,
                                                 ObjectContactCallbackFun* obj_callback) {
    CObjectContactCallback::RemoveCallback((dGeomGetUserData(geom))->object_callbacks,
                                           (obj_callback));
}

// XRPHYSICS_API bool dGeomUserDataHasCallback(dxGeom* geom,ObjectContactCallbackFun
// *obj_callback);
