...
void CPHElement::SetTransform(const Fmatrix& m0, motion_history_state history_state) {
    VERIFY2(_valid(m0),
            make_string("invalid_form_in_set_transform") + (PhysicsRefObject()->dump(full_capped)));
    VERIFY2(valid_pos(m0.c),
            dbg_valide_pos_string(m0.c, PhysicsRefObject(), "invalid_form_in_set_transform"));
    Fvector mc;
...
// begin, body position");
    ...
    if (debug_output().ph_dbg_draw_mask().test(phDbgDrawMassCenters)) {
        debug_output().DBG_DrawPoint(cast_fv(dBodyGetPosition(m_body)), 0.03f,
    ...
    if (!dV_valid(angular_velocity)) {
        ...
        Msg("matrix");
        Msg("x  %f,%f,%f", dBodyGetRotation(m_body)[0], dBodyGetRotation(m_body)[4],
            dBodyGetRotation(m_body)[8]);
        Msg("y  %f,%f,%f", dBodyGetRotation(m_body)[1], dBodyGetRotation(m_body)[5],
            dBodyGetRotation(m_body)[9]);
        Msg("z  %f,%f,%f", dBodyGetRotation(m_body)[2], dBodyGetRotation(m_body)[6],
    ...
    /*
            ...
            {
       //hack Fvector	pos;
       //hack m_body_interpolation.GetPosition(pos,0);				   //hack
    ...
    */
    ...
    if (!fis_zero(k_w))
        dBodyAddTorque(m_body, -angular_velocity[0] * k_w, -angular_velocity[1] * k_w,
...
void CPHElement::applyImpulseTrace(const Fvector& pos, const Fvector& dir, float val, u16 id) {
    ...
    if (debug_output().ph_dbg_draw_mask().test(phHitApplicationPoints)) {
        ...
        debug_output().DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)), dbg_position,
                                    D3DCOLOR_XRGB(255, 255, 255));
        debug_output().DBG_DrawLine(dbg_position,
...
void CPHElement::StataticRootBonesCallBack(CBoneInstance* B) {
    ...
    if (m_flags.test(flActivating)) {
        ...
        // VERIFY2(fsimilar(DET(B->mTransform),1.f,DET_CHECK_EPS),"Bones callback returns 0
        // matrix");
        VERIFY_RMATRIX(B->mTransform);
        VERIFY(valid_pos(B->mTransform.c, phBoundaries));
        // return;
...
void CPHElement::set_LinearVel(const Fvector& velocity) {
    ...
    if (velocity.magnitude() > m_l_limit)
        Msg(" CPHElement::set_LinearVel set velocity magnitude is too large %f",
            velocity.magnitude());
#endif
...
void CPHElement::set_AngularVel(const Fvector& velocity) {
    ...
    if (velocity.magnitude() > m_w_limit)
        Msg("CPHElement::set_AngularVel set velocity magnitude is too large %f",
            velocity.magnitude());
#endif
...
void CPHElement::add_Mass(const SBoneShape& shape, const Fmatrix& offset,
                          const Fvector& mass_center, float mass, CPHFracture* fracture) {
...
void CPHElement::get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
                                float& hi_ext) const {
...
void CPHElement::dbg_draw_geometry(float scale, u32 color,
