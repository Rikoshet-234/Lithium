...
IC void dMatrixSmallDeviation(const float* matrix33_from, const float* matrix33_to,
                              float* vector_dev) {
...
IC void dMatrixSmallDeviationAdd(const float* matrix33_from, const float* matrix33_to,
                                 float* vector_dev) {
    vector_dev[0] += matrix33_from[10] - matrix33_to[10];
    vector_dev[1] += matrix33_from[2] - matrix33_to[2];
    vector_dev[2] += matrix33_from[4] - matrix33_to[4];
...
IC float prg_pos_on_plane(const Fvector& in_norm, float d, const Fvector& in_pos,
                          Fvector& out_pos) {
...
IC u8 TransferenceAndThrowVelToTgA(const Fvector& transference, float throw_vel,
                                   float gravity_accel, Fvector2& tgA, float& s) {
...
IC u8 TransferenceAndThrowVelToTgA(const Fvector& transference, float throw_vel,
                                   float gravity_accel, Fvector2& tgA) {
    float s;
    return TransferenceAndThrowVelToTgA(transference, throw_vel, gravity_accel, tgA, s);
}
IC u8 TransferenceAndThrowVelToThrowDir(const Fvector& transference, float throw_vel,
                                        float gravity_accel, Fvector throw_dir[2]) {
