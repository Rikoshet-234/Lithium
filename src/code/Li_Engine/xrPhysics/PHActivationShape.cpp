...
void GetMaxDepthCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                         SGameMtl* material_2) {
...
bool CPHActivationShape::Activate(const Fvector need_size, u16 steps, float max_displacement,
                                  float max_rotation, bool un_freeze_later /*	=false*/) {
    ...
    float limit_l_vel = std::max(std::max(need_size.x, need_size.y), need_size.z) / fnum_it *
                        fnum_steps_r / fixed_step;
