...
int SetBackTrajectoryCnt(const dReal* p, const dReal* last_pos, Triangle& neg_tri, dxGeom* o1,
                         dxGeom* o2, dContactGeom* Contacts) {
...
IC int dcTriListCollider::dSortTriPrimitiveCollide(T primitive, dxGeom* o1, dxGeom* o2, int flags,
    ...
    if (*pushing_neg) {
        ...
        const dReal* neg_vertices[3] = { cast_fp(V_array[neg_tri.T->verts[0]]),
    ...
    for (I = B; I != E; ++I) {
        ...
        CDB::TRI* T = T_array + *I;
        const Point vertices[3] = { Point((dReal*)&V_array[T->verts[0]]),
        ...
        if (tri.dist < 0.f) {
            ...
            if ((!(last_pos_dist < 0.f)) || b_pushing)
                if (__aabb_tri(Point(p), Point((float*)&AABB), vertices)) {
                    ...
                    bool contain_pos = TriContainPoint(vertices[0], vertices[1], vertices[2],
                                                       tri.norm, tri.side0, tri.side1, p);
                    bool b_pased = false;
                    if (!b_pushing && !gb_pased) {
                        if (!no_last_pos && !b_passable) {
#ifdef DEBUG
                            if (debug_output().ph_dbg_draw_mask().test(phDbgDrawTriTrace))
                                debug_output().DBG_DrawLine(cast_fv(last_pos), cast_fv(p),
                            ...
                            if (debug_output().ph_dbg_draw_mask().test(phDbgDrawTriPoint))
                                debug_output().DBG_DrawPoint(cast_fv(tri_point), 0.01f,
                                                             D3DCOLOR_XRGB(255, 0, 255));
#endif
                            bool was_intersect = intersect;
                            intersect = intersect ||
                                        TriContainPoint(vertices[0], vertices[1], vertices[2],
                                                        tri.norm, tri.side0, tri.side1, tri_point);
                            b_pased = intersect && !was_intersect;
                            gb_pased = b_pased || gb_pased;
#ifdef DEBUG
                            if (b_pased &&
                                debug_output().ph_dbg_draw_mask().test(phDbgDrawTriPoint)) {
        ...
        if (neg_depth < dInfinity) {
            ...
            if (include) {
                ...
                if (neg_tri_contains_point) {
                    int bret = primitive.CollidePlain(neg_tri.side0, neg_tri.side1, neg_tri.norm,
                                                      neg_tri.T, neg_tri.dist, o1, o2, flags,
        ...
        if (include) {
            ...
            if (ret < flags - 10)
                bret = primitive.CollidePlain(
                    b_neg_tri.side0, b_neg_tri.side1, b_neg_tri.norm, b_neg_tri.T, b_neg_tri.dist,
                    o1, o2, flags, CONTACT(contact, *pushing_neg ? ret * skip : 0), skip);
            *pushing_b_neg = !!bret;
            if (*pushing_neg)
                ret += bret;
