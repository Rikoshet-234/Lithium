...
#pragma warning(disable : 4245)
            if (material_1->Flags.test(SGameMtl::flSlowDown) &&
                !(usr_data_2->pushing_neg || usr_data_2->pushing_b_neg))
#pragma warning(pop)
            {
...
#pragma warning(disable : 4245)
            if (material_2->Flags.test(SGameMtl::flSlowDown) &&
                !(usr_data_1->pushing_neg || usr_data_1->pushing_b_neg))
#pragma warning(pop)
            {
        ...
        if (flags_1.test(SGameMtl::flBounceable) && flags_2.test(SGameMtl::flBounceable)) {
            ...
            surface.bounce_vel =
                std::max(material_1->fPHBounceStartVelocity, material_2->fPHBounceStartVelocity);
...
// body - static case
float E_NlS(dBodyID body, const dReal* norm,
...
float E_NLD(dBodyID b1, dBodyID b2, const dReal* norm) // norm - from 2 to 1
{
    ...
    dVector3 c_mas_impuls = { impuls1[0] + impuls2[0], impuls1[1] + impuls2[1],
                              impuls1[2] + impuls2[2] };
    dReal cmass = m1.mass + m2.mass;
    dVector3 c_mass_vel = { c_mas_impuls[0] / cmass, c_mas_impuls[1] / cmass,
