...
bool test_sides(const Fvector& center, const Fvector& side_dir, const Fvector& fv_dir,
                const Fvector& box, int tri_id) {
    Triangle tri;
...
void CPHSimpleCharacter::TestPathCallback(bool& do_colide, bool bo1, dContact& c,
...
void CPHSimpleCharacter::PhDataUpdate(dReal /**step/**/) {
    ...
    if (!fis_zero(l_air))
        dBodyAddForce(m_body, -linear_velocity[0] * l_air, -linear_velocity[1] * l_air,
...
void CPHSimpleCharacter::PhTune(dReal step) {
    ...
    if (debug_output().ph_dbg_draw_mask().test(phDbgCharacterControl)) {
        if (b_air_contact_state)
            debug_output().DBG_DrawPoint(cast_fv(dBodyGetPosition(m_body)), m_radius,
    ...
    if ((ud->pushing_neg || ud->pushing_b_neg) && !b_death_pos) {
        ...
        //#ifdef DEBUG
        //		Msg("death pos
        //%f2.2,%f2.2,%f2.2",ud->last_pos[0],ud->last_pos[1],ud->last_pos[2]); #endif
        Fvector pos;
    ...
    if (is_control) {
        ...
        if (!b_lose_control || b_clamb_jump) //)&&!b_external_impulse
            dBodyAddForce(m_body,
                          ...
                          -m.mass * (50.f) *
                              (!b_lose_control && !(is_contact || (b_any_contacts))), //&&!b_climb
    ...
    if (b_jumping) {
        ...
        if (amag > 0.f)
            if (dif[0] * m_acceleration.x / amag + dif[2] * m_acceleration.z / amag < 0.3f) {
                Fvector jump_fv =
                    m_acceleration; //{ m_acceleration.x/amag*1000.f,0,m_acceleration.z/amag*1000.f
                                    //}
...
bool CPHSimpleCharacter::ValidateWalkOnObject() {
    ...
    if (b_clamb_jump) {
        ...
        if ( //! b_valide_wall_contact||
             //(dDOT(dif,dif)>CLAMB_DISTANCE*CLAMB_DISTANCE))	//	(m_wall_contact_normal[1]>
             //M_SQRT1_2) ||							]
        ...
        !b_side_contact) //&& dDOT(m_wall_contact_normal,m_ground_contact_normal)<.9f
    {
        ...
        if (((m_wall_contact_position[0] - m_ground_contact_position[0]) * m_control_force[0] +
             (m_wall_contact_position[2] - m_ground_contact_position[2]) * m_control_force[2]) >
                0.05f &&
    ...
    if (b_valide_wall_contact && (m_contact_count > 1) && b_clamb_jump)
        if (dFabs(
                ...
                (m_wall_contact_position[2] - m_ground_contact_position[2])) >
...
void CPHSimpleCharacter::ApplyAcceleration() {
    ...
    if (b_clamb_jump && b_valide_wall_contact) {
        ...
        if (b_valide_ground_contact &&
            (m_ground_contact_normal[1] > M_SQRT1_2)) { // M_SQRT1_2//0.8660f
    ...
    if (!m_elevator_state.ClimbingState() && b_clamb_jump) { //&&m_wall_contact_normal[1]<M_SQRT1_2
        ...
        m_control_force[0] =
            m_control_force[0] * accel[0] >= 0.f ? m_control_force[0] : -m_control_force[0];
        m_control_force[2] =
            m_control_force[2] * accel[2] >= 0.f ? m_control_force[2] : -m_control_force[2];
    }
...
void CPHSimpleCharacter::SafeAndLimitVelocity() {
    ...
    if (dV_valid(linear_velocity)) {
        ...
        if (mag > l_limit) { // CutVelocity(m_l_limit,m_w_limit);
            if (!fis_zero(l_limit)) {
                ...
                if (b_lose_ground && linear_velocity[1] < 0.f &&
                    ...
                    dBodySetLinearVel(m_body, linear_velocity[0] / f, linear_velocity[1],
...
u16 CPHSimpleCharacter::RetriveContactBone() {
    ...
    collide::ray_defs Q(m_collision_damage_info.HitPos(), dir, m_radius,
    ...
    if (inl_ph_world().ObjectSpace().RayQuery(RQR, m_phys_ref_object->ObjectCollisionModel(), Q)) {
        ...
        pos_in_object.sub(m_collision_damage_info.HitPos(),
                          m_phys_ref_object->ObjectPosition()); // vector from object center to
                                                                // contact position currently in
                                                                // global frame
        Fmatrix object_form;
        object_form.set(m_phys_ref_object->ObjectXFORM());
        object_form.transpose();
        object_form.transform_dir(pos_in_object); // project pos_in_object on object axes now it is
                                                  // position of contact in object frame
...
/////////////////////////////////////////////////////////////////
void CPHSimpleCharacter::InitContact(dContact* c, bool& do_collide, u16 material_idx_1,
                                     u16 material_idx_2) {
    ...
    bool bo1 = (g1 == m_wheel) || g1 == m_cap_transform || g1 == m_shell_transform ||
               g1 == m_hat_transform;
    ...
    if (object) {
        ...
        if (g1 == m_wheel || g2 == m_wheel) {
            dxGeomUserData* ud =
                bo1 ? retrieveGeomUserData(c->geom.g2) : retrieveGeomUserData(c->geom.g1);
    ...
    float soft_param =
        dumping_rate + normal[1] * (1.f - dumping_rate); //=(1.f-normal[1])*dumping_rate +normal[1]
...
void CPHSimpleCharacter::FootProcess(dContact* c, bool& do_collide, bool bo) {
    ...
    if (m_elevator_state.ClimbingState() || !is_control || !b_clamb_jump || b_was_side_contact ||
        b_side_contact || b_jumping || b_jump) {
...
//=static_cast<IPhysicsShellHolder*>(Level().Objects.net_Find(m_obj_id)); 	if(!object)return
//u16(-1); 	IDamageSource* ds=m_object->cast_IDamageSource(); 	if(ds) return
//ds->Initiator(); 	return u16(-1);
//}
...
void CPHSimpleCharacter::TestRestrictorContactCallbackFun(bool& do_colide, bool bo1, dContact& c,
                                                          SGameMtl* material_1,
                                                          SGameMtl* material_2) {
    ...
    CPHActorCharacter* actor_character =
        (static_cast<CPHCharacter*>(obj_data->ph_object))->CastActorCharacter();
    if (!actor_character)
        return;
    CPHSimpleCharacter* ch_this =
        static_cast<CPHSimpleCharacter*>(retrieveGeomUserData(g_this)->ph_object);
    VERIFY(ch_this);
...
bool PickMaterial(u16& res_material_idx, const Fvector& pos_, const Fvector& dir_, float range_,
    ...
    while (
        inl_ph_world().ObjectSpace().RayPick(pos, dir, range, collide::rqtBoth, R, ignore_object)) {
