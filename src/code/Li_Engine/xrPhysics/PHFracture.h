...
class CPHFracture : public CShellSplitInfo {
    ...
    dMass m_firstM;
    dMass m_secondM;
    // when breaked m_pos_in_element-additional force m_break_force-additional torque -x additional
    // torque-y add_torque_z - additional torque z
    float m_break_force;
...
class CPHFracturesHolder // stored in CPHElement
{
    ...
    PH_IMPACT_STORAGE
        m_impacts; // filled in anytime from CPHElement applyImpulseTrace cleared in PhDataUpdate
...
public:
    ...
    bool PhDataUpdate(CPHElement* element); // collect joints and external impacts in fractures
                                            // Update which set m_fractured; called from PhDataUpdate
                                            // of ShellSplitterHolder returns true if has breaks
