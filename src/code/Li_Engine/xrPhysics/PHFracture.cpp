...
void CPHFracturesHolder::PassEndFractures(u16 from, CPHElement* dest) {
    ...
    if (i_from + 1 != i_to) // insure it!!
    {
        ...
        dest_fract_holder->m_fractures.insert(dest_fract_holder->m_fractures.end(), i_from + 1,
                                              i_to);
...
void CPHFracturesHolder::InitNewElement(CPHElement* element, const Fmatrix& shift_pivot,
                                        float density) {
...
#ifdef DBG_BREAK
    Msg("bone_id %d break_torque - %f(max %f) break_force %f (max %f) breaked %d", m_bone_id,
        btm_dbg, m_break_torque, bfm, m_break_force, m_breaked);
#endif
    return m_breaked;
}
