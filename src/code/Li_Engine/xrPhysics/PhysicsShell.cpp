...
CPhysicsJoint* P_create_Joint(CPhysicsJoint::enumType type, CPhysicsElement* first,
...
CPhysicsShell* __stdcall P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                                       BONE_P_MAP* bone_map, bool not_set_bone_callbacks) {
    VERIFY(obj);
...
CPhysicsShell* P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                             BONE_P_MAP* p_bone_map, LPCSTR fixed_bones) {
...
void phys_shell_verify_model(IKinematics& K) {
    ...
    VERIFY2(has_physics_collision_shapes(K),
            make_string("Can not create physics shell for model %s because it has no physics "
...
void phys_shell_verify_object_model(IPhysicsShellHolder& O) {
    ...
    // VERIFY2( V, make_string( "Can not create physics shell for object %s it has no model",
    // O.ObjectName() )/*+ make_string("\n object dump: \n") + dbg_object_full_dump_string( &O )*/ );

    // IKinematics		*K = V->dcast_PKinematics();

    IKinematics* K = O.ObjectKinematics();

    VERIFY2(K, make_string("Can not create physics shell for object %s, model %s is not skeleton",
                           O.ObjectName(), O.ObjectNameVisual()));

    VERIFY2(
        has_physics_collision_shapes(*K),
        make_string(
            "Can not create physics shell for object %s, model %s has no physics collision shapes "
            "set",
            O.ObjectName(),
    ...
    VERIFY2(valid_pos(O.ObjectXFORM().c),
            dbg_valide_pos_string(O.ObjectXFORM().c, &O, "create physics shell"));
}
...
bool __stdcall can_create_phys_shell(string1024& reason, IPhysicsShellHolder& O) {
    ...
    if (!K) {
        xr_strcpy(
            reason,
            make_string("Can not create physics shell for object %s, model %s is not skeleton",
                        O.ObjectName(), O.ObjectNameVisual())
                .c_str());
        return false;
    }
    if (!has_physics_collision_shapes(*K)) {
        xr_strcpy(reason, make_string("Can not create physics shell for object %s, model %s has no "
                                      "physics collision shapes set",
                                      O.ObjectName(), O.ObjectNameVisual())
                              .c_str());
        return false;
    }
...
#ifdef DEBUG
        xr_strcpy(reason,
                  dbg_valide_pos_string(O.ObjectXFORM().c, &O, "create physics shell").c_str());
#else
        xr_strcpy(reason,
                  make_string("create physics shell: object position is not valid").c_str());
#endif
        return false;
    }
    return result;
...
float NonElasticCollisionEnergy(CPhysicsElement* e1, CPhysicsElement* e2,
