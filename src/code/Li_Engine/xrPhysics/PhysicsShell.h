...
// ABSTRACT:
class CPhysicsBase;
extern XRPHYSICS_API void get_box(const CPhysicsBase* shell, const Fmatrix& form, Fvector& sz,
                                  Fvector& c);
...
public:
    ...
    virtual void get_Extensions(const Fvector& axis, float center_prg, float& lo_ext,
    ...
    virtual void dbg_draw_geometry(float scale, u32 color,
...
public:
    ...
    virtual void add_Mass(const SBoneShape& shape, const Fmatrix& offset,
                          const Fvector& mass_center, float mass, CPHFracture* fracture = NULL) = 0;
...
XRPHYSICS_API float NonElasticCollisionEnergy(CPhysicsElement* e1, CPhysicsElement* e2,
...
public:
    ...
    virtual void SetAxisDirVsFirstElement(const float x, const float y, const float z,
                                          const int axis_num) = 0;
    virtual void SetAxisDirVsSecondElement(const float x, const float y, const float z,
                                           const int axis_num) = 0;
    ...
    virtual void SetLimitsVsSecondElement(const float low, const float high,
                                          const int axis_num) = 0;
    ...
    virtual bool isBreakable() = 0;
    virtual void SetForceAndVelocity(const float force, const float velocity = 0.f,
...
public:
    ...
    virtual CPhysicsElement* get_Element(const std::string_view bone_name) = 0;
    ...
    virtual CPhysicsJoint* get_Joint(const std::string_view bone_name) = 0;
...
XRPHYSICS_API CPhysicsShell* P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                                           LPCSTR fixed_bones);
XRPHYSICS_API CPhysicsShell* P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                                           U16Vec& fixed_bones);
XRPHYSICS_API CPhysicsShell* P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                                           BONE_P_MAP* bone_map, LPCSTR fixed_bones);

extern "C" XRPHYSICS_API
    CPhysicsShell* __stdcall P_build_Shell(IPhysicsShellHolder* obj, bool not_active_state,
                                           BONE_P_MAP* bone_map = 0,
                                           bool not_set_bone_callbacks = false);

XRPHYSICS_API CPhysicsShell* P_build_SimpleShell(IPhysicsShellHolder* obj, float mass,
                                                 bool not_active_state);
XRPHYSICS_API void ApplySpawnIniToPhysicShell(CInifile const* ini, CPhysicsShell* physics_shell,
                                              bool fixed);
...
extern "C" XRPHYSICS_API bool __stdcall can_create_phys_shell(string1024& reason,
