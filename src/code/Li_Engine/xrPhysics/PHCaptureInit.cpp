...
bool can_capture(CPHCharacter* a_character, IPhysicsShellHolder* a_taget_object) {
    if (!a_taget_object || !a_taget_object->ObjectPPhysicsShell() ||
        !a_taget_object->ObjectPPhysicsShell()->isActive() || a_taget_object->IsInventoryItem() ||
        !a_character || !a_character->b_exist || !a_character->PhysicsRefObject() ||
...
CPHCapture::CPHCapture(CPHCharacter* a_character, IPhysicsShellHolder* a_taget_object,
    ...
    : m_joint(NULL), m_ajoint(NULL), m_body(NULL), m_taget_object(a_taget_object),
      m_character(a_character), b_disabled(false), b_character_feedback(false), e_state(cstFree),
      ///////////////////////////////////////////////////////////////
      m_taget_element(0),
      // dJointFeedback		m_joint_feedback;
      // ,
...

{
    ...
    m_taget_element =
        m_taget_object->ObjectPPhysicsShell()->NearestToPoint(m_capture_bone->mTransform.c, cb);
    if (!m_taget_element)
        return;
    Init();
}
...
void CPHCapture::Init() {
    ...
    m_pull_force =
        pool_force_factor * ph_world->Gravity() * m_taget_element->PhysicsShell()->getMass();
