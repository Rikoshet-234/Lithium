...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
IPHCapture* phcapture_create(CPHCharacter* ch, IPhysicsShellHolder* object,
...
bool CPHCapture::Invalid() {
    return !m_taget_object->ObjectPPhysicsShell() ||
           !m_taget_object->ObjectPPhysicsShell()->isActive() || !m_character->b_exist;
}
...
void CPHCapture::PullingUpdate() {
    if (!m_taget_element->isActive() ||
        inl_ph_world().Device().dwTimeGlobal - m_time_start > m_capture_time) {
        Release();
        return;
    }
    ...
    if (dist < m_capture_distance) {
        ...
        dJointSetBallAnchor(m_joint, capture_bone_position.x, capture_bone_position.y,
                            capture_bone_position.z);
...
void CPHCapture::CapturedUpdate() {
    ...
    if (b_character_feedback && mag > m_capture_force / 2.2f) {
        ...
        m_character->ApplyForce(m_joint_feedback.f1[0] / f, m_joint_feedback.f1[1] / f,
...
void CPHCapture::object_contactCallbackFun(bool& do_colide, bool bo1, dContact& c,
