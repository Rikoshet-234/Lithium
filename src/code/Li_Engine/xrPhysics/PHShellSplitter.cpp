...
void CPHShellSplitterHolder::PassEndSplitters(const CShellSplitInfo& spl_inf, CPHShell* dest,
                                              u16 jt_add_shift, u16 el_add_shift) {
    ...
    auto i_elem = source_elements.begin(),
    ...
    //////correct data in fractures for elements allready added to dest with fractures from
    ///source///////
...
shell_root CPHShellSplitterHolder::ElementSingleSplit(const element_fracture& split_elem,
        ...
        for (; i != e; ++i) {
            ...
            if (joint->PFirst_element() == source_element) {
                ...
                CPHElement *el1 = (split_elem.first),
                           *el2 = static_cast<CPHElement*>(joint->PSecond_element());
    ...
    // the last new shell will have all splitted old elements end joints and one new element
    // reattached to old joint  m_splitters.erase(m_splitters.begin()+aspl);  now aspl points to the
    // next splitter
    if ((split_elem.first)
            ->FracturesHolder()) // if this element can be splitted add a splitter for it
    ...
    m_pShell->PassEndElements(split_elem.second.m_start_el_num, split_elem.second.m_end_el_num,
                              new_shell_last_desc);

    m_pShell->PassEndJoints(split_elem.second.m_start_jt_num, split_elem.second.m_end_jt_num,
                            new_shell_last_desc);
...
void CPHShellSplitterHolder::InitNewShell(CPHShell* shell) {
    ...
    if (m_pShell->IsGroupObject())
        CPHCollideValidator::RegisterObjToGroup(m_pShell->collide_bits(),
                                                *static_cast<CPHObject*>(shell));
}
