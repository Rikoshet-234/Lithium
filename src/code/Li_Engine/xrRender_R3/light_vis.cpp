...
void light::vis_prepare() {
    ...
    // sr[%f]/r[%f]",VPUSH(spatial.center),VPUSH(position),spatial.radius,range);  Msg
    // ("dist:%f, sa:%f",Device.vCameraPosition.distance_to(spatial.center),safe_area);
    bool skiptest = false;
    ...
    if (skiptest || Device.vCameraPosition.distance_to(spatial.sphere.P) <=
                        (spatial.sphere.R * 1.01f + safe_area)) { // small error
        vis.visible = true;
        vis.pending = false;
