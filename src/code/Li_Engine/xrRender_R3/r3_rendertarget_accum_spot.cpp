#include "stdafx.h"
#include "../xrRenderCommon/du_cone.h"
...
void CRenderTarget::accum_spot(light* L) {
    ...
    BOOL bIntersect = FALSE; // enable_scissor(L);
    {
        ...
        if (!RImplementation.o.dx10_msaa)
            ...
            RCache.set_Stencil(TRUE, D3DCMP_LESSEQUAL, 0x01, 0x7f, 0x7f, D3DSTENCILOP_KEEP,
                               D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE);
        draw_volume(L);
    }
...
void CRenderTarget::accum_volumetric(light* L) {
    ...
    */
    ...
    // Draw volume with projective texgen
    {
        ...
        RImplementation.o.smapsize); if (L->X.S.transluent)	_id	= SE_L_TRANSLUENT; else if
        (bFullSize)	_id	= SE_L_FULLSIZE; else					_id	=
        SE_L_NORMAL; } else { _id						= SE_L_UNSHADOWED;
                m_Shadow				= m_Lmap;
        }
        RCache.set_Element			(shader->E[ _id ]	);
        */
    ...
    /*
       ...
       (s_accum_mask->E[SE_MASK_ACCUM_VOL]	); RCache.set_c				("m_texgen",
       m_Texgen); RCache.set_c				("m_texgen_J",		m_Texgen_J	);
                    draw_volume					(L);
            }
    */
    // CHK_DX		(HW.pDevice->SetRenderState(D3DRS_SCISSORTESTENABLE,FALSE));
