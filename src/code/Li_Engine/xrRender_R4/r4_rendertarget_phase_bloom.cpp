...
void CRenderTarget::phase_bloom() {
    ...
    if (ps_r2_ls_flags.test(R2FLAG_FASTBLOOM)) {
        ...
        // Transfer into Bloom1, vertical filter
        {
            ...
            Fvector2 half = { .5f / _w, .5f / _h };
            Fvector4 a_0 = { half.x, half.y, half.y, half.x }; // center
            Fvector4 a_1 = { half.x, a_0.y - one.y - half.y, half.y + one.y + a_0.z,
                             half.x };                                       // -1,+1i
    ...
    // if (RImplementation.o.fp16_blend)		g_pGamePersistent->Environment().RenderFlares	();	//
    // lens-flares
    bool _menu_pp = g_pGamePersistent ? g_pGamePersistent->OnRenderPPUI_query() : false;
    if (_menu_pp) {
        // CHK_DX				(HW.pDevice->Clear( 0L, NULL, D3DCLEAR_TARGET,	0,	1.0f,
        // 0L));
