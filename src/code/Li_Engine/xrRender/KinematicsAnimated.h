...
#include "../xrRenderCommon/KinematicAnimatedDefs.h"
...
public:
    ...
    virtual void LL_BuldBoneMatrixDequatize(const CBoneData* bd, u8 channel_mask,
                                            SKeyTable& keys) = 0;
    virtual void LL_BoneMatrixBuild(CBoneInstance& bi, const Fmatrix* parent,
    ...
    virtual CBlend* LL_PlayCycle(u16 partition, MotionID motion, BOOL bMixing, float blendAccrue,
    ...
    //	void						LL_FadeCycle	(u16 partition, float	falloff, u8 mask_channel =
    //(1<<0));
    ...
    virtual CBlend* PlayCycle(LPCSTR N, BOOL bMixIn = TRUE, PlayCallback Callback = 0,
                              LPVOID CallbackParam = 0, u8 channel = 0) = 0;
    virtual CBlend* PlayCycle(MotionID M, BOOL bMixIn = TRUE, PlayCallback Callback = 0,
                              LPVOID CallbackParam = 0, u8 channel = 0) = 0;
    virtual CBlend* PlayCycle(u16 partition, MotionID M, BOOL bMixIn = TRUE,
                              PlayCallback Callback = 0, LPVOID CallbackParam = 0,
                              u8 channel = 0) = 0;
    // fx'es
