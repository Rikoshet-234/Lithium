...
//////////////////////////////////////////////////////////////////////////
/*
IC void mk_vertex					(CLightR_Vertex& D, Fvector& P, Fvector& N,
Fvector& C, float r2)
{
        D.P.set	(P);
        D.N.set	(P);
...
continue; float	alpha		= Device.vCameraPosition.distance_to(PPL.position)/MAX_DISTANCE; if
(alpha>=1)
continue; if (!RImplementation.ViewBase.testSphere_dirty (PPL.position,PPL.range))	continue;
...
(CLightR_Vertex*)RCache.Vertex.Lock(triLock*3,hGeom->vb_stride,vOffset);
                        ...
                        for (u32 t=0; t<triCount; t++)
                        {
                                ...
                                if (actual>=triLock)
                                {
                                        ...
                                        if (actual) RCache.Render
(D3DPT_TRIANGLELIST,vOffset,actual); actual						= 0; triLock
= _min(256u,triCount-t); VB							=
(CLightR_Vertex*)RCache.Vertex.Lock(triLock*3,hGeom->vb_stride,vOffset);
                                }
                        }
