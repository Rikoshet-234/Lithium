...
void CRender::create() {
    ...
    // c-setup
    dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup("L_dynamic_pos",
                                                                      &r1_dlight_binder_PR);
    dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup("L_dynamic_color",
                                                                      &r1_dlight_binder_color);
    dxRenderDeviceRender::Instance().Resources->RegisterConstantSetup("L_dynamic_xform",
    ...
    if (strstr(Core.Params, "-nocolormap"))
        o.color_mapping = FALSE;
    Msg("* color_mapping: %s, dev(%d),need(%d)", o.color_mapping ? "used" : "unavailable", v_dev,
        v_need);
...
void CRender::Calculate() {
    ...
    if (rmPortals) {
        ...
        for (int K = 0; K < Sectors_xrc.r_count(); K++) {
            CPortal* pPortal =
                (CPortal*)Portals[rmPortals->get_tris()[Sectors_xrc.r_begin()[K].id].dummy];
                ...
                if (spatial->spatial.type & STYPE_RENDERABLE) {
                    for (u32 v_it = 0; v_it < sector->r_frustums.size(); v_it++) {
                        ...
                        if (!View->testSphere_dirty(
                                ...
                                spatial->spatial.sphere
                                    .R) /*&& (spatial->spatial.type & STYPE_RENDERABLE)*/)
                            continue;
                        // renderable
