...
void CRender::level_Load(IReader* fs) {
    ...
    // Shaders
    g_pGamePersistent->LoadTitleOld("st_loading_shaders");
    ...
    // VB,IB,SWI
    g_pGamePersistent->LoadTitleOld("st_loading_geometry");
    g_pGamePersistent->LoadTitle();
    ...
    // Visuals
    g_pGamePersistent->LoadTitleOld("st_loading_spatial_db");
    g_pGamePersistent->LoadTitle();
    ...
    // Details
    g_pGamePersistent->LoadTitleOld("st_loading_details");
    g_pGamePersistent->LoadTitle();
    Details->Load();

    // Sectors
    g_pGamePersistent->LoadTitleOld("st_loading_sectors_portals");
    g_pGamePersistent->LoadTitle();
    LoadSectors(fs);
...
void CRender::LoadBuffers(CStreamReader* base_fs) {
    ...
    u32 dwUsage =
        D3DUSAGE_WRITEONLY | (HW.Caps.geometry.bSoftware ? D3DUSAGE_SOFTWAREPROCESSING : 0);
    ...
    if (base_fs->find_chunk(fsL_VB)) {
        ...
        for (u32 i = 0; i < count; i++) {
            ...
            // Create and fill
            BYTE* pData = 0;
            R_CHK(HW.pDevice->CreateVertexBuffer(vCount * vSize, dwUsage, 0, D3DPOOL_MANAGED,
                                                 &VB[i], 0));
            HW.stats_manager.increment_stats(vCount * vSize, enum_stats_buffer_type_vertex,
                                             D3DPOOL_MANAGED);
            R_CHK(VB[i]->Lock(0, 0, (void**)&pData, 0));
            fs->r(pData, vCount * vSize);
            //			CopyMemory			(pData,fs->pointer(),vCount*vSize);	//.???? copy while skip
            //T&B
            VB[i]->Unlock();
    ...
    if (base_fs->find_chunk(fsL_IB)) {
        ...
        for (u32 i = 0; i < count; i++) {
            ...
            // Create and fill
            BYTE* pData = 0;
            R_CHK(HW.pDevice->CreateIndexBuffer(iCount * 2, dwUsage, D3DFMT_INDEX16,
                                                D3DPOOL_MANAGED, &IB[i], 0));
            HW.stats_manager.increment_stats(iCount * 2, enum_stats_buffer_type_index,
                                             D3DPOOL_MANAGED);
