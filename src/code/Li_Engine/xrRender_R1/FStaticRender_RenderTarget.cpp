...
BOOL CRenderTarget::Create() {
    ...
    if ((rtHeight != Device.dwHeight) || (rtWidth != Device.dwWidth)) {
        R_CHK(HW.pDevice->CreateDepthStencilSurface(rtWidth, rtHeight, HW.Caps.fDepth,
                                                    D3DMULTISAMPLE_NONE, 0, TRUE, &ZB, NULL));
    } else {
        ZB = HW.pBaseZB;
        ZB->AddRef();
    }
    ...
    R_CHK(HW.pDevice->CreateDepthStencilSurface(512, 512, HW.Caps.fDepth, D3DMULTISAMPLE_NONE, 0,
                                                TRUE, &pTempZB, NULL));
    ...
    HW.pDevice->CreateOffscreenPlainSurface(rtWidth, rtHeight, HW.Caps.fTarget, D3DPOOL_SYSTEMMEM,
                                            &pFB, NULL);
...
void CRenderTarget::calc_tc_noise(Fvector2& p0, Fvector2& p1) {
    ...
    //.	u32			tw					= iCeil(float(T->get_Width
    //())*param_noise_scale+EPS_S);
    //.	u32			th					= iCeil(float(T->get_Height
    //())*param_noise_scale+EPS_S);
...
BOOL CRenderTarget::Perform() {
    return Available() && (((BOOL)RImplementation.m_bMakeAsyncSS) || NeedPostProcess() ||
                           (ps_r__Supersample > 1) || (frame_distort == (Device.dwFrame - 1)));
}
...
struct TL_2c3uv {
    ...
    u32 color1;
    Fvector2 uv[3];
    IC void set(float x, float y, u32 c0, u32 c1, float u0, float v0, float u1, float v1, float u2,
                float v2) {
        p.set(x, y, EPS_S, 1.f);
        color0 = c0;
        color1 = c1;
...
void CRenderTarget::End() {
    ...
    // Msg
    // ("param_gray:%f(%d),param_noise:%f(%d)",param_gray,gblend,param_noise,nblend); Msg
    // ("base: %d,%d,%d",	color_get_R(p_color),		color_get_G(p_color),
    // color_get_B(p_color));
    // Msg				("gray: %d,%d,%d",	color_get_R(p_gray),		color_get_G(p_gray),
    // color_get_B(p_gray));
