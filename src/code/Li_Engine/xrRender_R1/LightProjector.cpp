        ...
        if (current) {
            ...
            if (0 == spatial)
                ...
                if (0 == spatial->spatial.sector) {
                    ...
                    if (obj)
                        Msg("! Invalid object '%s' position. Outside of sector structure.",
                            obj->cName().c_str());
                    current = 0;
                }
            }
...
void CLightProjector::calculate() {
    ...
    CHK_DX(HW.pDevice->Clear(0, 0, D3DCLEAR_ZBUFFER | (HW.Caps.bStencil ? D3DCLEAR_STENCIL : 0), 0,
                             1, 0));
    ...
    for (u32 c_it = 0; c_it < cache.size(); c_it++) {
        ...
        // Msg
        // ("[%f,%f,%f]-%f",C.C.x,C.C.y,C.C.z,C.O->renderable.visual->vis.sphere.R); calculate
        // projection-matrix
        Fmatrix mProject;
        ...
        CHK_DX(HW.pDevice->Clear(0, 0, D3DCLEAR_TARGET,
                                 color_rgba_f(cap.x, cap.y, cap.z, (cap.x + cap.y + cap.z) / 4.f),
                                 1, 0));
    ...
    /*
    {
            // Fill vertex buffer
            u32							Offset;
            FVF::TL4uv* pv				= (FVF::TL4uv*) RCache.Vertex.Lock
    (4,geom_Blur.stride(),Offset); RImplementation.ApplyBlur4
    (pv,P_rt_size,P_rt_size,P_blur_kernel); RCache.Vertex.Unlock
    (4,geom_Blur.stride());
...
void CLightProjector::render() {
    ...
    (P_rt_size+.5f)/P_rt_size);
            ...
            pv->set(0,			0,			.0001f,.9999f, C, p0.x, p0.y);
