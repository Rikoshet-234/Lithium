...
void dxRainRender::Render(CEffect_Rain& owner) {
    ...
    Fvector upper;
    upper.set(Device.vCameraPosition.x, Device.vCameraPosition.y + source_offset,
    ...
    u32 vOffset;
    FVF::LIT* verts =
        (FVF::LIT*)RCache.Vertex.Lock(desired_items * 4, hGeom_Rain->vb_stride, vOffset);
    FVF::LIT* start = verts;
    ...
    for (u32 I = 0; I < owner.items.size(); I++) {
        ...
        static Fvector2 UV[2][4] = { { { 0, 1 }, { 0, 0 }, { 1, 1 }, { 1, 0 } },
                                     { { 1, 0 }, { 1, 1 }, { 0, 0 }, { 0, 1 } } };
