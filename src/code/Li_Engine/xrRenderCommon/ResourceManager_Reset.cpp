...
void CResourceManager::reset_begin() {
    ...
    //	DX10 cut 	for (map_RTCIt rtc_it=m_rtargets_c.begin(); rtc_it!=m_rtargets_c.end();
    //rtc_it++) 	DX10 cut 		rtc_it->second->reset_begin();
...
#pragma todo("container is created in stack!")
      ...
      //	DX10 cut 		for (map_RTCIt rt_it=m_rtargets_c.begin();
      //rt_it!=m_rtargets_c.end(); rt_it++)	rt.push_back(rt_it->second);
      //	DX10 cut 		std::sort(rt.begin(),rt.end(),cmp_rtc);
      //	DX10 cut 		for (u32 _it=0; _it<rt.size(); _it++)
      //rt[_it]->reset_end	();
    }
...
void CResourceManager::Dump(bool bBrief) {
    ...
    if (!bBrief)
        mdump(m_rtargets);
    //	DX10 cut 	Msg		("* RM_Dump: rtargetsc : %d",		m_rtargets_c.size());	if(!bBrief)
    //mdump(m_rtargets_c);
