...
#else  //	USE_DX10
    R_CHK(HW.pDevice->CreateVertexBuffer(mSize, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0,
...
void* _VertexStream::Lock(u32 vl_Count, u32 Stride, u32& vOffset) {
    ...
    if ((vl_Count + vl_mPosition) >= vl_mSize) {
        ...
        if (res != D3D_OK)
            Msg(" pVB->Lock - failed: res = %d,mPosition = %d, bytes_need = %d, &pData = %x, "
                "LOCKFLAGS_FLUSH",
...
#else  //	USE_DX10
    R_CHK(HW.pDevice->CreateIndexBuffer(mSize, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC,
...
#if defined(USE_DX11)
    D3D_MAP MapMode =
        (dwFlags == LOCKFLAGS_APPEND) ? D3D_MAP_WRITE_NO_OVERWRITE : D3D_MAP_WRITE_DISCARD;
