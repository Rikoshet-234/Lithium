...
void CSector::traverse(CFrustum& F, _scissor& R_scissor) {
    ...
    for (u32 I = 0; I < m_portals.size(); I++) {
        ...
        if (PortalTraverser.i_options & CPortalTraverser::VQ_SCISSOR && (!PORTAL->bDualRender)) {
            ...
            // Msg	("bb(%s): (%f,%f)-(%f,%f), d=%f", PORTAL->bDualRender?"true":"false",bb.min.x,
            // bb.min.y, bb.max.x, bb.max.y,depth);
            if (depth < EPS) {
                scissor = R_scissor;
                ...
                // perform intersection (this is just to be sure, it is probably clipped in 3D
                // already)
                ...
                // Msg	("scissor: (%f,%f)-(%f,%f)", scissor.min.x, scissor.min.y, scissor.max.x,
                // scissor.max.y); Check if box is non-empty
