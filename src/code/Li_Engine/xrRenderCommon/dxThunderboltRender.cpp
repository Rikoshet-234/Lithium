...
void dxThunderboltRender::Render(CEffect_Thunderbolt& owner) {
    ...
    IRender_DetailModel::fvfVertexOut* v_ptr =
        (IRender_DetailModel::fvfVertexOut*)RCache.Vertex.Lock(vCount_Lock, hGeom_model->vb_stride,
                                                               v_offset);
    ...
    // top
    {
        ...
        vecSx.mul(Device.vCameraRight,
                  owner.current->m_GradientTop->fRadius.x * owner.lightning_size);
        vecSy.mul(Device.vCameraTop,
                  -owner.current->m_GradientTop->fRadius.y * owner.lightning_size);
    ...
    // center
    {
        ...
        vecSy.mul(Device.vCameraTop,
                  -owner.current->m_GradientCenter->fRadius.y * owner.lightning_size);
