...
IC bool InterpolateAndDither(float* alpha255, u32 x, u32 y, u32 sx, u32 sy, u32 size,
                             int dither[16][16]) {
...
// Item.scale	= 0.1f;
            ...
            if (det_render_debug)
                draw_obb(mXform,
                         color_rgba(255, 0, 0,
                                    255)); // Fmatrix().mul_43( mXform, Fmatrix().scale(5,5,5) )
#endif
#endif
...
*/
// float c_f						=	1.f;	//Interpolate
// (gray255,x,z,d_size)+.5f;
// int c_dw						=	255;	//iFloor
// (c_f);  clamp							(c_dw,0,255);
// Item.C_dw						=	color_rgba
// (c_dw,c_dw,c_dw,255);
#if RENDER == R_R1
