...
//(&saved,D3DXIFF_TGA,pFB,0,0));
    ...
    case IRender_interface::SM_FOR_CUBEMAP: {
        ...
        VERIFY				(name);
        strconcat
        (sizeof(buf),buf,"ss_",Core.UserName,"_",timestamp(t_stemp),"_#",name); xr_strcat
        (buf,".tga"); IWriter*		fs	= FS.w_open	("$screenshots$",buf); R_ASSERT(fs);
        TGAdesc				p;
        p.format			= IMG_24B;
...
void CRender::ScreenshotImpl(ScreenshotMode mode, LPCSTR name, CMemoryWriter* memory_writer) {
    ...
    D3DLOCKED_RECT D;
    HRESULT hr;
    hr = HW.pDevice->CreateOffscreenPlainSurface(Device.dwWidth, Device.dwHeight, D3DFMT_A8R8G8B8,
    ...
    */
    ...
    case IRender_interface::SM_FOR_GAMESAVE: {
        ...
        hr = D3DXCreateTexture(HW.pDevice, GAMESAVE_SIZE, GAMESAVE_SIZE, 1, 0, D3DFMT_DXT1,
    ...
    case IRender_interface::SM_FOR_MPSENDING: {
        // texture
        ID3DTexture2D* texture = NULL;
        hr = D3DXCreateTexture(HW.pDevice, SM_FOR_SEND_WIDTH, SM_FOR_SEND_HEIGHT, 1, 0,
                               D3DFMT_R8G8B8, D3DPOOL_SCRATCH, &texture);
        if (hr != D3D_OK)
            goto _end_;
        if (NULL == texture)
            goto _end_;
    ...
    if (Target->rt_Color->fmt == D3DFMT_A16B16G16R16F) {
        ...
        while (pPixel != pEnd) {
            ...
            for (int i = 0; i < iProcessPixels; ++i) {
                *pPixel =
                    color_argb_f(1.0f, tmpArray[i * 4], tmpArray[i * 4 + 1], tmpArray[i * 4 + 2]);
