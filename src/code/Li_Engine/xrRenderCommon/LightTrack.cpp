...
void CROS_impl::update(IRenderable* O) {
    ...
    if (MODE & IRender_ObjectSpecific::TRACE_LIGHTS) {
        ...
        for (u32 lit = 0; lit < lights.size(); lit++) {
...
#if RENDER != R_R1
            float a = (1 / (L->attenuation0 + L->attenuation1 * d + L->attenuation2 * d * d) -
                       d * L->falloff) *
            ...
            float koef = (lights[lit].color.r + lights[lit].color.g + lights[lit].color.b) / 3.0f *
                         a * ps_r2_dhemi_light_scale;
...
void CROS_impl::calc_sun_value(Fvector& position, CObject* _object) {
    ...
    if (MODE & IRender_ObjectSpecific::TRACE_SUN) {
        if (--result_sun < 0) {
            ...
            sun_value = !(g_pGameLevel->ObjectSpace.RayTest(position, direction, 500.f,
                                                            collide::rqtBoth, &cache_sun, _object))
                            ? 1.f
                            : 0.f;
        }
    }
...
void CROS_impl::calc_sky_hemi_value(Fvector& position, CObject* _object) {
    ...
    for (int it = 0; it < result_count; it++) {
        if (result[it]) {
            accum_hemi(hemi_cube, Fvector3().set(hdir[it][0], hdir[it][1], hdir[it][2]),
...
void CROS_impl::prepare_lights(Fvector& position, IRenderable* O) {
    ...
    if (bTraceLights) {
...
#if RENDER != R_R1
        g_SpatialSpace->q_box(RImplementation.lstSpatial, 0, STYPE_LIGHTSOURCEHEMI, position,
                              bb_size);
#else
        ...
        for (s32 id = 0; id < s32(track.size()); id++) {
            ...
            if (g_pGameLevel->ObjectSpace.RayTest(LP, D.div(f), f, collide::rqtStatic, &I->cache,
                                                  _object))
                amount -= lt_dec;
            else
                amount += lt_inc;
