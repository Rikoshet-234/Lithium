...
u32 GetNVGpuNum() {
    ...
    for (u32 i = 0; i < logicalGPUCount; ++i) {
        status =
            NvAPI_GetPhysicalGPUsFromLogicalGPU(logicalGPUs[i], physicalGPUs, &physicalGPUCount);
        if (status == NVAPI_OK)
...
void CHWCaps::Update() {
    ...
    geometry.bVTF = (geometry_major >= 3) &&
                    HW.support(D3DFMT_R32F, D3DRTYPE_TEXTURE, D3DUSAGE_QUERY_VERTEXTEXTURE);
    ...
    Msg("* GPU vertex cache: %s, %d", (1 == vc.OptMethod) ? "recognized" : "unrecognized",
