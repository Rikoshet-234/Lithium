...
void CDetailManager::hw_Load_Geom() {
    ...
    R_CHK(HW.pDevice->CreateIndexBuffer(dwIndices * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED,
                                        &hw_IB, 0));
    HW.stats_manager.increment_stats_ib(hw_IB);

#endif //	USE_DX10
    Msg("* [DETAILS] Batch(%d), VB(%dK), IB(%dK)", hw_BatchSize, (dwVerts * vSize) / 1024,
        (dwIndices * 2) / 1024);
...
void CDetailManager::hw_Render() {
    // Render-prepare
    //	Update timer
    //	Can't use RDEVICE.fTimeDelta since it is smoothed! Don't know why, but smoothed value looks
    //more choppy!
...
// Build color
                    ...
                    if (dwBatch == hw_BatchSize) {
                        ...
                        RCache.get_ConstantCache_Vertex().get_array_f().dirty(c_base,
                                                                              c_base + dwBatch * 4);
