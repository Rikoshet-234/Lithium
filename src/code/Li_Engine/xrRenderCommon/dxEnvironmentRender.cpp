...
static u16 hbox_faces[20 * 3] = { 0, 2,  3,  3, 1, 0, 4, 5,  7,  7,  6,  4,  0,  1,  9,
    ...
    if (::Render->get_generation() == IRender_interface::GENERATION_R2) {
        ...
        if (HW.Caps.raster_major >= 3 && HW.Caps.geometry.bVTF) {
            ...
            mixRen.sky_r_textures.push_back(
                std::make_pair(u32(D3DVERTEXTEXTURESAMPLER0), tonemap)); //. hack
...
//	TODO: DX10: Implement environment parameters setting for DX10 (if necessary)
    ...
    CHK_DX(HW.pDevice->SetRenderState(D3DRS_FOGCOLOR,
...
void dxEnvironmentRender::RenderSky(CEnvironment& env) {
    ...
    // The state may be not set by RCache if the state is changed using API SetRenderState()
    // function before and the RCache flag will remain unchanged to it's old value.
    //
    RCache.set_Z(FALSE);
    RCache.set_Z(TRUE);
...
void dxEnvironmentRender::RenderClouds(CEnvironment& env) {
    ...
    // Fill vertex buffer
    v_clouds* pv =
        (v_clouds*)RCache.Vertex.Lock(env.CloudsVerts.size(), clouds_geom.stride(), v_offset);
    ...
    RCache.Render(D3DPT_TRIANGLELIST, v_offset, 0, env.CloudsVerts.size(), i_offset,
                  env.CloudsIndices.size() / 3);
