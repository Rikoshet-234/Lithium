...
void bwdithermap(int levels, int magic[16][16]) {
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            for (int k = 0; k < 4; k++)
                for (int l = 0; l < 4; l++)
                    magic[4 * k + i][4 * l + j] = (int)(0.5 + magic4x4[i][j] * magicfact +
    ...
    for (int _mz = 0; _mz < dm_cache1_line; _mz++) {
        for (int _mx = 0; _mx < dm_cache1_line; _mx++) {
            ...
            for (u32 _i = 0; _i < dwCC; _i++) {
                ...
                //				if ( ( _i + 1 ) < dwCC );
                //					_mm_prefetch( (char *) *MS.slots[ _i + 1 ]  ,
                //_MM_HINT_T1 );
                ...
                if (fcvPartial == res) {
                    u32 _mask = mask;
                    u32 _res =
                        View.testSAABB(S.vis.sphere.P, S.vis.sphere.R, S.vis.box.data(), _mask);
                    if (fcvNone == _res) {
                        continue; // invisible-view frustum
                    }
                }
                ...
                if (RDEVICE.dwFrame > S.frame) {
                    ...
                    if (dist_sq > fade_limit)
                        continue;
                    float alpha =
                        (dist_sq < fade_start) ? 0.f : (dist_sq - fade_start) / fade_range;
