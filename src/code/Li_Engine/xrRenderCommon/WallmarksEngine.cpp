...
void CWallmarksEngine::clear() {
    {
        for (auto p_it = marks.begin(); p_it != marks.end(); p_it++) {
            for (auto m_it = (*p_it)->static_items.begin(); m_it != (*p_it)->static_items.end();
                 m_it++)
...
void CWallmarksEngine::AddWallmark_internal(CDB::TRI* pTri, const Fvector* pVerts,
    ...
    // calculate adjacency
    {
        ...
        sml_collector.clear();
        sml_collector.add_face_packed_D(pVerts[pTri->verts[0]], pVerts[pTri->verts[1]],
                                        pVerts[pTri->verts[2]], 0);
        for (u32 t = 0; t < triCount; t++) {
            CDB::TRI* T = tris + xrc.r_begin()[t].id;
            if (T == pTri)
                continue;
            sml_collector.add_face_packed_D(pVerts[T->verts[0]], pVerts[T->verts[1]],
                                            pVerts[T->verts[2]], 0);
        }
...
void CWallmarksEngine::AddStaticWallmark(CDB::TRI* pTri, const Fvector* pVerts,
                                         const Fvector& contact_point, ref_shader hShader,
                                         float sz) {
...
ICF void FlushStream(ref_geom hGeom, ref_shader shader, u32& w_offset, FVF::LIT*& w_verts,
                     FVF::LIT*& w_start, BOOL bSuppressCull) {
