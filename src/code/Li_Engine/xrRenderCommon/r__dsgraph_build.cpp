...
void R_dsgraph_structure::r_dsgraph_insert_dynamic(dxRender_Visual* pVisual, Fvector& Center) {
    ...
    if (RImplementation.o.distortion && sh_d && sh_d->flags.bDistort &&
        pmask[sh_d->flags.iPriority / 2]) {
...
//	mapNormalVS::TNode*			Nvs		= map.insert		(&*pass.vs);
//#else	//	USE_DX10
//	mapNormalVS::TNode*			Nvs		= map.insert
//(pass.vs->vs); #endif	//	USE_DX10 	mapNormalPS::TNode*			Nps
//= Nvs->val.insert	(pass.ps->ps); #endif
...
void CRender::add_leafs_Dynamic(dxRender_Visual* pVisual) {
    ...
    case MT_SKELETON_RIGID: {
        ...
        if (pV->m_lod) {
            ...
            float ssa = CalcSSA(
                D, Tpos, pV->vis.sphere.R / 2.f); // assume dynamics never consume full sphere
            if (ssa < r_ssaLOD_A)
                _use_lod = TRUE;
        }
        if (_use_lod) {
