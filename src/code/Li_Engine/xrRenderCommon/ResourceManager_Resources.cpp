...
SGeometry* CResourceManager::CreateGeom(D3DVERTEXELEMENT9* decl, IDirect3DVertexBuffer9* vb,
...
SGeometry* CResourceManager::CreateGeom(u32 FVF, IDirect3DVertexBuffer9* vb,
                                        IDirect3DIndexBuffer9* ib) {
...
SVS* CResourceManager::_CreateVS(LPCSTR _name) {
    ...
    if (I != m_vs.end())
        ...
        _hr = ::Render->shader_compile(
            ...
            D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR /*| D3DXSHADER_PREFER_FLOW_CONTROL*/,
            &pShaderBuf, &pErrorBuf, NULL);
        //		_hr = D3DXCompileShader		(LPCSTR(fs->pointer()),fs->length(), NULL,
        //&Includer, "main", target, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf,
        //&pErrorBuf, NULL);
        FS.r_close(fs);

        if (SUCCEEDED(_hr)) {
            if (pShaderBuf) {
                _hr = HW.pDevice->CreateVertexShader((DWORD*)pShaderBuf->GetBufferPointer(),
                                                     &_vs->vs);
                if (SUCCEEDED(_hr)) {
                    LPCVOID data = NULL;
                    _hr = D3DXFindShaderComment((DWORD*)pShaderBuf->GetBufferPointer(),
...
SPS* CResourceManager::_CreatePS(LPCSTR name) {
    ...
    if (I != m_ps.end())
        ...
        _hr = ::Render->shader_compile(name, data, size, NULL, &Includer, c_entry, c_target,
                                       ...
                                       &pShaderBuf, &pErrorBuf, NULL);
        //_hr = D3DXCompileShader		(text,text_size, NULL, &Includer, c_entry, c_target,
        ...
        if (SUCCEEDED(_hr)) {
            if (pShaderBuf) {
                _hr =
                    HW.pDevice->CreatePixelShader((DWORD*)pShaderBuf->GetBufferPointer(), &_ps->ps);
                if (SUCCEEDED(_hr)) {
                    LPCVOID data = NULL;
                    _hr = D3DXFindShaderComment((DWORD*)pShaderBuf->GetBufferPointer(),
                                                MAKEFOURCC('C', 'T', 'A', 'B'), &data, NULL);
