...
void CKinematics::Load(const char* N, IReader* data, u32 dwFlags) {
    ...
    if (LD) {
        ...
        // From stream
        {
            ...
            //.         strconcat		(sizeof(name_load),name_load, short_name, ":lod:",
            //lod_name.c_str());
            ...
            /*
                        ...
                        m_lod 			= ::Render->model_CreateChild(name_load,LD);
                                    VERIFY			(m_lod->Type==MT_SKELETON_GEOMDEF_PM ||
...
#pragma todo("container is created in stack!")
    ...
    // Msg				("!!! %d bones",dwCount);
    // if (dwCount >= 64)	Msg			("!!! More than 64 bones is a crazy thing! (%d),
    // %s",dwCount,N);
...
void CKinematics::AddWallmark(const Fmatrix* parent_xform, const Fvector3& start,
                              const Fvector3& dir, ref_shader shader, float size) {
    Fvector S, D, normal = { 0, 0, 0 };
    // transform ray from world to model
    Fmatrix P;
    P.invert(*parent_xform);
