...
void CHW::Reset(HWND hwnd) {
    ...
    while (TRUE) {
        ...
        if (SUCCEEDED(_hr))
            break;
        Msg("! ERROR: [%dx%d]: %s", DevPP.BackBufferWidth, DevPP.BackBufferHeight,
            Debug.error2string(_hr));
        Sleep(100);
    }
...
#pragma todo("R2 need to specify depth format")
    ...
    for (int it = 0; it < fDS_Cnt; it++) {
        if (SUCCEEDED(pD3D->CheckDeviceFormat(DevAdapter, DevT, fTarget, D3DUSAGE_DEPTHSTENCIL,
                                              D3DRTYPE_SURFACE, fDS_Try[it]))) {
            if (SUCCEEDED(pD3D->CheckDepthStencilMatch(DevAdapter, DevT, fTarget, fTarget,
                                                       fDS_Try[it]))) {
                return fDS_Try[it];
            }
        }
...
void CHW::CreateDevice(HWND m_hWnd, bool move_window) {
    ...
    Msg("* GPU [vendor:%X]-[device:%X]: %s", adapterID.VendorId, adapterID.DeviceId,
