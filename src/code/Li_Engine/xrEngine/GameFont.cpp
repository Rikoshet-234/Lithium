...
#endif
#ifdef _EDITOR
unsigned short int mbhMulti2Wide(wide_char* WideStr, wide_char* WidePos,
...
void CGameFont::Initialize(LPCSTR cShader, LPCSTR cTextureName) {
    ...
    if (ini->section_exist("mb_symbol_coords")) {
        ...
        //.			d						= ini->r_float("width_correction",
        //"value");
...
u16 CGameFont::SplitByWidth(u16* puBuffer, u16 uBufferSize, float fTargetWidth,
                            const char* pszText) {
    ...
    for (u16 i = 1; i <= len; i++) {
        ...
        if (((fCurWidth + fDelta) > fTargetWidth) && // overlength
            ...
            ((i > 1) && (!IsBadEndCharacter(
                            wsStr[i - 1]))) // && // do not stop the string on a "bad" character
                                            //				( ( i > 1 ) && ( ! ( ( IsAlphaCharacter( wsStr[ i - 1 ] ) ) && (
            //IsAlphaCharacter( wsStr[ i ] ) ) ) ) ) // do not split numbers or words
        ) {
            fCurWidth = fDelta;
