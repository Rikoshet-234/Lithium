...
public:
    ...
    virtual void
    add_Occluder(Fbox2& bb_screenspace) = 0; // mask screen region as oclluded (-1..1, -1..1)
    virtual void
    add_Visual(IRenderVisual* V) = 0; // add visual leaf	(no culling performed at all)
    virtual void add_Geometry(IRenderVisual* V) = 0; // add visual(s)	(all culling performed)
                                                     //	virtual void					add_StaticWallmark		(ref_shader& S, const Fvector& P, float s,
                                                     //CDB::TRI* T, Fvector* V)=0;
    ...
    // 0;  virtual void					add_SkeletonWallmark	(const Fmatrix*
    // xf, CKinematics* obj, ref_shader& sh, const Fvector& start, const Fvector& dir, float size)=0;
    //	Prefer this function when possible
    virtual void add_SkeletonWallmark(const Fmatrix* xf, IKinematics* obj, IWallMarkArray* pArray,
                                      const Fvector& start, const Fvector& dir, float size) = 0;

    // virtual IBlender*				blender_create			(CLASS_ID cls)								=
    // 0;  virtual void					blender_destroy
    // (IBlender* &)								= 0;
    ...
    //0;  virtual IRenderDetailModel*		model_CreateDM			(IReader*
    // F)								= 0;  virtual IRenderVisual*
    // model_Create			(LPCSTR name, IReader*	data=0)				= 0;
