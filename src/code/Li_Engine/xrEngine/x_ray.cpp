...
static const char* month_id[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
int APIENTRY WinMain_impl(HINSTANCE hInstance, HINSTANCE hPrevInstance, char* lpCmdLine,
                          int nCmdShow) {
    Debug._initialize();
    ...
    if (!IsDebuggerPresent()) {
        ...
        typedef BOOL(__stdcall * HeapSetInformation_type)(HANDLE, HEAP_INFORMATION_CLASS, PVOID,
                                                          SIZE_T);
        ...
        if (heap_set_information) {
            ...
            BOOL const result =
#endif // #ifdef DEBUG
                heap_set_information(GetProcessHeap(), HeapCompatibilityInformation, &HeapFragValue,
...
#define STALKER_PRESENCE_MUTEX "Local\\STALKER-COP"
    ...
    // Title window
    logoWindow =
        CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_STARTUP), nullptr, logDlgProc);
...

    {
        ...
        if (/*xr_strlen(g_sLaunchOnExit_params) && */ xr_strlen(g_sLaunchOnExit_app)) {
            ...
            char const* temp_wf =
                (xr_strlen(g_sLaunchWorkingFolder) > 0) ? g_sLaunchWorkingFolder : NULL;
...
void CApplication::LoadTitleIntOld(LPCSTR str)
{
...
void InitLauncher() {
    ...
    R_ASSERT2(
        hLauncher,
        "xrLauncher DLL raised exception during loading or there is no xrLauncher.dll at all");
