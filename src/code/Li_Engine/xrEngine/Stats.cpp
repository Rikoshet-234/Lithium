...
void CStats::Show() {
    ...
    if (vtune.enabled()) {
        ...
        pFont->OutSet(Device.dwWidth / 2.0f + (pFont->SizeOf_("--= tune =--") / 2.0f),
...
// F.OutNext	("VERT:        %d/%d",
// RCache.stat.verts,RCache.stat.calls?RCache.stat.verts/RCache.stat.calls:0);  F.OutNext	("POLY:
// %d/%d",		RCache.stat.polys,RCache.stat.calls?RCache.stat.polys/RCache.stat.calls:0);
...
// RCache.stat.target_rt,RCache.stat.ps,RCache.stat.vs);  F.OutNext	("DCL/VB/IB:   %d/%d/%d",
...
#define PPP(a) (100.f * float(a) / float(EngineTOTAL.result))
        ...
        F.OutNext("uParticles:  Qstart[%d] Qactive[%d] Qdestroy[%d]", Particles_starting,
                  Particles_active, Particles_destroy);
...
#define PPP(a) (100.f * float(a) / float(RenderTOTAL.result))
        ...
        F.OutNext("  Wallmarks: %2.2fms, %d/%d - %d", RenderDUMP_WM.result, RenderDUMP_WMS_Count,
                  RenderDUMP_WMD_Count, RenderDUMP_WMT_Count);
        ...
        F.OutNext("  TGT/SIM/E: %d/%d/%d", snd_stat._rendered, snd_stat._simulated,
                  snd_stat._events);
        ...
        RCache.stat.r.s_static.dips ); F.OutNext	("flora:         %3.1f/%d",
        ...
        RCache.stat.r.s_flora_lods.dips ); F.OutNext	("dynamic:       %3.1f/%d",
        RCache.stat.r.s_dynamic.verts/1024.f,		RCache.stat.r.s_dynamic.dips ); F.OutNext
        ("  dynamic_sw:  %3.1f/%d",	RCache.stat.r.s_dynamic_sw.verts/1024.f,
        RCache.stat.r.s_dynamic_sw.dips ); F.OutNext	("  dynamic_inst:%3.1f/%d",
        RCache.stat.r.s_dynamic_inst.verts/1024.f,	RCache.stat.r.s_dynamic_inst.dips );
        ...
        RCache.stat.r.s_dynamic_2B.verts/1024.f,	RCache.stat.r.s_dynamic_2B.dips ); F.OutNext
        ("details:       %3.1f/%d",	RCache.stat.r.s_details.verts/1024.f,
...
#else
        for (u32 it = (u32)std::max(int(0), (int)errors.size() - g_ErrorLineCount);
             it < errors.size(); it++)
            F.OutNext("%s", errors[it].c_str());
#endif
        F.OnRender();
        ...
        for (; _I != _E; _I++) {
            ...
            if (item._3D) {
                ...
                if (g_stats_flags.is(st_sound_min_dist))
                    DU->DrawSphere(Fidentity, item.params.position, item.params.min_distance,
                                   0x400000FF, 0xFF0000FF, true, true);
