...
void manager::fill(editor::property_holder* holder) {
    ...
    holder->add_property("altitude", "thunderbolts",
                         "this option is resposible for thunderbolts altitude (in degrees)",
                         rad2deg(m_environment.p_var_alt), float_getter, float_setter, -360.0f,
                         360.f);
    ...
    holder->add_property("minimum distance factor", "thunderbolts",
                         "this option is resposible for thunderbolts minimum distance factor "
                         "(distance from far plane)",
    ...
    holder->add_property("tilt", "thunderbolts",
    ...
    holder->add_property("thunderbolt collections", "thunderbolts",
                         ...
                         m_collections_collection);
    holder->add_property("thunderbolts", "thunderbolts",
                         "this option is resposible for thunderbolts", m_thunderbolt_collection);
}
...
shared_str manager::unique_thunderbolt_id(shared_str const& id) const {
    if (m_thunderbolt_collection->unique_id(id.c_str()))
        return (id);
	shared_str outString = m_thunderbolt_collection->generate_unique_id(id.c_str()).c_str();
    return outString;
}
...
shared_str manager::unique_collection_id(shared_str const& id) const {
    if (m_collections_collection->unique_id(id.c_str()))
        return (id);
	shared_str outString = m_collections_collection->generate_unique_id(id.c_str()).c_str();
    return outString;
}
