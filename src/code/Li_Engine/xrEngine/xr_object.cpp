...
void CObject::MakeMeCrow() {
    ...
    if ((u32)_InterlockedCompareExchange((long*)&dwFrame_AsCrow, device_frame_id,
                                         object_frame_id) == device_frame_id)
        return;
...
void CObject::processing_activate() {
    VERIFY3(255 != Props.bActiveCounter,
            "Invalid sequence of processing enable/disable calls: overflow", *cName());
...
void CObject::processing_deactivate() {
    VERIFY3(0 != Props.bActiveCounter,
            "Invalid sequence of processing enable/disable calls: underflow", *cName());
    Props.bActiveCounter--;
...
// VERIFY2(renderable.visual,*cName()); return renderable.visual->vis.sphere.R;
// }
float CObject::Radius() const {
    VERIFY2(renderable.visual, *cName());
...
void CObject::spatial_update(float eps_P, float eps_R) {
    ...
    if (bUpdate) {
        ...
        if (spatial.node_ptr) { // Object registered!
...
// consistency check
    ...
    if (Parent && spatial.node_ptr)
        Debug.fatal(DEBUG_INFO, "Object %s has parent but is still registered inside spatial DB",
                    *cName());

    if ((0 == collidable.model) && (spatial.type & STYPE_COLLIDEABLE))
        Debug.fatal(DEBUG_INFO, "Object %s registered as 'collidable' but has no collidable model",
                    *cName());
#endif
...
void CObject::shedule_Update(u32 T) {
    // consistency check
    // Msg						("-SUB-:[%x][%s]
    // CObject::shedule_Update",dynamic_cast<void*>(this),*cName());
    ...
    /*
    if (AlwaysTheCrow())
    MakeMeCrow	(); else if (Device.vCameraPosition.distance_to_sqr(Position()) <
