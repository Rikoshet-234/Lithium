...
//------------------------------------------------------------------------------
void CLensFlareDescriptor::SetSource(float fRadius, BOOL ign_color, LPCSTR tex_name,
                                     LPCSTR sh_name) {
...
void CLensFlareDescriptor::SetGradient(float fMaxRadius, float fOpacity, LPCSTR tex_name,
                                       LPCSTR sh_name) {
...
void CLensFlareDescriptor::AddFlare(float fRadius, float fOpacity, float fPosition, LPCSTR tex_name,
                                    LPCSTR sh_name) {
    SFlare F;
    F.fRadius = fRadius;
    F.fOpacity = fOpacity;
...
struct STranspParam {
    ...
    float vis;
    float vis_threshold;
    STranspParam(collide::ray_cache* p, const Fvector& _P, const Fvector& _D, float _f,
...
IC BOOL material_callback(collide::rq_result& result, LPVOID params) {
    ...
    if (result.O) {
        ...
        if (K && (result.element > 0))
            vis =
                g_pGamePersistent->MtlTransparent(K->LL_GetData(u16(result.element)).game_mtl_idx);
    } else {
...
void CLensFlare::OnFrame(shared_str id) {
    ...
    CLensFlareDescriptor* desc =
        id.size() ? g_pGamePersistent->Environment().add_flare(m_Palette, id) : 0;
    ...
    case lfsShow:
        m_StateBlend =
            m_Current ? (m_StateBlend + m_Current->m_StateBlendUpSpeed * Device.fTimeDelta * tf)
                      : 1.f + EPS;
        if (m_StateBlend >= 1.f)
            m_State = lfsIdle;
        break;
    case lfsHide:
        m_StateBlend =
            m_Current ? (m_StateBlend - m_Current->m_StateBlendDnSpeed * Device.fTimeDelta * tf)
                      : 0.f - EPS;
        if (m_StateBlend <= 0.f) {
            m_State = lfsShow;
            m_Current = desc;
    ...
    for (int i = 0; i < MAX_RAYS; ++i) {
        ...
        if (m_ray_cache[i].result && m_ray_cache[i].similar(TP.P, TP.D, TP.f)) {
            ...
            if (CDB::TestRayTri(TP.P, TP.D, m_ray_cache[i].verts, _u, _v, _range, false) &&
                ...
                r_dest.r_clear();
                if (g_pGameLevel->ObjectSpace.RayQuery(r_dest, RD, material_callback, &TP, NULL,
                                                       o_main))
...
if (m_ray_cache.result&&m_ray_cache.similar(TP.P,TP.D,TP.f)){
        ...
        float _u,_v,_range;
        if (CDB::TestRayTri(TP.P,TP.D,m_ray_cache.verts,_u,_v,_range,false)&&(_range>0 &&
...
void CLensFlare::Render(BOOL bSun, BOOL bFlares, BOOL bGradient) {
    ...
    if (m_Current->m_Flags.is(CLensFlareDescriptor::flSource)){
        ...
        u32 c				= color.get();
        pv->set				(vecLight.x+vecSx.x-vecSy.x,
    vecLight.y+vecSx.y-vecSy.y, vecLight.z+vecSx.z-vecSy.z, c, 0, 0); pv++; pv->set
    (vecLight.x+vecSx.x+vecSy.x, vecLight.y+vecSx.y+vecSy.y, vecLight.z+vecSx.z+vecSy.z, c, 0, 1);
    pv++;
        pv->set				(vecLight.x-vecSx.x-vecSy.x,
    vecLight.y-vecSx.y-vecSy.y, vecLight.z-vecSx.z-vecSy.z, c, 1, 0); pv++; pv->set
    (vecLight.x-vecSx.x+vecSy.x, vecLight.y-vecSx.y+vecSy.y, vecLight.z-vecSx.z+vecSy.z, c, 1, 1);
    pv++; _2render.push_back	(m_Current->m_Source.hShader);
    }
    }
    if (fBlend>=EPS_L)
    {
    ...
    it!=m_Current->m_Flares.end(); it++){ CLensFlareDescriptor::SFlare&	F = *it; vec.mul
    (vecAxis, F.fPosition); vec.add				(vecCenter); vecSx.mul
    (vecDx, F.fRadius*fDistance); vecSy.mul			(vecDy, F.fRadius*fDistance); float
    cl			= F.fOpacity * fBlend * m_StateBlend; color.set			( dwLight );
                color.mul_rgba		( cl );
                u32 c				= color.get();
                pv->set				(vec.x+vecSx.x-vecSy.x,
    vec.y+vecSx.y-vecSy.y, vec.z+vecSx.z-vecSy.z, c, 0, 0); pv++; pv->set
    (vec.x+vecSx.x+vecSy.x, vec.y+vecSx.y+vecSy.y, vec.z+vecSx.z+vecSy.z, c, 0, 1); pv++; pv->set
    (vec.x-vecSx.x-vecSy.x, vec.y-vecSx.y-vecSy.y, vec.z-vecSx.z-vecSy.z, c, 1, 0); pv++; pv->set
    (vec.x-vecSx.x+vecSy.x, vec.y-vecSx.y+vecSy.y, vec.z-vecSx.z+vecSy.z, c, 1, 1); pv++;
        ...
        if (m_Current->m_Flags.is(CLensFlareDescriptor::flGradient)){
            vecSx.mul			(vecX,
    m_Current->m_Gradient.fRadius*fGradientValue*fDistance); vecSy.mul			(vecY,
    m_Current->m_Gradient.fRadius*fGradientValue*fDistance);
    ...
    (vecLight.x-vecSx.x+vecSy.x, vecLight.y-vecSx.y+vecSy.y, vecLight.z-vecSx.z+vecSy.z, c, 1, 1);
    pv++; _2render.push_back	(m_Current->m_Gradient.hShader);
        }
            }
    }
