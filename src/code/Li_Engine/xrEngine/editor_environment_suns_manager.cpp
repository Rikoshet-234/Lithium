...
template <>
void property_collection<manager::container_type, manager>::display_name(u32 const& item_index,
                                                                         LPSTR const& buffer,
...
void manager::fill(editor::property_holder* holder) {
    VERIFY(holder);
    holder->add_property("suns", "suns", "this option is resposible for sound channels",
                         m_collection);
}
...
shared_str manager::unique_id(shared_str const& id) const {
    if (m_collection->unique_id(id.c_str()))
        return (id);
	shared_str outString = m_collection->generate_unique_id(id.c_str()).c_str();
    return outString;
}
...
CLensFlareDescriptor* manager::get_flare(shared_str const& id) const {
    //	container_type::const_iterator	found = std::find_if(m_suns.begin(), m_suns.end(),
    //predicate(id)); 	VERIFY							(found !=
    //m_suns.end()); 	return							((*found)->);
    return (0);
}
