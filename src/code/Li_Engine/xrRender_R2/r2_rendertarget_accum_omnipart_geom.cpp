#include "stdafx.h"
#include "../xrRenderCommon/du_sphere_part.h"
...
void CRenderTarget::accum_omnip_geom_create() {
    ...
    // vertices
    {
        ...
        std::memcpy(pData, du_sphere_part_vertices, vCount * vSize);
    ...
    // Indices
    {
        ...
        BYTE* pData = 0;
        R_CHK(HW.pDevice->CreateIndexBuffer(iCount * 2, dwUsage, D3DFMT_INDEX16, D3DPOOL_MANAGED,
                                            &g_accum_omnip_ib, 0));
        HW.stats_manager.increment_stats_ib(g_accum_omnip_ib);
        R_CHK(g_accum_omnip_ib->Lock(0, 0, (void**)&pData, 0));
        std::memcpy(pData, du_sphere_part_faces, iCount * 2);
