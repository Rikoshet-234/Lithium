...
{  
    ...
    Msg("%s", header);
    for (size_t i = 1; i < stackTrace.count; ++i) Msg("%s", stackTrace[i]);
}
...
#if 0 //def USE_OWN_ERROR_MESSAGE_WINDOW
		//black magic
        //buffer += xr_sprintf(buffer, assertion_info_size - size_t(buffer - buffer_base), "stack trace:%s%s", endline, endline);
...
#ifdef USE_OWN_ERROR_MESSAGE_WINDOW
    const auto result = MessageBox(0, assertion_info, "Fatal Error", MB_CANCELTRYCONTINUE | MB_ICONERROR | MB_SYSTEMMODAL);
...
void _terminate() {
    ...
    MessageBox(GetTopWindow(nullptr), assertion_info, "Fatal Error", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
...
static void invalid_parameter_handler(const wchar_t* expression, const wchar_t* function,
    ...
    Debug.backend("error handler is invoked!", expression_, {}, file_, line, function_, ignore_always);
}
