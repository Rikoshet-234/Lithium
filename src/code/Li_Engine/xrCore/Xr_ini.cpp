...
BOOL CInifile::Sect::line_exist(LPCSTR L, LPCSTR* val) {
    auto A = std::lower_bound(Data.begin(), Data.end(), L, item_pred);
                    ...
                    if (bInsideSTR) // multiline str value
                    {
                        while (bInsideSTR) {
                            ...
                            R_ASSERT2(
                                ...
                                make_string("Incorrect inifile format: section[%s], variable[%s]. "
    ...
    for (auto r_it = DATA.begin(); r_it != DATA.end(); ++r_it) {
        ...
        if (bcheck) {
            xr_sprintf(temp, sizeof(temp), "; %d %d %d", (*r_it)->Name._get()->dwCRC,
                       (*r_it)->Name._get()->dwReference, (*r_it)->Name._get()->dwLength);
            writer.w_string(temp);
        }
...
CInifile::Sect& CInifile::r_section(LPCSTR S) const {
    ...
    if (!(I != DATA.cend() && xr_strcmp(*(*I)->Name, section) == 0)) {
        ...
        Debug.fatal(DEBUG_INFO,
                    "Can't open section '%s'. Please attach [*.ini_log] file to your bug report",
                    S);
    }
    return **I;
}
...
BOOL CInifile::r_bool_def(LPCSTR S, LPCSTR L, BOOL def_val) const
{
	if (!line_exist(S, L))
		return def_val;
...
void CInifile::w_color(LPCSTR S, LPCSTR L, u32 V, LPCSTR comment) {
    string128 temp;
    xr_sprintf(temp, sizeof(temp), "%d,%d,%d,%d", color_get_R(V), color_get_G(V), color_get_B(V),
               color_get_A(V));
    w_string(S, L, temp, comment);
}
