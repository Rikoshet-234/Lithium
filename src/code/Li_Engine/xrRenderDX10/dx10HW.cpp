...
void CHW::CreateD3D() {
    ...
    (_CreateDXGIFactory*)GetProcAddress(hD3D,"CreateDXGIFactory"); R_ASSERT(CreateFactory);
    ...
    */
    ...
    DirectX before running this program"); typedef IDirect3D9 * WINAPI _Direct3DCreate9(UINT
    SDKVersion); _Direct3DCreate9* createD3D	=
...
void CHW::CreateDevice(HWND m_hWnd, bool move_window) {
    ...
    */
    ...
    Msg		("* GPU driver: %d.%d.%d.%d",u32(drv_Product),u32(drv_Version),u32(drv_SubVersion),
    u32(drv_Build));
    */
    ...
    */
    ...
    the game."); FlushLog			(); MessageBox			(NULL,"Failed to
    ...
    */
    ...
    // else					P.FullScreen_RefreshRateInHz	=
...
// u32 GPU		= selectGPU();
    ...
    R = D3D11CreateDeviceAndSwapChain(
        ...
        m_DriverType, NULL, createDeviceFlags, pFeatureLevels,
        sizeof(pFeatureLevels) / sizeof(pFeatureLevels[0]), D3D11_SDK_VERSION, &sd, &m_pSwapChain,
        &pDevice, &FeatureLevel, &pContext);
#else
    R = D3DX10CreateDeviceAndSwapChain(m_pAdapter, m_DriverType, NULL, createDeviceFlags, &sd,
                                       &m_pSwapChain, &pDevice);
...
//	_RELEASE			(dwDebugSB);
    ...
    = bWindowed; DevPP.PresentationInterval	= D3DPRESENT_INTERVAL_IMMEDIATE; if( !bWindowed )
    ...
    DevPP.FullScreen_RefreshRateInHz	= D3DPRESENT_RATE_DEFAULT; #endif
            ...
            while	(TRUE)	{
                    HRESULT _hr							= HW.pDevice->Reset
    (&DevPP); if (SUCCEEDED(_hr))					break;
                    Msg		("! ERROR: [%dx%d]:
    %s",DevPP.BackBufferWidth,DevPP.BackBufferHeight,Debug.error2string(_hr)); Sleep	(100);
            }
...
#ifdef DEBUG
R_CHK				(pDevice->CreateStateBlock
(D3DSBT_ALL,&dwDebugSB)); #endif #ifndef _EDITOR updateWindowProps	(hwnd); #endif
*/
}
...
u32 CHW::selectGPU ()
{
        ...
        if(caps.DevCaps&D3DDEVCAPS_HWTRANSFORMANDLIGHT)
        {
                if (Caps.bForceGPU_NonPure)	return D3DCREATE_HARDWARE_VERTEXPROCESSING;
                        ...
                        if (caps.DevCaps&D3DDEVCAPS_PUREDEVICE) return
    ...
    if (psDeviceFlags.is(rsRefresh60hz)) {
        ...
        for (u32 i = 0; i < num; ++i) {
            ...
            if ((desc.Width == dwWidth) && (desc.Height == dwHeight)) {
                ...
                float TempFreq =
                    float(desc.RefreshRate.Numerator) / float(desc.RefreshRate.Denominator);
...
BOOL CHW::support(D3DFORMAT fmt, DWORD type, DWORD usage) {
    ...
    /*
    HRESULT hr		=
    pD3D->CheckDeviceFormat(DevAdapter,DevT,Caps.fTarget,usage,(D3DRESOURCETYPE)type,fmt); if
    (FAILED(hr))	return FALSE; else			return TRUE;
    */
    return TRUE;
}
        ...
        if (m_move_window) {
            if (strstr(Core.Params, "-no_dialog_header"))
                ...
                SetWindowLong(m_hWnd, GWL_STYLE,
                              dwWindowStyle = (WS_BORDER | WS_DLGFRAME | WS_VISIBLE | WS_SYSMENU |
                                               WS_MINIMIZEBOX));
            ...
            if (bCenter) {
                ...
                SetRect(&m_rcWindowBounds, 0, 0, m_ChainDesc.BufferDesc.Width,
...
void fill_vid_mode_list(CHW* _hw) {
    ...
    for(i=0; i<cnt;++i)
    {
            ...
            xr_sprintf						(str,sizeof(str),"%dx%d", Mode.Width,
Mode.Height);
