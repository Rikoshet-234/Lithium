...
#include "../xrRenderCommon/r_constants.h"
...
BOOL R_constant_table::parseConstants(ID3DShaderReflectionConstantBuffer* pTable, u32 destination) {
    ...
    // (LPBYTE(desc)+desc->ConstantInfo);  LPBYTE					 ptr	=
    // LPBYTE(desc);  for (u32 dwCount = desc->Constants; dwCount; dwCount--,it++)
    for (u32 i = 0; i < TableDesc.Variables; ++i) {
        ...
        case D3D10_SVC_OBJECT: {
            ...
            if (!C)	{
            C					=
            xr_new<R_constant>();//.g_constant_allocator.create();
            C->name				=	name;
            C->destination		=	RC_dest_sampler;
            C->type				=	RC_sampler;
            R_constant_load& L	=	C->samp;
            L.index				=	u16(r_index	+ ( (destination&1)? 0 :
            D3DVERTEXTEXTURESAMPLER0 )); L.cls				=	RC_sampler	;
            table.push_back		(C);
            } else {
...
BOOL R_constant_table::parseResources(ID3DShaderReflection* pReflection, int ResNum,
                                      u32 destination) {
    ...
    if (ShaderDesc.ConstantBuffers) {
        ...
        for (u16 iBuf = 0; iBuf < ShaderDesc.ConstantBuffers; ++iBuf) {
            ...
            if (pTable) {
                ...
                updatedDest |= iBuf
                               << dest_to_shift_value(destination); /*((destination&RC_dest_pixel)
                ...
                ref_cbuffer tempBuffer =
                    dxRenderDeviceRender::Instance().Resources->_CreateConstantBuffer(pTable);
