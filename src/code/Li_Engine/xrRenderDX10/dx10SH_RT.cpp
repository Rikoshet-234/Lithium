...
#endif
{
    ...
    //&pSurface,NULL);  if (FAILED(_hr) || (0==pSurface))	return;
    ...
    if (SampleCount <= 1)
        ...
        desc.BindFlags = (bUseAsDepth ? D3D_BIND_DEPTH_STENCIL
...
void CRTC::create	(LPCSTR Name, u32 size,	D3DFORMAT f)
{
        ...
        // Try to create texture/surface
        DEV->Evict					();
        _hr = HW.pDevice->CreateCubeTexture	(size, 1, D3DUSAGE_RENDERTARGET, f,
D3DPOOL_DEFAULT, &pSurface,NULL); if (FAILED(_hr) || (0==pSurface))	return;
