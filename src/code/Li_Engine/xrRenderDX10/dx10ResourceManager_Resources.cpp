...
SVS* CResourceManager::_CreateVS(LPCSTR _name) {
    ...
    if (I != m_vs.end())
        ...
        HRESULT const _hr =
            ::Render->shader_compile(name, (DWORD const*)data, size, c_entry, c_target,
...
SGS* CResourceManager::_CreateGS(LPCSTR name) {
    ...
    if (I != m_gs.end())
        ...
        HRESULT const _hr =
            ::Render->shader_compile(name, (DWORD const*)file->pointer(), file->length(), c_entry,
...
SGeometry* CResourceManager::CreateGeom(D3DVERTEXELEMENT9* decl, ID3DVertexBuffer* vb,
                                        ID3DIndexBuffer* ib) {
    R_ASSERT(decl && vb);
...
//--------------------------------------------------------------------------------------------------------------
dx10ConstantBuffer*
CResourceManager::_CreateConstantBuffer(ID3DShaderReflectionConstantBuffer* pTable) {
    VERIFY(pTable);
...
SInputSignature* CResourceManager::_CreateInputSignature(ID3DBlob* pBlob) {
    ...
    for (u32 it = 0; it < v_input_signature.size(); it++) {
        ...
        if ((pBlob->GetBufferSize() == sign->signature->GetBufferSize()) &&
            (!(memcmp(pBlob->GetBufferPointer(), sign->signature->GetBufferPointer(),
                      pBlob->GetBufferSize())))) {
            return sign;
        }
    }
