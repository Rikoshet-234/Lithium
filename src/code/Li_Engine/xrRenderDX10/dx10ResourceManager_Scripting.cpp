...
#include "../xrRenderCommon/tss.h"
#include "../xrRenderCommon/blenders/blender.h"
#include "../xrRenderCommon/blenders/blender_recorder.h"
...
public:
    ...
    //return *this;	} 	adopt_sampler&			_projective		(bool _b)
    ...
    //{ if (C) C->i_Filter	(stage,D3DTEXF_ANISOTROPIC,D3DTEXF_LINEAR,D3DTEXF_ANISOTROPIC);	return
...
public:
        adopt_dx10texture	(CBlender_Compile*	C, u32 TextureIndex)	: m_pC(C),
m_TI(TextureIndex)		{ if (u32(-1)==m_TI) m_pC=0;} adopt_dx10texture	(const
adopt_dx10texture&	_C)				: m_pC(_C.m_pC), m_TI(_C.m_TI)	{
...
public:
    ...
    adopt_compiler& _dx10Stencil(bool Enable, u32 Func, u32 Mask, u32 WriteMask, u32 Fail, u32 Pass,
                                 u32 ZFail) {
...
void CResourceManager::LS_Load() {
    ...
    module(
        LSVM)[class_<adopt_dx10options>("_dx10options")
                  .def("dx10_msaa_alphatest_atoc", &adopt_dx10options::_dx10_msaa_alphatest_atoc)
              //.def("",					&adopt_dx10options::_dx10Options
              //),	// returns options-object
              ,

              class_<adopt_dx10sampler>("_dx10sampler")
              //.def("texture",
              //&adopt_sampler::_texture		,return_reference_to(_1)) .def("project",
              //&adopt_sampler::_projective		,return_reference_to(_1)) .def("clamp",
              //&adopt_sampler::_clamp			,return_reference_to(_1)) .def("wrap",
              //&adopt_sampler::_wrap			,return_reference_to(_1)) .def("mirror",
              //&adopt_sampler::_mirror			,return_reference_to(_1))
              //.def("f_anisotropic",				&adopt_sampler::_f_anisotropic
              //,return_reference_to(_1)) .def("f_trilinear",
              //&adopt_sampler::_f_trilinear	,return_reference_to(_1)) .def("f_bilinear",
              //&adopt_sampler::_f_bilinear		,return_reference_to(_1)) .def("f_linear",
              //&adopt_sampler::_f_linear		,return_reference_to(_1)) .def("f_none",
              //&adopt_sampler::_f_none			,return_reference_to(_1)) .def("fmin_none",
              //&adopt_sampler::_fmin_none		,return_reference_to(_1)) .def("fmin_point",
              //&adopt_sampler::_fmin_point		,return_reference_to(_1))
              //.def("fmin_linear",
              //&adopt_sampler::_fmin_linear	,return_reference_to(_1)) .def("fmin_aniso",
              //&adopt_sampler::_fmin_aniso		,return_reference_to(_1)) .def("fmip_none",
              //&adopt_sampler::_fmip_none		,return_reference_to(_1)) .def("fmip_point",
              //&adopt_sampler::_fmip_point		,return_reference_to(_1))
              //.def("fmip_linear",
              //&adopt_sampler::_fmip_linear	,return_reference_to(_1)) .def("fmag_none",
              //&adopt_sampler::_fmag_none		,return_reference_to(_1)) .def("fmag_point",
              //&adopt_sampler::_fmag_point		,return_reference_to(_1))
              //.def("fmag_linear",
              //&adopt_sampler::_fmag_linear	,return_reference_to(_1))
              ,
              ...
              class_<adopt_compiler>("_compiler")
                  ...
                  .def("dx10color_write_enable", &adopt_compiler::_dx10color_write_enable,
                       return_reference_to<1>())
                  .def("color_write_enable", &adopt_compiler::_dx10color_write_enable,
                       return_reference_to<1>())
              ...
              class_<adopt_cmp_func>("cmp_func")
                  .enum_("cmp_func")
                      [value("never", int(D3DCMP_NEVER)), value("less", int(D3DCMP_LESS)),
...
Shader* CResourceManager::_lua_Create(LPCSTR d_shader, LPCSTR s_textures) {
    ...
    if (Script::bfIsObjectPresent(LSVM, s_shader, "normal_hq", LUA_TFUNCTION)) {
        ...
        C.bDetail =
            dxRenderDeviceRender::Instance().Resources->m_textures_description.GetDetailTexture(
