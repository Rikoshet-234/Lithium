...
static void draw_obb		( const Fmatrix &matrix, const u32 &color )
{
        ...
        rdebug_render->add_lines						(aabb,
sizeof(aabb)/sizeof(Fvector), &aabb_id[0], sizeof(aabb_id)/(2*sizeof(u16)), color);
}
*/
...
dx103DFluidObstacles::dx103DFluidObstacles(int gridWidth, int gridHeight, int gridDepth,
...
void dx103DFluidObstacles::ProcessObstacles(const dx103DFluidData& FluidData, float timestep) {
    ...
    Fmatrix WorldToFluid;
    {
        ...
        // (float)m_iTextureDepth-1);  Translate.translate(0.5, 0.5, 0.5);  It seems that y axis is
        // inverted in fluid simulation, so shange maths a bit
...
void dx103DFluidObstacles::ProcessStaticObstacles(const dx103DFluidData& FluidData,
...
void dx103DFluidObstacles::ProcessDynamicObstacles(const dx103DFluidData& FluidData,
    ...
    for (u32 i = 0; i < m_lstRenderables.size(); ++i) {
        ...
        // disassociated from S/P structure  if	(PortalTraverser.i_marker != sector->r_marker)
        // continue;	// inactive (untouched) sector

        // renderable
        // IRenderable*	renderable		= spatial->dcast_Renderable	();
        // if (0==renderable)				continue;					// unknown, but
        // renderable object (r1_glow???)
...
void dx103DFluidObstacles::RenderPhysicsElement(const IPhysicsElement& Element,
    ...
    TranslationVelocity.set(TranslationVelocity3.x, TranslationVelocity3.y, TranslationVelocity3.z,
                            0.0f);
...
/*
void dx103DFluidObstacles::RenderDynamicOOBB( const IPhysicsElement &Element, const Fmatrix
