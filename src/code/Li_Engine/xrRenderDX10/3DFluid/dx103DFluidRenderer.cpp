...
LPCSTR dx103DFluidRenderer::m_pRTNames[RRT_NumRT] = { "$user$rayDataTex", "$user$rayDataTexSmall",
                                                      "$user$rayCastTex", "$user$edgeTex" };

LPCSTR dx103DFluidRenderer::m_pResourceRTNames[RRT_NumRT] = { "rayDataTex", "rayDataTexSmall",
                                                              "rayCastTex", "edgeTex" };
...
void dx103DFluidRenderer::Initialize(int gridWidth, int gridHeight, int gridDepth) {
    ...
    // Initialize the grid offset matrix
    {
        ...
        D3DXMatrixScaling(&scaleM, m_vGridDim[0] / m_fMaxDim, m_vGridDim[1] / m_fMaxDim,
        ...
        // m_gridMatrix.scale(m_vGridDim[0] / m_fMaxDim, m_vGridDim[1] / m_fMaxDim, m_vGridDim[2] /
        // m_fMaxDim);  m_gridMatrix.translate_over(-0.5, -0.5, -0.5);
    }
...
void dx103DFluidRenderer::CreateGridBox() {
    VsInput vertices[] = {
        { D3DXVECTOR3(0, 0, 0) }, { D3DXVECTOR3(0, 0, 1) }, { D3DXVECTOR3(0, 1, 0) },
        { D3DXVECTOR3(0, 1, 1) }, { D3DXVECTOR3(1, 0, 0) }, { D3DXVECTOR3(1, 0, 1) },
    ...
    CHK_DX(
        dx10BufferUtils::CreateVertexBuffer(&m_pGridBoxVertexBuffer, vertices, sizeof(vertices)));
    ...
    HW.stats_manager.increment_stats(sizeof(indices), enum_stats_buffer_type_index,
                                     D3DPOOL_MANAGED);
...
void dx103DFluidRenderer::CreateScreenQuad() {
    ...
    // V_RETURN( m_pD3DDevice->CreateInputLayout( quadlayout, numElements,
    // PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &pQuadLayout ) );
...
// desc.Usage = D3D_USAGE_IMMUTABLE;
    ...
    // pEffect->GetVariableByName("jitterTex")->AsShaderResource() -> SetResource
    // (JitterTextureSRV);

    m_JitterTexture =
        dxRenderDeviceRender::Instance().Resources->_CreateTexture("$user$NVjitterTex");
...
void dx103DFluidRenderer::CreateRayDataResources(int width, int height) {
    ...
    /*
            ...
            V_RETURN( m_pD3DDevice->CreateRenderTargetView(pRayDataSmallTex2D, &DescRT,
       &pRayDataSmallRTV)); V_RETURN( m_pD3DDevice->CreateRenderTargetView(pRayCastTex2D, &DescRT,
       &pRayCastRTV)); DescRT.Format = DXGI_FORMAT_R32_FLOAT; V_RETURN(
       m_pD3DDevice->CreateRenderTargetView(pEdgeTex2D, &DescRT, &pEdgeRTV));
            ...
            V_RETURN(m_pD3DDevice->CreateShaderResourceView(pRayDataSmallTex2D, &SRVDesc,
       &pRayDataSmallSRV)); V_RETURN(m_pD3DDevice->CreateShaderResourceView(pRayCastTex2D, &SRVDesc,
       &pRayCastSRV)); SRVDesc.Format = DXGI_FORMAT_R32_FLOAT;
...
void dx103DFluidRenderer::Draw(const dx103DFluidData& FluidData) {
    ...
    // The length of one of the axis of the worldView matrix is the length of longest side of the
    // box
    ...
    // We prepend the current world matrix with this other matrix which adds an offset (-0.5, -0.5,
    // -0.5)
    //  and scale factors to account for unequal number of voxels on different sides of the volume
    //  box.
    ...
    // WorldViewProjection.transpose();
    D3DXMatrixInverse((D3DXMATRIX*)&InvWorldViewProjection, NULL,
                      (D3DXMATRIX*)&WorldViewProjection);
    // WorldViewProjection.transpose();
...
void dx103DFluidRenderer::ComputeEdgeTexture() {
    ...
    // Downsample the rayDataTexture to a new small texture, simply using point sample (no
    // filtering)
