...
LPCSTR dx103DFluidManager::m_pShaderTextureNames[NUM_RENDER_TARGETS] = {
    ...
    "Texture_velocity0",    //	RENDER_TARGET_VELOCITY0 = NUM_OWN_RENDER_TARGETS,	//	For
...
dx103DFluidManager::dx103DFluidManager()
    : m_bInited(false),
      ...
      m_fSaturation(0.78f), m_bAddDensity(true), m_fImpulseSize(0.15f), m_fConfinementScale(0.0f),
      m_fDecay(1.0f), m_pGrid(0), m_pRenderer(0), m_pObstaclesHandler(0) {
...
void dx103DFluidManager::Initialize(int width, int height, int depth) {
    ...
    m_pObstaclesHandler =
        xr_new<dx103DFluidObstacles>(m_iTextureWidth, m_iTextureHeight, m_iTextureDepth, m_pGrid);

    m_pEmittersHandler =
        xr_new<dx103DFluidEmitters>(m_iTextureWidth, m_iTextureHeight, m_iTextureDepth, m_pGrid);
...
void dx103DFluidManager::PrepareTexture(int rtIndex) {
    pRTTextures[rtIndex] =
        dxRenderDeviceRender::Instance().Resources->_CreateTexture(m_pEngineTextureNames[rtIndex]);
}
...
void dx103DFluidManager::Update(dx103DFluidData& FluidData, float timestep) {
    ...
    // Update the obstacle velocity based on its movement
    {
            ...
            obstVelocity.x *= grid->dim[0]; obstVelocity.y *= grid->dim[1]; obstVelocity.z *=
    grid->dim[2];
...
void dx103DFluidManager::AdvectColorBFECC(float timestep, bool bTeperature) {
    ...
    // pRenderTargetShaderViews[RENDER_TARGET_TEMPVECTOR] ); 	Overwrite RENDER_TARGET_COLOR0 with
    //RENDER_TARGET_TEMPVECTOR 	Find texture index and patch texture manually using DirecX call!
    ...
    Fvector4 halfVol;
    halfVol.set((float)m_iTextureWidth / 2.0f, (float)m_iTextureHeight / 2.0f,
...
void dx103DFluidManager::ComputePressure(float timestep) {
    ...
    // pRTTextures[RENDER_TARGET_TEMPSCALAR]->surface_get();  ID3DxxTexture3D	*pPressure =
    // (ID3DxxTexture3D*) pRTTextures[RENDER_TARGET_PRESSURE]->surface_get();
