...
void SArtefactActivation::PhDataUpdate(float step) {
    ...
    if (m_cur_activation_state == eFlying) {
        Fvector dir = { 0, -1.f, 0 };
        if (Level().ObjectSpace.RayTest(m_af->Position(), dir, 1.0f, collide::rqtBoth, NULL,
                                        m_af)) {
...
void SArtefactActivation::ChangeEffects() {
    ...
    if (state_def.m_particle.size()) {
        ...
        m_af->CParticlesPlayer::StartParticles(state_def.m_particle, dir, m_af->ID(),
...
void SArtefactActivation::SpawnAnomaly() {
    ...
    m_af->Center(pos);
    CSE_Abstract* object =
        Level().spawn_item(zone_sect, pos, m_af->ai_location().level_vertex_id(), 0xffff, true);
