...
void CGameObject::OnEvent(NET_Packet& P, u16 type) {
    ...
    case GE_HIT_STATISTIC: {
        /*
                                ...
                                SHit	HDS = SHit(power, dir, Hitter, element,
           position_in_bone_space, impulse, (ALife::EHitType)hit_type, ap);
        */
        SHit HDS;
        HDS.PACKET_TYPE = type;
        HDS.Read_Packet_Cont(P);
        //			Msg("Hit received: %d[%d,%d]", HDS.whoID, HDS.weaponID,
        //HDS.BulletID);
    ...
    case GE_DESTROY: {
        if (H_Parent()) {
            Msg("! ERROR (GameObject): GE_DESTROY arrived to object[%d][%s], that has "
                "parent[%d][%s], frame[%d]",
                ID(), cNameSect().c_str(), H_Parent()->ID(), H_Parent()->cName().c_str(),
                Device.dwFrame);
...
BOOL CGameObject::net_Spawn(CSE_Abstract* DC) {
    ...
    setID(E->ID);
    //	if (GameID() != eGameIDSingle)
    //		Msg ("CGameObject::net_Spawn -- object %s[%x] setID [%d]", *(E->s_name), this,
    //E->ID);
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
        ...
        Msg("CGameObject::net_Spawn obj %s Position set from CSE_Abstract %f,%f,%f",
...
#pragma warning(pop)
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
        stricmp(PH_DBG_ObjectTrackName(), *cName()) == 0) {
        Msg("CGameObject::net_Spawn obj %s After Script Binder reinit %f,%f,%f",
            PH_DBG_ObjectTrackName(), Position().x, Position().y, Position().z);
    }
#endif
    // load custom user data from server
    if (!E->client_data.empty()) {
        //		Msg				("client data is present for object [%d][%s], load
        //is processed",ID(),*cName());
        IReader ireader = IReader(&*E->client_data.begin(), E->client_data.size());
        net_Load(ireader);
    } else {
        //		Msg				("no client data for object [%d][%s], load is
        //skipped",ID(),*cName());
    }
    ...
    if (ai().get_level_graph()) {
        if (E->ID_Parent == 0xffff) {
            ...
            if (UsedAI_Locations() &&
                ...
                Position().y =
                    EPS_L + ai().level_graph().vertex_plane_y(*ai_location().level_vertex(),
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
        stricmp(PH_DBG_ObjectTrackName(), *cName()) == 0) {
        Msg("CGameObject::net_Spawn obj %s Before CScriptBinder::net_Spawn %f,%f,%f",
            PH_DBG_ObjectTrackName(), Position().x, Position().y, Position().z);
    }
...
// ----------------------------------------------------------
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
        stricmp(PH_DBG_ObjectTrackName(), *cName()) == 0) {
        Msg("CGameObject::net_Load obj %s (loaded) %f,%f,%f", PH_DBG_ObjectTrackName(),
    ...
    for (u32 k = 0, j; spawn_ini()->r_line("spawn", k, &N, &V); k++) {
        ...
        for (u32 i = 0; i < j; ++i)
            if (::Random.randF(1.f) < p) {
                CSE_Abstract* A =
                    Level().spawn_item(N, Position(), ai_location().level_vertex_id(), ID(), true);
...
void CGameObject::setup_parent_ai_locations(bool assign_position) {
    ...
    if (ai().game_graph().valid_vertex_id(l_tpGameObject->ai_location().game_vertex_id()))
        ...
        ai_location().game_vertex(
            ai().cross_table().vertex(ai_location().level_vertex_id()).game_vertex_id());
    //	VERIFY2
...
void CGameObject::OnH_B_Independent(bool just_before_destroy) {
    ...
    VERIFY(parent);
    if (ai().get_level_graph() &&
        ai().level_graph().valid_vertex_id(parent->ai_location().level_vertex_id()))
        validate_ai_locations(false);
}
...
void CGameObject::add_visual_callback(visual_callback* callback) {
    ...
    CALLBACK_VECTOR_IT I =
        std::find(visual_callbacks().begin(), visual_callbacks().end(), callback);
...
void VisualCallback(IKinematics* tpKinematics) {
    CGameObject* game_object =
        static_cast<CGameObject*>(static_cast<CObject*>(tpKinematics->GetUpdateCallbackParam()));
    VERIFY(game_object);
...
u32 CGameObject::ef_creature_type() const {
    ...
    R_ASSERT3(false, "Invalid creature type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
}
...
u32 CGameObject::ef_equipment_type() const {
    string16 temp;
    CLSID2TEXT(CLS_ID, temp);
    R_ASSERT3(false, "Invalid equipment type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
    //	return		(6);
}
...
u32 CGameObject::ef_main_weapon_type() const {
    ...
    CLSID2TEXT(CLS_ID, temp);
    R_ASSERT3(false,
              "Invalid main weapon type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
    //	return		(5);
}
...
u32 CGameObject::ef_anomaly_type() const {
    string16 temp;
    CLSID2TEXT(CLS_ID, temp);
    R_ASSERT3(false, "Invalid anomaly type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
}
...
u32 CGameObject::ef_weapon_type() const {
    string16 temp;
    CLSID2TEXT(CLS_ID, temp);
    R_ASSERT3(false, "Invalid weapon type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
    //	return		(u32(0));
}
...
u32 CGameObject::ef_detector_type() const {
    string16 temp;
    CLSID2TEXT(CLS_ID, temp);
    R_ASSERT3(false, "Invalid detector type request, virtual function is not properly overridden!",
              temp);
    return (u32(-1));
}
...
void render_box(IRenderVisual* visual, const Fmatrix& xform, const Fvector& additional,
                bool draw_child_boxes, const u32& color) {
