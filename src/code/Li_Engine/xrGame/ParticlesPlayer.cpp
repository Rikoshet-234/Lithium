...
CParticlesPlayer::SBoneInfo::FindParticles(const shared_str& ps_name) {
...
CParticlesPlayer::SBoneInfo* CParticlesPlayer::get_nearest_bone_info(IKinematics* K,
                                                                     u16 bone_index) {
...
void CParticlesPlayer::StartParticles(const shared_str& particles_name, u16 bone_num,
                                      const Fvector& dir, u16 sender_id, int life_time,
                                      bool auto_stop) {
    Fmatrix xform;
...
void CParticlesPlayer::StartParticles(const shared_str& particles_name, u16 bone_num,
                                      const Fmatrix& xform, u16 sender_id, int life_time,
                                      bool auto_stop) {
    ...
    SBoneInfo* pBoneInfo =
        get_nearest_bone_info(smart_cast<IKinematics*>(object->Visual()), bone_num);
    if (!pBoneInfo)
        return;
...
void CParticlesPlayer::StartParticles(const shared_str& ps_name, const Fmatrix& xform,
...
void CParticlesPlayer::StartParticles(const shared_str& ps_name, const Fvector& dir, u16 sender_id,
...
void CParticlesPlayer::GetBonePos(CObject* pObject, u16 bone_id, const Fvector& offset,
                                  Fvector& result) {
    VERIFY(pObject);
...
void CParticlesPlayer::MakeXFORM(CObject* pObject, u16 bone_id, const Fvector& dir,
                                 const Fvector& offset, Fmatrix& result) {
