...
void CMaterialManager::Load(LPCSTR section) {
    ...
    if (debug_character_material_load) {
        ...
        if (entity_alive) {
            ...
            VERIFY(m);
            Msg("(CMaterialManager::Load(LPCSTR section)) material: %s loaded for %s, from "
                "section: %s ",
...
void CMaterialManager::reinit() {
    ...
    if (entity_alive) {
        ...
        entity_alive->character_physics_support()->movement()->SetPLastMaterialIDX(
            &m_last_material_idx);

        //		if (entity_alive->use_simplified_visual()) {
        //			IKinematics			*kinematics =
        //smart_cast<IKinematics*>(entity_alive->Visual());
        //			m_my_material_idx	=
        //kinematics->LL_GetData(kinematics->LL_GetBoneRoot()).game_mtl_idx;
        //		}
...
void CMaterialManager::update(float time_delta, float volume, float step_time, bool standing) {
    ...
    VERIFY3(mtl_pair,
            "Undefined material pair: ", *GMLib.GetMaterialByIdx(m_last_material_idx)->m_Name);
