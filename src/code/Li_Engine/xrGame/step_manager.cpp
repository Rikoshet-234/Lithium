...
void CStepManager::reload(LPCSTR section) {
    ...
    for (u32 i = 0; pSettings->r_line(anim_section, i, &anim_name, &val); ++i) {
        ...
        if (!motion_id) {
            ...
            Msg("! (CStepManager::reload) no anim :%s object:%s, visual: %s, step_params section: "
                "%s ",
                anim_name, m_object->cName().c_str(), m_object->cNameVisual().c_str(),
                anim_section);
        ...
        if (debug_step_info_load) {
            ...
            Msg("step_params loaded for object :%s, visual: %s, motion: %s, anim set: %s  ",
                m_object->cName().c_str(), m_object->cNameVisual().c_str(), anim_name.first,
                anim_name.second);
        }
#endif
    ...
    if (m_steps_map.empty())
        Msg("! no steps info loaded for :%s, section :s, step_params section: %s ",
...
void CStepManager::update(bool b_hud_view) {
    ...
    for (u32 i = 0; i < m_legs_count; i++) {
        ...
        u32 offset_time =
            m_time_anim_started + u32(1000 * (cycle_anim_time * (m_step_info.cur_cycle - 1) +
        ...
        if (offset_time <= cur_time) {
            ...
            if (b_play && !mtl_pair->CollideParticles.empty()) {
                LPCSTR ps_name =
                    *mtl_pair
                         ->CollideParticles[::Random.randI(0, mtl_pair->CollideParticles.size())];
    ...
    if (m_step_info.cur_cycle < step.cycles)
        m_step_info.cur_cycle =
            1 + u8(float(cur_time - m_time_anim_started) / (1000.f * cycle_anim_time));

    // если анимация циклическая...
    u32 time_anim_end =
        m_time_anim_started + u32(get_blend_time() * 1000); // время завершения работы анимации
...
void CStepManager::reload_foot_bones() {
    ...
    if (ini && ini->section_exist("foot_bones")) {
        ...
        load_foot_bones(
            pSettings->r_section(pSettings->r_string(*m_object->cNameSect(), "foot_bones")));
    }
...
void CStepManager::material_sound::play_next(SGameMtlPair* mtl_pair, CEntityAlive* object,
                                             float volume, bool b_hud_mode) {
    ...
    mtl_pair->StepSounds[m_last_step_sound_played].play_no_feedback(object, b_hud_mode ? sm_2D : 0,
                                                                    0, &sound_pos, &vol);
}