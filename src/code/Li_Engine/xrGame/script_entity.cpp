...
void CScriptEntity::SetScriptControl(const bool bScriptControl, shared_str caSciptName) {
    if (!(((m_bScriptControl && !bScriptControl) || (!m_bScriptControl && bScriptControl)) &&
          (bScriptControl ||
           (xr_strlen(*m_caScriptName) && !xr_strcmp(caSciptName, m_caScriptName))))) {
    ...
    control",*caSciptName,*object().cName()); else ai().script_engine().script_log
...
void CScriptEntity::AddAction(const CScriptEntityAction* tpEntityAction, bool bHighPriority) {
    ...
    if (!bHighPriority || m_tpActionQueue.empty())
        ...
        m_tpActionQueue.insert(m_tpActionQueue.begin(),
...
void CScriptEntity::vfUpdateSounds() {
    ...
    if (xr_strlen(l_tSoundAction.m_caBoneName) && m_current_sound && m_current_sound->_feedback())
        m_current_sound->_feedback()->set_position(GetUpdatedMatrix(l_tSoundAction.m_caBoneName,
...
//%s",Device.dwTimeGlobal,*l_tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
        ...
        if (true /*psAI_Flags.is(aiLua)*/) {
            object().callback(GameObject::eActionTypeRemoved)(object().lua_game_object(),
                                                              u32(eActionTypeRemoved));
        }
    ...
    if (m_tpActionQueue.empty()) {
        ...
        if (empty_queue)
            ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeInfo,
                                            "Object %s has an empty script queue!",
                                            *object().cName());
#endif
        return;
    }
    ...
    try {
        ...
        if (l_tpEntityAction->m_tWatchAction.m_bCompleted && !l_bCompleted)
            object().callback(GameObject::eActionTypeWatch)(object().lua_game_object(),
                                                            u32(eActionTypeWatch));
        ...
        if (l_tpEntityAction->m_tSoundAction.m_bCompleted && !l_bCompleted)
            object().callback(GameObject::eActionTypeSound)(object().lua_game_object(),
                                                            u32(eActionTypeSound));
        ...
        if (l_tpEntityAction->m_tParticleAction.m_bCompleted && !l_bCompleted)
            object().callback(GameObject::eActionTypeParticle)(object().lua_game_object(),
                                                               u32(eActionTypeParticle));
        ...
        if (l_tpEntityAction->m_tObjectAction.m_bCompleted && !l_bCompleted)
            object().callback(GameObject::eActionTypeObject)(object().lua_game_object(),
                                                             u32(eActionTypeObject));
        ...
        if (l_tpEntityAction->m_tMovementAction.m_bCompleted && !l_bCompleted)
            object().callback(GameObject::eActionTypeMovement)(object().lua_game_object(),
                                                               u32(eActionTypeMovement), -1);
...
bool CScriptEntity::bfAssignAnimation(CScriptEntityAction* tpEntityAction) {
    ...
    m_tpNextAnimation =
        tVisualObject.ID_Cycle_Safe(*GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay);
    m_use_animation_movement_controller =
        GetCurrentAction()->m_tAnimationAction.m_use_animation_movement_controller;
    return (true);
}
...
#ifdef _DEBUG
//				Msg									("%6d Starting
//sound %s",Device.dwTimeGlobal,*l_tSoundAction.m_caSoundToPlay);
#endif
                ...
                m_current_sound->play_at_pos(m_object, l_tMatrix.c,
...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction* tpEntityAction) {
    ...
    case CScriptMovementAction::eGoalTypePatrolPath: {
        ...
        m_monster->movement().patrol().set_path(l_tMovementAction.m_path,
                                                l_tMovementAction.m_path_name);
    ...
    case CScriptMovementAction::eGoalTypePathPosition: {
        ...
        if (!ai().level_graph().valid_vertex_id(vertex_id)) {
            string256 S;
            xr_sprintf(S,
                       "Cannot find corresponding level vertex for the specified position "
                       "[%f][%f][%f] for monster %s",
...
LPCSTR CScriptEntity::GetPatrolPathName() {
    ...
    if (!GetScriptControl()) {
        ai().script_engine().script_log(
            ...
            "Object %s is not under script control while you are trying to get patrol path name!",
            *m_object->cName());
        return "";
    }
#endif
...
void CScriptEntity::sound_callback(const CObject* object, int sound_type, const Fvector& position,
                                   float sound_power) {
