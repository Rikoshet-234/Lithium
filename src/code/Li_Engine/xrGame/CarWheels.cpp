...
IC void CCar::SWheel::applywheelCollisionParams(const dxGeomUserData* ud, bool& do_colide,
                                                dContact& c, SGameMtl* material_1,
                                                SGameMtl* material_2) {
...
void CCar::SWheel::WheellCollisionCallback(bool& do_colide, bool bo1, dContact& c,
...
void CCar::SWheel::Init() {
    ...
    // R_ASSERT2(dJointGetType(joint->GetDJoint())==dJointTypeHinge2,"No wheel join was set for a
    // wheel, only wheel-joint valid!!!");
...
void CCar::SWheelSteer::Init() {
    ...
    float steering_torque =
        pKinematics->LL_UserData()->r_float("car_definition", "steering_torque");
    VERIFY(pwheel);
...
float CCar::SWheelSteer::GetSteerAngle() {
    ...
    VERIFY(pwheel->joint);
    return -pos_right *
           pwheel->joint->GetAxisAngle(0); // dJointGetHinge2Angle1 (pwheel->joint->GetDJoint());
}
...
void CCar::SWheelBreak::Load(LPCSTR section) {
    ...
    hand_break_torque =
        READ_IF_EXISTS(ini, r_float, "car_definition", "hand_break_torque", break_torque);
    if (ini->section_exist(section)) {
        break_torque = READ_IF_EXISTS(ini, r_float, section, "break_torque", break_torque);
        hand_break_torque =
            READ_IF_EXISTS(ini, r_float, section, "hand_break_torque", hand_break_torque);
    }
}
