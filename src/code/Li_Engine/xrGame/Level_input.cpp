...
#define CURRENT_ENTITY() \
    (game ? ((GameID() == eGameIDSingle) ? CurrentEntity() : CurrentControlEntity()) : NULL)
...
void CLevel::IR_OnMouseWheel(int direction) {
    ...
    if (CURRENT_ENTITY()) {
        IInputReceiver* IR =
            smart_cast<IInputReceiver*>(smart_cast<CGameObject*>(CURRENT_ENTITY()));
        if (IR)
...
#ifdef INGAME_EDITOR
    if (Device.editor() &&
        (pInput->iGetAsyncKeyState(DIK_LALT) || pInput->iGetAsyncKeyState(DIK_RALT)))
        return;
        ...
        if (i < n) {
            ...
            for (++i; i < n; ++i) {
                CEntityAlive* tpEntityAlive =
                    smart_cast<CEntityAlive*>(Objects.o_get_by_iterator(i));
                if (tpEntityAlive) {
                    bOk = true;
                    break;
                }
