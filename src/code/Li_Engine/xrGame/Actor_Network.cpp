...
CActor* Actor() {
    R_ASSERT2(GameID() == eGameIDSingle,
...
void CActor::OnChangeVisual() {
    ...
    if (V) {
        ...
        m_r_hand = smart_cast<IKinematics*>(Visual())->LL_BoneID(
            pSettings->r_string(*cNameSect(), "weapon_bone0"));
        m_l_finger1 = smart_cast<IKinematics*>(Visual())->LL_BoneID(
            pSettings->r_string(*cNameSect(), "weapon_bone1"));
        m_r_finger2 = smart_cast<IKinematics*>(Visual())->LL_BoneID(
            pSettings->r_string(*cNameSect(), "weapon_bone2"));
        //-------------------------------------------------------------------------------
    ...
    */
    ...
    if (m_bInInterpolation) {
        ...
        for (u32 k = 0; k < 3; k++) {
            ...
            SP1[k] =
                (c * c * SCoeff[k][0] * 3 + c * SCoeff[k][1] * 2 + SCoeff[k][2]) / 3; //     3 !!!!

            HP0[k] = c * (c * (c * HCoeff[k][0] + HCoeff[k][1]) + HCoeff[k][2]) + HCoeff[k][3];
            HP1[k] =
                (c * c * HCoeff[k][0] * 3 + c * HCoeff[k][1] * 2 + HCoeff[k][2]) / 3; //     3 !!!!
        };
    ...
    if (g_Alive() && m_bInInterpolation) {
        ...
        if (CurTime >= m_dwIEndTime) {
            ...
            for (u32 k = 0; k < 3; k++) {
                IPosL[k] = NewPos[k];
                IPosS[k] =
                    factor * (factor * (factor * SCoeff[k][0] + SCoeff[k][1]) + SCoeff[k][2]) +
                    SCoeff[k][3];
                IPosH[k] =
                    factor * (factor * (factor * HCoeff[k][0] + HCoeff[k][1]) + HCoeff[k][2]) +
                    HCoeff[k][3];
            };
            ...
            case 1: {
                for (int k = 0; k < 3; k++)
                    SpeedVector[k] = (factor * factor * SCoeff[k][0] * 3 +
                                      factor * SCoeff[k][1] * 2 + SCoeff[k][2]) /
                                     3; //     3       !!!!
            ...
            case 2: {
                for (int k = 0; k < 3; k++)
                    SpeedVector[k] = (factor * factor * HCoeff[k][0] * 3 +
                                      factor * HCoeff[k][1] * 2 + HCoeff[k][2]);
...
std::lower_bound(B,E,u64(ph_world->m_steps_num-1)); if (I != E)
        ...
        else
        {
                ...
                if (SMemoryPosStack.front().dwTime0 < (Level().timeServer() - 2000))
SMemoryPosStack.pop_front();
        };
};
...
void CActor::OnRender_Network() {
    ...
    //	dbg_draw_piramid(Position(), m_PhysicMovementControl->GetVelocity(), size/2, -r_model_yaw,
    ...
    if (g_Alive()) {
        ...
        if (dbg_net_Draw_Flags.test(dbg_draw_actor_alive) && V) {
            if (this != Level().CurrentViewEntity() || cam_active != eacFirstEye) {
                ...
                for (u16 i=0; i<BoneCount; i++)
                {
                        ...
                        Level().debug_renderer().draw_obb(BoneMatrix, BoneOBB.m_halfsize,
                ...
                if (Skeleton) {
                    ...
                    for (CCF_Skeleton::ElementVec::const_iterator I = Elements.begin();
                        ...
                        case SBoneShape::stBox: {
                            ...
                            Level().debug_renderer().draw_obb(M, h_size,
                                                              color_rgba(0, 255, 0, 255));
                        } break;
        ...
        dbg_draw_piramid(Position(), character_physics_support()->movement()->GetVelocity(), size,
                         -r_model_yaw, color_rgba(128, 255, 128, 255));
        dbg_draw_piramid(IStart.Pos, IStart.Vel, size, -IStart.o_model, color_rgba(255, 0, 0, 255));
        //		Fvector tmp, tmp1; tmp1.set(0, .1f, 0);
        //		dbg_draw_piramid(tmp.add(IStartT.Pos, tmp1), IStartT.Vel, size,
        //-IStartT.o_model, color_rgba(155, 0, 0, 155));
        dbg_draw_piramid(IRec.Pos, IRec.Vel, size, -IRec.o_model, color_rgba(0, 0, 255, 255));
        //		dbg_draw_piramid(tmp.add(IRecT.Pos, tmp1), IRecT.Vel, size, -IRecT.o_model,
        //color_rgba(0, 0, 155, 155));
        dbg_draw_piramid(IEnd.Pos, IEnd.Vel, size, -IEnd.o_model, color_rgba(0, 255, 0, 255));
        //		dbg_draw_piramid(tmp.add(IEndT.Pos, tmp1), IEndT.Vel, size, -IEndT.o_model,
        //color_rgba(0, 155, 0, 155));
        ...
        Fvector point0S, point1S, point0H, point1H, point0L, point1L, *ppoint0 = NULL,
                                                                      *ppoint1 = NULL;
        Fvector tS, tH;
        ...
        for (int i = 0; i < 11; i++) {
            ...
            for (u32 k = 0; k < 3; k++) {
                ...
                point1H[k] =
                    c * (c * (c * HCoeff[k][0] + HCoeff[k][1]) + HCoeff[k][2]) + HCoeff[k][3];
        ...
        PS.set(IPosS);
        PS.y += 1;
        //		Level().debug_renderer().draw_aabb			(PS, size, size, size,
        //color_rgba(128, 128, 255, 255)); 		Level().debug_renderer().draw_aabb
        ...
        if (!m_States.empty()) {
            ...
            if (!g_Alive() && PHGetSyncItemsNumber() > 2) {
                ...
                Level().debug_renderer().draw_aabb(LC, LS.x, LS.y, LS.z,
                                                   color_rgba(255, 128, 128, 255));
                //-----------------------------------------------------------------
            };
        }
    }
