    ...
    if (m_pPhysicsShell) {
        if (pHDS->hit_type == ALife::eHitTypeExplosion) {
            ...
            if ((pHDS->impulse > EPS) && (BI_NONE != pHDS->bone()))
                m_pPhysicsShell->applyImpulseTrace(pHDS->p_in_bone_space, pHDS->dir, pHDS->impulse,
                                                   pHDS->bone());
        }
    }
}
...
void CBreakableObject::CreateBroken() {
    ...
    m_Shell->SetMaxAABBRadius(std::max(std::max(b.m_halfsize.x, b.m_halfsize.y), b.m_halfsize.z) *
                              2.f); //+2.f
}
...
void CBreakableObject::Break() {
    ...
    for (u16 i = 0; i < el_num; i++) {
        ...
        dir.normalize();
        m_pPhysicsShell->get_ElementByStoreOrder(i)->applyImpulseTrace(pos, dir,
                                                                       Random.randF(0.5f, 3.f), 0);
    }
...
void CBreakableObject::CollisionHit(u16 source_id, u16 bone_id, float c_damage, const Fvector& dir,
                                    Fvector& pos) {
