...
void CCustomRocket::SetLaunchParams(const Fmatrix& xform, const Fvector& vel,
    ...
    m_time_to_explode =
        Device.fTimeGlobal + pSettings->r_float(cNameSect(), "force_explode_time") / 1000.0f;
#ifdef DEBUG
    gbg_rocket_speed1 = 0;
    gbg_rocket_speed2 = 0;
#endif
...
void CCustomRocket::create_physic_shell() {
    ...
    CHOOSE_MAX(obb.m_halfsize.x, ax.set(obb.m_rotate.i); ax.mul(obb.m_halfsize.x);
               ...
               ax.mul(obb.m_halfsize.y); radius = std::min(obb.m_halfsize.x, obb.m_halfsize.z);
               obb.m_halfsize.x /= 2.f;
               obb.m_halfsize.z /= 2.f, obb.m_halfsize.z, ax.set(obb.m_rotate.k);
               ax.mul(obb.m_halfsize.z); radius = std::min(obb.m_halfsize.y, obb.m_halfsize.x);
...
void CCustomRocket::ObjectContactCallback(bool& do_colide, bool bo1, dContact& c,
...
void CCustomRocket::reload(LPCSTR section) {
    ...
    if (pSettings->line_exist(section, "snd_fly_sound")) {
        m_flyingSound.create(pSettings->r_string(section, "snd_fly_sound"), st_Effect,
                             sg_SourceType);
    }
}
...
void CCustomRocket::UpdateEngine() {
    ...
    if (!getVisible()) {
        Msg("! CCustomRocket::UpdateEngine called, but false==getVisible() id[%d] frame[%d]", ID(),
            Device.dwFrame);
    }
...
void CCustomRocket::StartFlyParticles() {
    ...
    VERIFY3(m_pFlyParticles->IsLooped(), "must be a looped particle system for rocket fly: %s",
            *m_sFlyParticles);
}
