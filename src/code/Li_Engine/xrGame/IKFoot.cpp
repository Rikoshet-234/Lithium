...
CIKFoot::CIKFoot()
    : m_bind_b2_to_b3(Fidentity), m_ref_bone(u16(-1)), m_foot_bone_id(BI_NONE),
      m_toe_bone_id(BI_NONE), m_K(0) {}
...
void CIKFoot::set_toe(u16 bones[4]) {
    ...
    m_heel_position.v.add(
        Fvector().mul(foot_dir, Fvector().sub(m_toe_position.v, pos).dotproduct(foot_dir) * 0.2f));
...
Fmatrix& CIKFoot::ref_bone_to_foot(Fmatrix& foot, const Fmatrix& ref_bone) const {
    ...
    /*
            ...
            //m.mul_43( Fmatrix().invert( Kinematics()->LL_GetTransform(m_bones[2] )
...
ik_goal_matrix::e_collide_state CIKFoot::CollideFoot(float angle, float& out_angle,
    ...
    float dfoot_tri =
        -p.d - p.n.dotproduct(global_bone_pos); // dist from foot bone pos to tri plain
    Fvector axp;
...
bool CIKFoot::GetFootStepMatrix(ik_goal_matrix& m, const SCalculateData& cd,
                                const SIKCollideData& cld, bool collide, bool rotate) const {
    return GetFootStepMatrix(m, cd.state.anim_pos, cld, collide, rotate);
}

ik_goal_matrix::e_collide_state CIKFoot::rotate(Fmatrix& xm, const Fplane& p,
                                                const Fvector& foot_normal,
...
bool CIKFoot::GetFootStepMatrix(ik_goal_matrix& m, const Fmatrix& g_anim, const SIKCollideData& cld,
                                bool collide, bool rotation, bool b_make_shift /*=true*/) const {
...
//{
//	DBG_DrawLine( global_point, Fvector().add( global_point, foot_normal ), D3DCOLOR_XRGB( 0,
//255, 255) );
//}
#endif
...
void CIKFoot::set_ref_bone() {
    set_ref_bone(get_ref_bone(Kinematics()->LL_GetTransform(m_foot_bone_id),
                              Kinematics()->LL_GetTransform(m_toe_bone_id)));
}
void CIKFoot::SetFootGeom(ik_foot_geom& fg, const Fmatrix& ref_bone,
