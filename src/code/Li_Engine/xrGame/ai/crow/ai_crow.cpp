...
void CAI_Crow::Load(LPCSTR section) {
    ...
    VERIFY2(valid_pos(Position()),
            dbg_valide_pos_string(Position(), this, "CAI_Crow::Load( LPCSTR section )"));
}
...
void CAI_Crow::switch2_DeathFall() {
    ...
    smart_cast<IKinematicsAnimated*>(Visual())->PlayCycle(m_Anims.m_death.GetRandom(), TRUE,
                                                          cb_OnHitEndPlaying, this);
}
...
void CAI_Crow::state_Flying(float fdt) {
    ...
    VERIFY2(valid_pos(Position()),
            dbg_valide_pos_string(Position(), this, "state_Flying		(float fdt)"));
}
...
void CAI_Crow::state_DeathFall() {
    ...
    VERIFY2(valid_pos(Position()),
            dbg_valide_pos_string(Position(), this, "CAI_Crow::state_DeathFall()"));
}
...
void CAI_Crow::UpdateCL() {
    inherited::UpdateCL();
    VERIFY2(valid_pos(Position()),
            dbg_valide_pos_string(Position(), this, " CAI_Crow::UpdateCL		()"));
    if (m_pPhysicsShell) {
...
void CAI_Crow::shedule_Update(u32 DT) {
    ...
    VERIFY2(
        valid_pos(Position()),
        dbg_valide_pos_string(Position(), this, " CAI_Crow::shedule_Update		(u32 DT)"));
}
...
void CAI_Crow::net_Import(NET_Packet& P) {
    ...
    VERIFY2(valid_pos(Position()),
            dbg_valide_pos_string(Position(), this, " CAI_Crow::net_Import	(NET_Packet& P)"));
}
//---------------------------------------------------------------------
...
//---------------------------------------------------------------------
void CAI_Crow::HitImpulse(float /**amount/**/, Fvector& /**vWorldDir/**/,
                          Fvector& /**vLocalDir/**/) {}
//---------------------------------------------------------------------
void CAI_Crow::CreateSkeleton() {
    m_pPhysicsShell =
        P_build_Shell(this, false, (BONE_P_MAP*)0); // P_build_SimpleShell(this,0.3f,false);
...
// void CAI_Crow::Hit	(float P, Fvector &dir, CObject* who, s16 element,Fvector
