...
void CBaseMonster::Load(LPCSTR section) {
    ...
    //		m_item_section					=
    //pSettings->r_string(section,"Spawn_Inventory_Item_Section"); 		m_spawn_probability
    //= pSettings->r_float(section,"Spawn_Inventory_Item_Probability"); 	} else
    //m_spawn_probability			= 0.f;

    m_melee_rotation_factor =
        READ_IF_EXISTS(pSettings, r_float, section, "Melee_Rotation_Factor", 1.5f);
    berserk_always = !!READ_IF_EXISTS(pSettings, r_bool, section, "berserk_always", false);

    m_feel_enemy_who_just_hit_max_distance =
        READ_IF_EXISTS(pSettings, r_float, section, "feel_enemy_who_just_hit_max_distance",
    ...
    m_feel_enemy_who_made_sound_max_distance =
        READ_IF_EXISTS(pSettings, r_float, section, "feel_enemy_who_made_sound_max_distance",
    ...
    if ((separate_factor > 0.0001f) && (separate_range > 0.01f)) {
        ...
        m_grouping_behaviour = xr_new<squad_grouping_behaviour>(
            this, Fvector3().set(0.f, 0.f, 0.f), Fvector3().set(0.f, separate_factor, 0.f),
            separate_range);
    ...
    if (pSettings->line_exist(section, "protections_sect")) {
        ...
        m_fHitFracMonster =
            READ_IF_EXISTS(pSettings, r_float, protections_sect, "hit_fraction_monster", 0.1f);
    }
...
void CBaseMonster::PostLoad(LPCSTR section) {
    ...
    aom.far_radius = READ_IF_EXISTS(pSettings, r_float, section, "aom_far_radius",
                                    detail::base_monster::aom_far_radius);
    aom.attack_radius = READ_IF_EXISTS(pSettings, r_float, section, "aom_attack_radius",
                                       detail::base_monster::aom_attack_radius);
    aom.update_side_period = READ_IF_EXISTS(pSettings, r_float, section, "aom_update_side_period",
                                            detail::base_monster::aom_update_side_period);
    aom.prediction_factor = READ_IF_EXISTS(pSettings, r_float, section, "aom_prediction_factor",
                                           detail::base_monster::aom_prediction_factor);
    aom.prepare_time = READ_IF_EXISTS(pSettings, r_float, section, "aom_prepare_time",
                                      detail::base_monster::aom_prepare_time);
    aom.prepare_radius = READ_IF_EXISTS(pSettings, r_float, section, "aom_prepare_radius",
                                        detail::base_monster::aom_prepare_radius);
    aom.max_go_close_time =
        READ_IF_EXISTS(pSettings, r_float, section, "aom_max_go_close_time", 8.f);
    ...
    if (pSettings->line_exist(section, "anti_aim_effectors")) {
        ...
        pcstr anti_aim_animation =
            READ_IF_EXISTS(pSettings, r_string, section, "anti_aim_animation", "stand_attack_");
...
#define LOAD_SOUND(sound_name, _type, _prior, _mask, _int_type)                                    \
    if (pSettings->line_exist(section, sound_name))                                                \
        sound().add(pSettings->r_string(section, sound_name), DEFAULT_SAMPLE_COUNT, _type, _prior, \
                    u32(_mask), _int_type, m_head_bone_name);
...
void CBaseMonster::reload(LPCSTR section) {
    ...
    // load base sounds
    LOAD_SOUND("sound_idle", SOUND_TYPE_MONSTER_TALKING, MonsterSound::eLowPriority,
               MonsterSound::eBaseChannel, MonsterSound::eMonsterSoundIdle);
    ...
    LOAD_SOUND("sound_eat", SOUND_TYPE_MONSTER_TALKING, MonsterSound::eNormalPriority + 4,
               MonsterSound::eBaseChannel, MonsterSound::eMonsterSoundEat);
    ...
    LOAD_SOUND("sound_steal", SOUND_TYPE_MONSTER_STEP, MonsterSound::eNormalPriority + 1,
               MonsterSound::eBaseChannel, MonsterSound::eMonsterSoundSteal);
    LOAD_SOUND("sound_panic", SOUND_TYPE_MONSTER_STEP, MonsterSound::eNormalPriority + 2,
               MonsterSound::eBaseChannel, MonsterSound::eMonsterSoundPanic);
...
void CBaseMonster::settings_read(CInifile const* ini, LPCSTR section, SMonsterSettings& data) {
    ...
    if (ability_run_attack()) {
        ...
        READ_SETTINGS(data.m_run_attack_start_dist, "RunAttack_StartDistance", r_float, ini,
                      section);
    }
    ...
    if (ini->line_exist(section, "attack_effector")) {
        ...
        READ_SETTINGS(data.m_attack_effector.ppi.noise.intensity, "noise_intensity", r_float, ini,
                      ppi_section);
        READ_SETTINGS(data.m_attack_effector.ppi.noise.grain, "noise_grain", r_float, ini,
                      ppi_section);
        ...
        if (ini->line_exist(ppi_section, "color_base"))
            sscanf(ini->r_string(ppi_section, "color_add"), "%f,%f,%f",
                   &data.m_attack_effector.ppi.color_add.r, &data.m_attack_effector.ppi.color_add.g,
        ...
        READ_SETTINGS(data.m_attack_effector.time_release, "time_release", r_float, ini,
                      ppi_section);

        READ_SETTINGS(data.m_attack_effector.ce_time, "ce_time", r_float, ini, ppi_section);
        READ_SETTINGS(data.m_attack_effector.ce_amplitude, "ce_amplitude", r_float, ini,
                      ppi_section);
        READ_SETTINGS(data.m_attack_effector.ce_period_number, "ce_period_number", r_float, ini,
                      ppi_section);
