...
void CBaseMonster::feel_sound_new(CObject* who, int eType, CSound_UserDataPtr user_data,
...
void CBaseMonster::HitEntity(const CEntity* pEntity, float fDamage, float impulse, Fvector& dir,
    ...
    if (EnemyMan.get_enemy() == pEntity) {
        ...
        if (pEntityNC == Actor() && draw_hit_marks) {
            ...
            CEffectorCam* ce =
                Actor()->Cameras().GetCamEffector((ECamEffectorType)effBigMonsterHit);
            if (!ce) {
...
void CBaseMonster::Hit_Psy(CObject* object, float value) {
    NET_Packet P;
    SHit HS;
    HS.GenHeader(GE_HIT, object->ID()); //					//	u_EventGen
                                        //(P,GE_HIT, object->ID());				//
    ...
    HS.weaponID =
        (ID());                              // own		//	P.w_u16			(ID());
                                             // // own
    HS.dir = (Fvector().set(0.f, 1.f, 0.f)); // direction	//	P.w_dir
                                             // (Fvector().set(0.f,1.f,0.f));
                                             // // direction
    HS.power = (value); // hit value	//	P.w_float		(value);
                        // // hit value
    HS
        .boneID = (BI_NONE); // bone		//	P.w_s16
                             // (BI_NONE);
                             // // bone
    HS.p_in_bone_space =
        (Fvector().set(0.f, 0.f, 0.f)); //	P.w_vec3 (Fvector().set(0.f,0.f,0.f));
    HS.impulse = (0.f);                 //	P.w_float		(0.f);
    HS.hit_type =
        (ALife::eHitTypeTelepatic); //	P.w_u16			(u16(ALife::eHitTypeTelepatic));
    HS.Write_Packet(P);
    u_EventSend(P);
}
