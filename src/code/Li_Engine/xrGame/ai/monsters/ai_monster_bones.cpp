...
void bonesBone::Turn(u32 dt) {
    ...
    float cur_speed =
        params.r_speed *
        _cos(PI_DIV_2m - PIm * _abs(params.target_yaw - params.cur_yaw) / params.dist_yaw);
...
void bonesManipulation::SetMotion(CBoneInstance* bone, u8 axis, float target_yaw, float r_speed,
                                  u32 t) {
    int index = -1;
        ...
        if ((0 != time_started) && (time_started + freeze_time < cur_time)) { // время вышло?
            ...
            for (u32 i = 0; i < m_Bones.size(); ++i) {
                ...
                m_Bones[i].params.dist_yaw =
                    _abs(m_Bones[i].params.target_yaw - m_Bones[i].params.cur_yaw);
            }
            bActive = false;
        }
    }
