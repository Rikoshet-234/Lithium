...
CAI_Rat::CAI_Rat() : m_behaviour_manager(0) {
    ...
    //	m_behaviour_manager->add		(xr_new<steering_behaviour::cohesion>(this),
    //.5f);
    //	m_behaviour_manager->add		(xr_new<steering_behaviour::separation>(this),
    //.5f);
    //	m_behaviour_manager->add		(xr_new<steering_behaviour::alignment>(this),
    //.5f);
}
...
void CAI_Rat::reload(LPCSTR section) {
    ...
    sound().add(pSettings->r_string(section, "sound_death"), 100, SOUND_TYPE_MONSTER_DYING, 0,
                u32(eRatSoundMaskDie), eRatSoundDie, head_bone_name);
    ...
    sound().add(pSettings->r_string(section, "sound_eat"), 100, SOUND_TYPE_MONSTER_EATING, 3,
                u32(eRatSoundMaskEat), eRatSoundEat, head_bone_name);
}
...
BOOL CAI_Rat::net_Spawn(CSE_Abstract* DC) {
    ...
    if (g_Alive())
        m_tSpawnPosition.set(
            Level().seniority_holder().team(g_Team()).squad(g_Squad()).leader()->Position());
    else
...
void CAI_Rat::net_Export(NET_Packet& P) {
    ...
    P.w(&l_game_vertex_id, sizeof(l_game_vertex_id));
    //	P.w						(&m_fGoingSpeed,
    //sizeof(m_fGoingSpeed)); 	P.w						(&m_fGoingSpeed,
    //sizeof(m_fGoingSpeed));
    float f1 = 0;
...
void CAI_Rat::CreateSkeleton() {
    ...
    if (!fsimilar(0.f, m_saved_impulse)) {
        m_pPhysicsShell->applyHit(m_saved_hit_position, m_saved_hit_dir, m_saved_impulse, 0,
                                  m_saved_hit_type);
    }
    /*
...
///////////////////////////////////////////////////////////////////////////////////////
    ...
    if (!Useful()) {
        ...
        if (squad && ((squad->GetLeader() != this && !squad->GetLeader()->g_Alive()) ||
...
// void CAI_Rat::Hit(float P,Fvector &dir,CObject*who,s16 element,Fvector p_in_object_space,float
// impulse, ALife::EHitType hit_type /*= ALife::eHitTypeWound*/)
void CAI_Rat::Hit(SHit* pHDS) {
    ...
    if (!m_pPhysicsShell) {
        ...
        //		CEatableItem::Hit		(P,dir,who,element,p_in_object_space,impulse,
        //hit_type);
