...
void CAI_Rat::make_turn() {
    ...
    if (m_bFiring && (angle_difference(movement().m_body.target.yaw,
...
bool CAI_Rat::calc_node(Fvector const& next_position) {
    ...
    if (ai().level_graph().valid_vertex_id(dwNewNode) &&
        ai().level_graph().inside(*tpNewNode, QueryPos)) {
        m_tNewPosition.y =
            ai().level_graph().vertex_plane_y(*tpNewNode, next_position.x, next_position.z);
...
void CAI_Rat::move(bool bCanAdjustSpeed, bool bStraightForward) {
    ...
    if ((angle_difference(movement().m_body.target.yaw, movement().m_body.current.yaw) >
         PI_DIV_6)) {
        make_turn();
        return;
    }
    ...
    if (m_bNoWay && (!m_turning || (angle_difference(movement().m_body.target.yaw,
...
Fvector CAI_Rat::get_next_target_point() {
    ...
    if (Position().distance_to(
            ai().level_graph().vertex_position(vertex->data().level_vertex_id())) < 1.5f) {
