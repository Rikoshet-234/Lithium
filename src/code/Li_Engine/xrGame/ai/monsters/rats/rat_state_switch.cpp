...
bool CAI_Rat::get_if_tp_entity() {
    if (m_tLastSound.tpEntity && (m_tLastSound.tpEntity->g_Team() != g_Team()) &&
...
CAI_Rat::ERatStates CAI_Rat::get_state() {
    return ERatStates(dwfChooseAction(m_dwActionRefreshRate, m_fAttackSuccessProbability,
                                      m_fAttackSuccessProbability, m_fAttackSuccessProbability,
                                      m_fAttackSuccessProbability, g_Team(), g_Squad(), g_Group(),
                                      aiRatAttackMelee, aiRatAttackMelee, aiRatAttackMelee,
                                      aiRatRetreat, aiRatRetreat, this, 30.f));
}

bool CAI_Rat::switch_if_porsuit() {
    if (m_home_position.distance_to(memory().enemy().selected()->Position()) >
        m_fMaxPursuitRadius) {
        return true;
    }
    return false;
...
void CAI_Rat::set_dir() {
    if ((Device.dwTimeGlobal - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time) {
        ...
        if (squad && squad->SquadActive()) {
            ...
            m_dest_direction.x =
                (m_temp.x - m_enemy_position.x) / m_temp.distance_to(m_enemy_position);
            m_dest_direction.y =
                (m_temp.y - m_enemy_position.y) / m_temp.distance_to(m_enemy_position);
            m_dest_direction.z =
                (m_temp.z - m_enemy_position.z) / m_temp.distance_to(m_enemy_position);
...
bool CAI_Rat::switch_if_no_enemy() {
    if (!switch_if_enemy() ||
        (switch_if_enemy() &&
         (!switch_if_alife() ||
          ((Device.dwTimeGlobal - memory().memory(memory().enemy().selected()).m_level_time >
            m_dwRetreatTime) &&
...
bool CAI_Rat::switch_if_lost_rtime() {
    if (Device.dwTimeGlobal - memory().memory(memory().enemy().selected()).m_level_time >=
        m_dwLostRecoilTime) {
        return true;
    }
    return false;
