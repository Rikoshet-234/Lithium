...
void CBurer::Load(LPCSTR section) {
    ...
    ::Sound->create(sound_gravi_wave, pSettings->r_string(section, "sound_gravi_wave"), st_Effect,
                    SOUND_TYPE_WORLD);
    ::Sound->create(sound_tele_hold, pSettings->r_string(section, "sound_tele_hold"), st_Effect,
                    SOUND_TYPE_WORLD);
    ::Sound->create(sound_tele_throw, pSettings->r_string(section, "sound_tele_throw"), st_Effect,
                    SOUND_TYPE_WORLD);
    ...
    m_weight_to_stamina_hit =
        READ_IF_EXISTS(pSettings, r_float, section, "weight_to_stamina_hit", 0.02f);
    m_weapon_drop_stamina_k =
        READ_IF_EXISTS(pSettings, r_float, section, "weapon_drop_stamina_k", 3.f);
    ...
    m_shield_keep_particle =
        READ_IF_EXISTS(pSettings, r_string, section, "shield_keep_particle", 0);
    m_shield_keep_particle_period =
        READ_IF_EXISTS(pSettings, r_u32, section, "shield_keep_particle_period", 1000);
    ...
    // "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r"); 	anim().AddAnim(eAnimSteal,
    // "stand_steal_",			-1, &velocity_steal,	PS_STAND); //, 	"fx_stand_f", "fx_stand_b",
    // "fx_stand_l", "fx_stand_r"); 	anim().AddAnim(eAnimEat,			"sit_eat_",
    // -1, &velocity_none,		PS_SIT); //, 	"fx_stand_f", "fx_stand_b", "fx_stand_l",
    // "fx_stand_r");
    //
    // 	anim().AddAnim(eAnimSitIdle,		"sit_idle_",			-1, &velocity_none,		PS_SIT); //,
    // "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
    // 	anim().AddAnim(eAnimCheckCorpse,	"sit_check_corpse_",	-1, &velocity_none,		PS_SIT); //,
    // "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
    ...
    //	anim().AddTransition(PS_SIT,		PS_STAND,		eAnimSitStandUp,
    //false); 	anim().AddTransition(PS_STAND,		PS_SIT,			eAnimStandSitDown,
    //false);
...
void xr_stdcall CBurer::StaminaHit() {
    ...
    bool const do_weapon_drop =
        Actor()->conditions().GetPower() < stamina_hit * m_weapon_drop_stamina_k;
...
void CBurer::UpdateGraviObject() {
    ...
    if (Level().ObjectSpace.RayPick(new_pos, dir, trace_dist, collide::rqtBoth, l_rq, NULL)) {
        ...
        if ((l_rq.O == enemy) && (l_rq.range < trace_dist)) {
            ...
            if (b_enemy_visible) {
                ...
                HitEntity(m_gravi_object.enemy, m_gravi.hit_power, m_gravi.impulse_to_enemy,
                          impulse_dir, ALife::eHitTypeStrike, false);
                m_gravi_object.deactivate();
                return;
            }
        }
    ...
    for (u32 i = 0; i < m_nearest.size(); i++) {
        ...
        dir.normalize();
        obj->m_pPhysicsShell->applyImpulse(dir, m_gravi.impulse_to_objects *
...
void CBurer::StartGraviPrepare() {
    ...
    pA->CParticlesPlayer::StartParticles(particle_gravi_prepare, Fvector().set(0.0f, 0.1f, 0.0f),
                                         pA->ID());
}
