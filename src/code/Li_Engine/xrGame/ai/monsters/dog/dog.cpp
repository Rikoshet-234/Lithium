...
void CAI_Dog::Load(LPCSTR section) {
    ...
    // anim().AddAnim(eAnimSniff,		"stand_sniff_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimHowling,		"stand_howling_",		-1,
    // &velocity_none,		PS_STAND);

    // anim().AddAnim(eAnimJumpGlide,   	"jump_glide_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimJumpGlide,   	"stand_jump_left_",		 0,
    // &velocity_none,		PS_STAND);
    ...
    anim().AddAnim(eAnimHomeWalkSmelling, "stand_walk_smelling_", -1, &velocity_walk_smell,
                   PS_STAND);
    ...
    // define transitions
    // order : 1. [anim -> anim]	2. [anim->state]	3. [state -> anim]		4. [state ->
    // state]
...
void CAI_Dog::start_animation() {
    ...
    smart_cast<IKinematicsAnimated*>(Visual())->PlayCycle(get_current_animation(), TRUE,
                                                          animation_end, this);
    b_state_end = true;
}
