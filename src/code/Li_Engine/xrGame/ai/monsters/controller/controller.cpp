...
void CController::Load(LPCSTR section) {
    ...
    ::Sound->create(control_start_sound, pSettings->r_string(section, "sound_control_start"),
                    st_Effect, SOUND_TYPE_WORLD);
    ::Sound->create(control_hit_sound, pSettings->r_string(section, "sound_control_hit"), st_Effect,
                    SOUND_TYPE_WORLD);
    ...
    sscanf(pSettings->r_string(ppi_section, "color_base"), "%f,%f,%f",
           &m_control_effector.ppi.color_base.r, &m_control_effector.ppi.color_base.g,
           &m_control_effector.ppi.color_base.b);
    sscanf(pSettings->r_string(ppi_section, "color_gray"), "%f,%f,%f",
           &m_control_effector.ppi.color_gray.r, &m_control_effector.ppi.color_gray.g,
           &m_control_effector.ppi.color_gray.b);
    sscanf(pSettings->r_string(ppi_section, "color_add"), "%f,%f,%f",
           &m_control_effector.ppi.color_add.r, &m_control_effector.ppi.color_add.g,
    ...
    // move().get_velocity(MonsterMovement::eVelocityParameterRunNormal);  SVelocityParam
    // &velocity_walk_dmg	=
    // move().get_velocity(MonsterMovement::eVelocityParameterWalkDamaged);  SVelocityParam
    // &velocity_run_dmg	= move().get_velocity(MonsterMovement::eVelocityParameterRunDamaged);
    ...
    // anim().AddAnim(eAnimStandIdle,		"new_torso_steal_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimStandTurnLeft,	"new_torso_steal_",
    // -1, &velocity_turn,		PS_STAND);  anim().AddAnim(eAnimStandTurnRight,
    // "new_torso_steal_",			-1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimStandDamaged,	"new_torso_steal_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimSitIdle,		"sit_idle_",			-1,
    // &velocity_none,		PS_SIT);  anim().AddAnim(eAnimEat,			"sit_eat_",
    // -1, &velocity_none,		PS_SIT);  anim().AddAnim(eAnimWalkFwd,
    // "new_torso_steal_",			-1, &velocity_steal,	PS_STAND);
    // anim().AddAnim(eAnimWalkDamaged,	"new_torso_steal_",			-1, &velocity_steal,
    // PS_STAND);  anim().AddAnim(eAnimRun,			"new_torso_steal_",
    // -1,	&velocity_steal,	PS_STAND);  anim().AddAnim(eAnimRunDamaged,
    // "new_torso_steal_",			-1, &velocity_steal,	PS_STAND);
    // anim().AddAnim(eAnimAttack,			"stand_attack_",		-1, &velocity_turn,
    // PS_STAND);  anim().AddAnim(eAnimSteal,			"new_torso_steal_",
    // -1, &velocity_steal,	PS_STAND);  anim().AddAnim(eAnimCheckCorpse,
    // "stand_check_corpse_",	-1,	&velocity_none,		PS_STAND);  anim().AddAnim(eAnimDie,
    // "stand_die_",			-1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimStandSitDown,	"stand_sit_down_",		-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimSitStandUp,		"sit_stand_up_",		-1,
    // &velocity_none,		PS_SIT);  anim().AddAnim(eAnimSleep,
    // "sit_sleep_",			-1, &velocity_none,		PS_SIT);

    // anim().AddAnim(eAnimStandIdle,		"stand_idle_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimStandTurnLeft,	"stand_turn_ls_",		-1,
    // &velocity_turn,		PS_STAND);  anim().AddAnim(eAnimStandTurnRight,	"stand_turn_rs_",
    // -1, &velocity_turn,		PS_STAND);  anim().AddAnim(eAnimStandDamaged,
    // "stand_idle_dmg_",		-1, &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimSitIdle,		"sit_idle_",			-1, &velocity_none,
    // PS_SIT);
    // anim().AddAnim(eAnimEat,			"sit_eat_",				-1, &velocity_none,
    // PS_SIT);
    // anim().AddAnim(eAnimWalkFwd,		"stand_walk_fwd_",		-1, &velocity_walk,
    // PS_STAND);  anim().AddAnim(eAnimWalkDamaged,	"stand_walk_dmg_",		-1,
    // &velocity_walk_dmg,	PS_STAND);
    // anim().AddAnim(eAnimRun,			"run_scared_",			-1,	&velocity_run,
    // PS_STAND);  anim().AddAnim(eAnimRunDamaged,		"stand_run_dmg_",		-1,
    // &velocity_run_dmg,	PS_STAND);  anim().AddAnim(eAnimAttack,
    // "stand_attack_",		-1, &velocity_turn,		PS_STAND);
    // anim().AddAnim(eAnimSteal,			"stand_steal_",			-1, &velocity_steal,
    // PS_STAND);  anim().AddAnim(eAnimCheckCorpse,	"stand_check_corpse_",	-1,
    // &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimDie,			"stand_die_",			-1, &velocity_none,
    // PS_STAND);  anim().AddAnim(eAnimStandSitDown,	"stand_sit_down_",		-1,
    // &velocity_none,		PS_STAND);
    // anim().AddAnim(eAnimSitStandUp,		"sit_stand_up_",		-1, &velocity_none,
    // PS_SIT);
    // anim().AddAnim(eAnimSleep,			"sit_sleep_",			-1, &velocity_none,
    // PS_SIT);
    ...
    m_sound_aura_left_channel.create("monsters\\controller\\controller_psy_aura_l", st_Effect,
                                     sg_SourceType);
    m_sound_aura_right_channel.create("monsters\\controller\\controller_psy_aura_r", st_Effect,
                                      sg_SourceType);
    m_sound_aura_hit_left_channel.create("monsters\\controller\\controller_psy_hit_l", st_Effect,
                                         sg_SourceType);
    m_sound_aura_hit_right_channel.create("monsters\\controller\\controller_psy_hit_l", st_Effect,
                                          sg_SourceType);

    m_sound_tube_start.create("monsters\\controller\\controller_first_hit", st_Effect,
                              sg_SourceType);
    m_sound_tube_pull.create("monsters\\controller\\controller_whoosh", st_Effect, sg_SourceType);
    m_sound_tube_hit_left.create("monsters\\controller\\controller_final_hit_l", st_Effect,
                                 sg_SourceType);
    m_sound_tube_hit_right.create("monsters\\controller\\controller_final_hit_r", st_Effect,
                                  sg_SourceType);

    m_sound_tube_prepare.create("monsters\\controller\\controller_tube_prepare", st_Effect,
                                sg_SourceType);
...
void CController::reload(LPCSTR section) {
    ...
    com_man().ta_fill_data(anim_triple_control, "stand_sit_down_attack_0", "control_attack_0",
                           "sit_stand_up_attack_0", true, false);
}
...
void CController::control_hit() {
    ...
    Actor()->Cameras().AddPPEffector(
        xr_new<CMonsterEffector>(m_control_effector.ppi, m_control_effector.time,
...
void CController::UpdateCL() {
    ...
    if (active_control_fx) {
        ...
        float percent =
            float((Device.dwTimeGlobal - time_control_hit_started)) / float(time_to_show);
...
void CController::TranslateActionToPathParams() {
    ...
    u32 des_mask = (m_bDamaged ? MonsterMovement::eVelocityParameterWalkDamaged
...
void CController::debug_on_key(int key) {
    switch (key) {
    case DIK_MINUS:
        // m_sound_aura_left_channel.play_at_pos(Level().CurrentEntity(), Fvector().set(-1.f, 0.f,
        // 1.f), sm_2D);  m_sound_aura_right_channel.play_at_pos(Level().CurrentEntity(),
    ...
    case DIK_EQUALS:
        ...
        // m_sound_aura_hit_left_channel.play_at_pos(Level().CurrentEntity(), Fvector().set(-1.f,
        // 0.f, 1.f), sm_2D);  m_sound_aura_hit_right_channel.play_at_pos(Level().CurrentEntity(),
        // Fvector().set(1.f, 0.f, 1.f), sm_2D);
        break;
    }
}
