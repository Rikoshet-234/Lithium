...
void CControlJump::start_jump(const Fvector& point) {
    ...
    if (is_flag(SControlJumpData::ePrepareSkip)) {
        ...
        if (is_flag(SControlJumpData::ePrepareInMove)) {
            ...
            // get animation time
            float time = m_man->animation().motion_time(m_data.state_prepare_in_move.motion,
                                                        m_object->Visual());
            // set acceleration and velocity
            SVelocityParam& vel =
                m_object->move().get_velocity(m_data.state_prepare_in_move.velocity_mask);
...
void CControlJump::update_frame() {
    ...
    if (m_anim_state_current == eStateGlide && in_auto_aim()) {
        ...
        ctrl_data_dir->heading.target_angle =
            m_man->direction().angle_to_target(m_data.target_object->Position());
...
bool CControlJump::is_on_the_ground() {
    ...
    if (Level().ObjectSpace.RayPick(trace_from, direction, m_trace_ground_range, collide::rqtStatic,
                                    l_rq, m_object)) {
...
void CControlJump::calculate_jump_time(Fvector const& target, bool const check_force_factor) {
    ...
    float cur_factor =
        (check_force_factor && m_data.force_factor > 0) ? m_data.force_factor : m_jump_factor;
...
void CControlJump::hit_test() {
    ...
    if (Level().ObjectSpace.RayPick(trace_from, m_object->Direction(), m_hit_trace_range,
                                    collide::rqtObject, l_rq, m_object)) {
...
bool CControlJump::jump_intersect_geometry(Fvector const& target, CObject* const ignored_object) {
    ...
    Fvector const traj_target =
        target + Fvector().set(0, 1.2f, 0) - (normalize(start_to_target) * 1);

    if (trajectory_intersects_geometry(m_jump_time, traj_start, traj_target, velocity,
                                       collide_position, m_object, ignored_object, temp_rq_results,
...
bool CControlJump::can_jump(Fvector const& target, bool const aggressive_jump) {
    if (m_time_next_allowed != 0) {
        ...
        if (m_time_next_allowed - (int)aggressive_jump * (2 * m_delay_after_jump / 3) >
            Device.dwTimeGlobal) {
            return false;
        }
    }
    ...
    const float test_min_distance =
        aggressive_jump ? std::min(1.f, m_min_distance) : m_min_distance;
