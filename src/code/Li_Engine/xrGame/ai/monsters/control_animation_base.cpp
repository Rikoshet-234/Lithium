...
bool CControlAnimationBase::get_animation_info(EMotionAnim anim, u32 index, MotionID& motion,
                                               float& length) const {
...
bool CControlAnimationBase::CheckTransition(EMotionAnim from, EMotionAnim to) {
    ...
    while (!bVectEmpty) { // вход в цикл, если вектор переходов не пустой
        ...
        bool from_is_good =
            ((I->from.state_used) ? (I->from.state == state_from) : (I->from.anim == cur_from));
        bool target_is_good =
            ((I->target.state_used) ? (I->target.state == state_to) : (I->target.anim == to));
...
void CControlAnimationBase::UpdateAnimCount() {
    ...
    for (auto it = m_anim_storage.begin(); it != m_anim_storage.end(); it++) {
        ...
        if (count != 0)
            ...
            xr_sprintf(s, "Error! No animation: %s for monster %s", *((*it)->target_name),
                       *m_object->cName());
            R_ASSERT2(count != 0, s);
        }
    }
    ...
    for (u32 i = 0; pSettings->r_line(section, i, &anim_name, &val); ++i) {
        ...
        if (_GetItemCount(val) == 1) {
            ...
            for (u32 k = 0; pSettings->r_line(compound_section, k, &unused_line_name, &val); ++k) {
                ...
                m_man->animation().add_anim_event(anim.motion, anim.time,
