...
u32 dwfChooseAction(u32 dwActionRefreshRate, float fMinProbability0, float fMinProbability1,
                    float fMinProbability2, float fMinProbability3, u32 dwTeam, u32 dwSquad,
                    u32 dwGroup, u32 a0, u32 a1, u32 a2, u32 a3, u32 a4, CEntity* tpEntity,
                    float fGroupDistance) {
    if (fis_zero(fMinProbability0))
        return (0);

    CGroupHierarchyHolder& Group =
        Level().seniority_holder().team(dwTeam).squad(dwSquad).group(dwGroup);
    ...
    if (!tpEntity)
        ...
        for (int k = 0; k < (int)Group.members().size(); ++k) {
            if (Group.members()[k]->g_Alive() &&
                ((Group.members()[k]->spatial.type & STYPE_VISIBLEFORAI) == STYPE_VISIBLEFORAI))
                if (tpEntity->Position().distance_to(Group.members()[k]->Position()) <
                    fGroupDistance) {
...
void CAniVector::Load(IKinematicsAnimated* tpKinematics, LPCSTR caBaseName) {
    ...
    for (int i = 0;; ++i)
        if (!!(tpMotionDef = tpKinematics->ID_Cycle_Safe(
                   strconcat(sizeof(S1), S1, caBaseName, itoa(i, S2, 10))))) {
