...
void CAI_Stalker::Hit(SHit* pHDS) {
    ...
    if (g_Alive()) {
        ...
        if (!wounded() && !already_critically_wounded) {
            ...
            if (!became_critically_wounded && animation().script_animations().empty() &&
                (HDS.bone() != BI_NONE)) {
                Fvector D;
                float yaw, pitch;
                D.getHP(yaw, pitch);
...
IC float CAI_Stalker::start_pick_distance() const {
    ...
    return (
        std::max(result, memory().enemy().selected()->Position().distance_to(Position()) + 1.f));
}
...
bool CAI_Stalker::undetected_anomaly() {
    return (inside_anomaly() || brain().CStalkerPlanner::m_storage.property(
...
bool CAI_Stalker::zoom_state() const {
    ...
    if ((movement().movement_type() != eMovementTypeStand) &&
        (movement().body_state() != eBodyStateCrouch) && !movement().path_completed())
        return (false);
...
void CAI_Stalker::update_range_fov(float& new_range, float& new_fov, float start_range,
                                   float start_fov) {
...
void CAI_Stalker::compute_throw_miss(u32 const vertex_id) {
    ...
    for (u32 i = 0; i < try_count; ++i) {
        ...
        Fvector const check_position =
            Fvector().mad(m_throw_target_position, direction, throw_miss_radius);
...
void CAI_Stalker::throw_target(const Fvector& position, u32 const vertex_id,
...
bool CAI_Stalker::throw_check_error(float low, float high, const Fvector& position,
...
void CAI_Stalker::check_throw_trajectory(const float& throw_time) {
    ...
    if (!trajectory_intersects_geometry(throw_time, m_throw_position, m_throw_target_position,
                                        m_throw_velocity, m_throw_collide_position, this,
                                        m_throw_ignore_object, rq_storage, trajectory_picks,
                                        collide_tris, box_size)) {
...
void CAI_Stalker::update_throw_params() {
    ...
    if (pMissile) {
        ...
        if (!pSettings->line_exist(pMissile->cNameSect(), third_person_offset_id)) {
            ...
            m_throw_position.add(
                pSettings->r_fvector3(pMissile->cNameSect(), third_person_offset_id));
    }
#endif
...
bool CAI_Stalker::critical_wound_external_conditions_suitable() {
    ...
    //	Msg								("%6d executing critical
    //hit",Device.dwTimeGlobal);
...
void CAI_Stalker::remove_critical_hit() {
    ...
    animation().global().remove_callback(
        CStalkerAnimationPair::CALLBACK_ID(this, &CAI_Stalker::remove_critical_hit));
}
