...
bool CAI_Stalker::bfAssignObject(CScriptEntityAction* tpEntityAction) {
    ...
    if (!inherited::bfAssignObject(tpEntityAction) || !l_tObjectAction.m_tpObject ||
        !l_tpInventoryItem) {
    ...
    case eObjectActionActivate: {
        ...
        //			if (inventory().ActiveItem() && (inventory().ActiveItem()->ID() ==
        //l_tpInventoryItem->ID())) 				if (l_tpWeapon && (CWeapon::eIdle ==
        //l_tpWeapon->STATE)) 				l_tObjectAction.m_bCompleted = true;
        return ((l_tObjectAction.m_bCompleted = (CObjectHandler::goal_reached())) == false);
