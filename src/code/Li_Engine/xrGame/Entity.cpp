...
// void CEntity::Hit			(float perc, Fvector &dir, CObject* who, s16 element,Fvector
...
#pragma todo("Jim to Dima: no specific figures or comments needed")
    ...
    m_dwBodyRemoveTime =
        READ_IF_EXISTS(pSettings, r_u32, section, "body_remove_time", BODY_REMOVE_TIME);
    //////////////////////////////////////
}
...
BOOL CEntity::net_Spawn(CSE_Abstract* DC) {
    ...
    if (!E) {
        ...
        R_ASSERT2(C || T || H, "Invalid entity (no inheritance from CSE_CreatureAbstract, "
    ...
    if (g_Alive() && IsGameTypeSingle()) {
        ...
        Level().seniority_holder().team(g_Team()).squad(g_Squad()).group(g_Group()).register_member(
            this);
        ++Level()
              .seniority_holder()
              .team(g_Team())
...
void CEntity::KillEntity(u16 whoID) {
    if (whoID != ID()) {
        ...
        if (m_killer_id != ALife::_OBJECT_ID(-1)) {
            Msg("! Entity [%s][%s] already has killer with id %d, but new killer id arrived - %d",
...
void CEntity::ChangeTeam(int team, int squad, int group) {
    ...
    Level().seniority_holder().team(g_Team()).squad(g_Squad()).group(g_Group()).unregister_member(
        this);
