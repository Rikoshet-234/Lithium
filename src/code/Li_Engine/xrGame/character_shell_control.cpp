...
character_shell_control::character_shell_control()
    : m_shot_up_factor(0.f), m_after_death_velocity_factor(1.f), m_was_wounded(false),
      m_Pred_Time(0.0) {}
...
void character_shell_control::Load(LPCSTR section) {
    ...
    // gray_wolf>Читаем из ltx параметры для поддержки изменяющегося трения у персонажей во время
    // смерти  gray_wolf<
...
void OnCharacterContactInDeath(bool& do_colide, bool bo1, dContact& c, SGameMtl* /*material_1*/,
    ...
    if (bo1) {
        l_character_physic_support =
            (character_shell_control*)PHRetrieveGeomUserData(c.geom.g1)->callback_data;
    } else {
        l_character_physic_support =
            (character_shell_control*)PHRetrieveGeomUserData(c.geom.g2)->callback_data;
    }
...
void character_shell_control::TestForWounded(const Fmatrix& xform, IKinematics* CKA) {
    ...
    xrc.ray_query(Level().ObjectSpace.GetStaticModel(), position_matrix.c,
                  Fvector().set(0.0f, -1.0f, 0.0f), pelvis_factor_low_pose_detect);
...
void character_shell_control::UpdateFrictionAndJointResistanse(CPhysicsShell* sh) {
    //Преобразование skel_ddelay из кадров в секунды и линейное нарастание сопротивления в джоинтах
    //со временем от момента смерти
    ...
    float curr_joint_resistance =
        hinge_force_factor1 - (skel_remain_time * hinge_force_factor1) / skel_ddelay;
