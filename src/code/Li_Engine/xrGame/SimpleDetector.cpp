...
void CSimpleDetector::UpdateAf() {
    ...
    af_info.cur_period =
        item_type->freq.x + (item_type->freq.y - item_type->freq.x) * (fRelPow * fRelPow);
    ...
    if (af_info.snd_time > af_info.cur_period) {
        af_info.snd_time = 0;
        HUD_SOUND_ITEM::PlaySound(item_type->detect_snds, Fvector().set(0, 0, 0), this, true,
                                  false);
...
void CUIArtefactDetectorSimple::setup_internals() {
    ...
    m_flash_light->set_range(
        pSettings->r_float(m_parent->HudItemData()->m_sect_name, "flash_light_range"));
    ...
    m_on_off_light->set_range(
        pSettings->r_float(m_parent->HudItemData()->m_sect_name, "onoff_light_range"));
