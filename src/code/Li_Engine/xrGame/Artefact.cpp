...
void CArtefact::Load(LPCSTR section) {
    ...
    if (pSettings->section_exist(pSettings->r_string(section, "hit_absorbation_sect"))) {
        m_ArtefactHitImmunities.LoadImmunities(pSettings->r_string(section, "hit_absorbation_sect"),
                                               pSettings);
    }
...
void SArtefactDetectorsSupport::SetVisible(bool b) {
    ...
    if (b) {
        ...
        m_parent->CParticlesPlayer::StartParticles(curr, bone_id, Fvector().set(0, 1, 0),
                                                   m_parent->ID());
