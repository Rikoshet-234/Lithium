...
void CCar::SDoor::Init() {
    ...
    if (!joint)
        return;
    // R_ASSERT2(dJointGetType(joint->GetDJoint())==dJointTypeHinge,"Wrong door joint!!! Only simple
    // joint valid for a door and only one axis can be active, check other axes are zerro limited
    // !!!");
    R_ASSERT2(joint->IsHingeJoint(), "Wrong door joint!!! Only simple joint valid for a door and "
                                     "only one axis can be active, check other axes are zerro "
                                     "limited !!!");
    ...
    door_test.normalize();
    joint->PFirst_element()->get_Extensions(door_test, door_transform.c.dotproduct(door_test),
                                            lo_ext, hi_ext);
    if (hi_ext > -lo_ext)
        pos_open = -1.f;
    else
...
void CCar::SDoor::ClosedToOpening() {
    ...
    bone_instance.set_callback(bctPhysics, pcar->PPhysicsShell()->GetBonesCallback(),
...
void CCar::SDoor::GetExitPosition(Fvector& pos) {
    if (!joint) {
        ...
        MAX_OF(abs(pf.i.y), add.set(pf.i);
               add.mul(bb.m_halfsize.x * fsignum(pf.i.y)), abs(pf.j.y), add.set(pf.j);
...
bool CCar::SDoor::CanEnter(const Fvector& pos, const Fvector& dir, const Fvector& foot_pos) {
    // if(!joint) return true;//temp for fake doors
    return (state == opened || state == broken || !joint) && TestPass(foot_pos, dir) &&
           IsInArea(pos, dir); //
}
...
void CCar::SDoor::SDoorway::Init(SDoor* adoor) {
    ...
    if (_abs(door_axis_in_door.x) > _abs(door_axis_in_door.y)) {
        if (_abs(door_axis_in_door.x) > _abs(door_axis_in_door.z)) {
            ...
            door_plane_axes.x =
                1; // door_plane_axes.x stores door direction it may be j or k in this point
