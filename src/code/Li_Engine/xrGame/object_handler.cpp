...
void CObjectHandler::reinit(CAI_Stalker* object) {
    ...
    m_r_hand = kinematics->LL_BoneID(
        pSettings->r_string(*planner().m_object->cNameSect(), "weapon_bone0"));
    m_l_finger1 = kinematics->LL_BoneID(
        pSettings->r_string(*planner().m_object->cNameSect(), "weapon_bone1"));
    m_r_finger2 = kinematics->LL_BoneID(
        pSettings->r_string(*planner().m_object->cNameSect(), "weapon_bone2"));
...
BOOL CObjectHandler::net_Spawn(CSE_Abstract* DC) {
    ...
    m_infinite_ammo =
        !!trader->m_trader_flags.test(CSE_ALifeTraderAbstract::eTraderFlagInfiniteAmmo);
    return (TRUE);
}
...
void CObjectHandler::set_goal(MonsterSpace::EObjectAction object_action, CGameObject* game_object,
    ...
    planner().set_goal(object_action, game_object, min_queue_size, max_queue_size,
                       min_queue_interval, max_queue_interval);
}

void CObjectHandler::set_goal(MonsterSpace::EObjectAction object_action,
                              CInventoryItem* inventory_item, u32 min_queue_size,
                              u32 max_queue_size, u32 min_queue_interval, u32 max_queue_interval) {
    set_goal(object_action, inventory_item ? &inventory_item->object() : 0, min_queue_size,
...
void CObjectHandler::actualize_strap_mode(CWeapon* weapon) const {
    ...
    if (!planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped)) {
        //		Msg						( "[%6d][%s] actualizing:
        //weapon_strapped = false", Device.dwTimeGlobal, planner().object().cName().c_str() );
        weapon->strapped_mode(false);
        return;
    }
...
bool CObjectHandler::weapon_strapped(CWeapon* weapon) const {
    ...
    if (!weapon->can_be_strapped()) {
        //		Msg						( "[%6d][%s] weapon_strapped = false0",
        //Device.dwTimeGlobal, planner().object().cName().c_str() );
        return (false);
    }
    ...
    if (almost_strapped ||
        ...
        //		Msg						( "[%6d][%s] weapon_strapped = %s1",
        //Device.dwTimeGlobal, planner().object().cName().c_str(), almost_strapped &&
        //!planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" :
        //"false" );
...
bool CObjectHandler::weapon_unstrapped() const {
    CWeapon* weapon = smart_cast<CWeapon*>(inventory().ActiveItem());
    if (!weapon) {
        //		Msg						( "[%6d][%s] no active item!!(%d)(%s)",
        //Device.dwTimeGlobal, planner().object().cName().c_str(), inventory().GetActiveSlot(),
        //inventory().ItemFromSlot(3) ? inventory().ItemFromSlot(3)->object().cName().c_str() :
        //"<no_item>" );
        return (true);
    }
...
bool CObjectHandler::weapon_unstrapped(CWeapon* weapon) const {
    ...
    if (!weapon->can_be_strapped()) {
        ...
        //true0", Device.dwTimeGlobal, planner().object().cName().c_str() );
        return (true);
    }
    ...
    if (almost_unstrapped ||
        ...
        //		Msg						( "[%6d][%s] weapon_unstrapped = %s1",
        //Device.dwTimeGlobal, planner().object().cName().c_str(), almost_unstrapped &&
        //!planner().m_storage.property(ObjectHandlerSpace::eWorldPropertyStrapped2Idle) ? "true" :
        //"false" );
...
void CObjectHandler::set_inertia(const CWeapon& weapon, const u32& action_id,
