...
IC bool b_is_blending(const CBlend* current_blend, const CBlend* b) {
    ...
    return current_blend && current_blend->blend_state() != CBlend::eFREE_SLOT &&
...
void ik_anim_state::update(IKinematicsAnimated* K, const CBlend* b, u16 i) {
    ...
    if (b_is_blending(current_blend, b)) {
        ...
        bool is_cur_step =
            (m_def_cur.marks.size() > i) && blend_in(*current_blend, m_def_cur.marks[i]);
...
bool ik_anim_state::time_step_begin(IKinematicsAnimated* K, const CBlend& B, u16 limb_id,
                                    float& time) {
    time = 0;
