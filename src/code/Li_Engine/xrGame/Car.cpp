...
CCar::CCar() {
    ...
    active_camera = 0;
    camera[ectFirst] =
        xr_new<CCameraFirstEye>(this, CCameraBase::flRelativeLink | CCameraBase::flPositionRigid);
...
void CCar::ActorObstacleCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                                 SGameMtl* material_2) {
    if (!do_colide) {
...
void CCar::SpawnInitPhysics(CSE_Abstract* D) {
    ...
    Init(); // inits m_driving_wheels,m_steering_wheels,m_breaking_wheels values using recieved in
            // ParceDefinitions & from bone_map
...
// void CCar::Hit(float P,Fvector &dir,CObject * who,s16 element,Fvector p_in_object_space, float
...
void CCar::ParseDefinitions() {
    ...
    m_power_increment_factor = READ_IF_EXISTS(ini, r_float, "car_definition",
    ...
    m_power_decrement_factor = READ_IF_EXISTS(ini, r_float, "car_definition",
...
void CCar::Init() {
    ...
    float l_time_to_explosion =
        READ_IF_EXISTS(ini, r_float, "car_definition", "time_to_explosion", 120.f);
    ...
    if (ini->section_exist("damage_items")) {
        ...
        for (auto I = data.Data.cbegin(); I != data.Data.cend(); I++) {
            ...
            if (i != m_wheels_map.end())
                ...
                R_ASSERT3(i != m_doors.end(), "only wheel and doors bones allowed for damage defs",
                          *item.first);
...
bool CCar::Use(const Fvector& pos, const Fvector& dir, const Fvector& foot_pos) {
    ...
    RQR.r_clear();
    collide::ray_defs Q(pos, dir, 3.f, CDB::OPT_CULL,
...
IC void CCar::fill_wheel_vector(LPCSTR S, xr_vector<T>& type_wheels) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (J == bone_map.end()) {
            ...
            SWheel& wheel =
                (m_wheels_map.insert(std::make_pair(bone_id, SWheel(this)))).first->second;
...
float CCar::RefWheelCurTorque() {
    if (b_transmission_switching)
        return 0.f;
    return EngineCurTorque() *
           ((m_current_gear_ratio < 0.f) ? -m_current_gear_ratio : m_current_gear_ratio);
}
