...
CInventoryItem::~CInventoryItem() {
...
#ifndef MASTER_GOLD
    bool B_GOOD =
        (!m_pInventory || (std::find(m_pInventory->m_all.begin(), m_pInventory->m_all.end(),
    ...
    if (!B_GOOD) {
        ...
        Msg("! ERROR item_id[%d] H_Parent=[%s][%d] [%d]", object().ID(),
            p ? p->cName().c_str() : "none", p ? p->ID() : -1, Device.dwFrame);
    }
...
"[good]"); string256		temp; strconcat		(temp,*m_nameComplex," ",cond)	;
...
void CInventoryItem::OnEvent(NET_Packet& P, u16 type) {
    ...
    case GE_ADDON_ATTACH: {
        u16 ItemID;
        P.r_u16(ItemID);
        CInventoryItem* ItemToAttach =
            smart_cast<CInventoryItem*>(Level().Objects.net_Find(ItemID));
        if (!ItemToAttach)
            break;
...
void CInventoryItem::net_Destroy() {
    if (m_pInventory) {
        VERIFY(std::find(m_pInventory->m_all.begin(), m_pInventory->m_all.end(), this) ==
...
void CInventoryItem::net_Import(NET_Packet& P) {
    ...
    CSE_ALifeInventoryItem::inventory_item_angular_null)) { N.State.angular_vel.x	=
    P.r_float_q8(0.f,10.f*PI_MUL_2); N.State.angular_vel.y	= P.r_float_q8(0.f,10.f*PI_MUL_2);
...
void CInventoryItem::net_Import_PH_Params(NET_Packet& P, net_update_IItem& N,
                                          mask_inv_num_items& num_items) {
...
void CInventoryItem::net_Export_PH_Params(NET_Packet& P, SPHNetState& State,
                                          mask_inv_num_items& num_items) {
    // UI().Font().pFontStat->OutSet(100.0f,100.0f);
...
void CInventoryItem::net_Export(NET_Packet& P) {
    ...
    CSE_ALifeInventoryItem::inventory_item_state_enabled; if
...
void CInventoryItem::PH_I_CrPr() // actions & operations between two phisic prediction steps
    {
      ...
      ///////////////////////////////////////////////
      Fmatrix xformX;
      pSyncObj->cv2obj_Xfrom(p->RecalculatedState.quaternion, p->RecalculatedState.position,
      xformX);
...
void CInventoryItem::PH_A_CrPr() {
    if (m_just_after_spawn) {
        ...
        VERIFY(K);
        if (!object().PPhysicsShell()) {
            Msg("! ERROR: PhysicsShell is NULL, object [%s][%d]", object().cName().c_str(),
                object().ID());
...
void CInventoryItem::Interpolate() {
    ...
    if (!object().H_Parent() && object().getVisible() && object().m_pPhysicsShell && !OnServer() &&
        p->NET_IItem.size()) {
        SPHNetState newState = p->NET_IItem.front().State;

        if (p->NET_IItem.size() >= 2) {

            float ret_interpolate =
                interpolate_states(p->NET_IItem.front(), p->NET_IItem.back(), newState);
...
float CInventoryItem::interpolate_states(net_update_IItem const& first,
                                         net_update_IItem const& last, SPHNetState& current) {
    float ret_val = 0.f;
    ...
    current.position.x =
        first.State.position.x + (factor * (last.State.position.x - first.State.position.x));
    current.position.y =
        first.State.position.y + (factor * (last.State.position.y - first.State.position.y));
    current.position.z =
        first.State.position.z + (factor * (last.State.position.z - first.State.position.z));
...
void CInventoryItem::reload(LPCSTR section) {
    ...
    m_holder_range_modifier =
        READ_IF_EXISTS(pSettings, r_float, section, "holder_range_modifier", 1.f);
...
void CInventoryItem::OnRender() {
    if (bDebug && object().Visual()) {
        ...
        (Position(), size, size, size, color_rgba(0, 255, 0, 255));
                                ...
                                {
                                        Pos2 = *It;
                                        Level().debug_renderer().draw_line(Fidentity, Pos1, Pos2,
        color_rgba(255, 255, 0, 255)); Pos1 = Pos2;
                                };
                        ...
                        if (OnClient() && !H_Parent() && m_bInInterpolation)
                        {
                                ...
                                Level().debug_renderer().draw_aabb			(IRecPos,
        size, size, size, color_rgba(255, 0, 255, 255));
        ...
        c*(c*(c*SCoeff[k][0]+SCoeff[k][1])+SCoeff[k][2])+SCoeff[k][3];
                                        ...
                                        Level().debug_renderer().draw_line(Fidentity, point0,
        point1, color_rgba(0, 0, 255, 255)); point0.set(point1);
                                };
                        };
                        */
    };
...
void CInventoryItem::SetDropManual(BOOL val) {
    ...
    if (!IsGameTypeSingle()) {
        if (!!m_name) {
            Msg("! WARNING: trying to set drop manual flag to item [%d][%s] to %d", object_id(),
                m_name.c_str(), val);
        }
    }
#endif // #ifdef DEBUG
