...
BOOL CLevel::Load_GameSpecific_Before() {
	...
	g_pGamePersistent->LoadTitleOld("st_loading_ai_objects");
...
BOOL CLevel::Load_GameSpecific_After() {
    ...
    if (FS.exist(fn_game, "$level$", "level.ps_static")) {
        ...
        for (IReader* OBJ = F->open_chunk_iterator(chunk); OBJ;
            ...
            if ((g_pGamePersistent->m_game_params.m_e_game_type & EGameIDs(gametype_usage)) ||
                (ver == 0)) {
...
void CLevel::Load_GameSpecific_CFORM(CDB::TRI* tris, u32 count) {
    ...
    if (static_mtl_count < 128) {
        ...
        for (; I != E; ++I) {
            ID_INDEX_PAIRS::iterator i =
                std::find(translator.begin(), translator.end(), (u16)(*I).material);
            if (i != translator.end()) {
    ...
    std::sort(translator.begin(), translator.end());
    {
        ...
        for (; I != E; ++I) {
            ID_INDEX_PAIRS::iterator i =
                std::lower_bound(translator.begin(), translator.end(), (u16)(*I).material);
