...
CBlend* PlayMotionByParts(IKinematicsAnimated* sa, MotionID motion_ID, BOOL bMixIn,
...
void CActor::Spin0Callback(CBoneInstance* B) {
    ...
    Fmatrix spin;
    float bone_yaw =
        angle_normalize_signed(A->r_torso.yaw - A->r_model_yaw - A->r_model_yaw_delta) *
        y_spin0_factor;
...
void STorsoWpn::Create(IKinematicsAnimated* K, LPCSTR base0, LPCSTR base1) {
    ...
    moving[eSprint] =
        K->ID_Cycle_Safe(strconcat(sizeof(buf), buf, base0, "_torso", base1, "_escape_0"));
    ...
    attack_zoom =
        K->ID_Cycle_Safe(strconcat(sizeof(buf), buf, base0, "_torso", base1, "_attack_0"));
...
void SVehicleAnimCollection::Create(IKinematicsAnimated* V, u16 num) {
    ...
    for (int i = 0; MAX_IDLES > i; ++i) {
        idles[i] = V->ID_Cycle_Safe(
            strconcat(sizeof(buf), buf, "steering_idle_", buff1, itoa(i, buff2, 10)));
        if (idles[i])
            idles_num++;
        else
            break;
...
void CActor::steer_Vehicle(float angle) {
    ...
    /*
            ...
            if(angle==0.f) 		smart_cast<IKinematicsAnimated*>
       (Visual())->PlayCycle(anims.idles[0]);
            else if(angle>0.f)	smart_cast<IKinematicsAnimated*>
       (Visual())->PlayCycle(anims.steer_right); else
...
void CActor::g_SetAnimation(u32 mstate_rl) {
    ...
    if (!M_legs) {
        if ((mstate_rl & mcCrouch) &&
            !isActorAccelerated(mstate_rl, IsZoomAimingMode())) //!(mstate_rl&mcAccel))
        {
    ...
    if ((Level().CurrentControlEntity() == this) && g_ShowAnimationInfo) {
        ...
        /*
                        ...
                        {
                        case CPHMovementControl::peOnGround:	xr_strcpy(buf,"ground");
           break; case CPHMovementControl::peInAir:		xr_strcpy(buf,"air");
           break; case CPHMovementControl::peAtWall:		xr_strcpy(buf,"wall");
           break;
                        }
                        UI().Font().pFontStat->OutNext	(buf);
                        UI().Font().pFontStat->OutNext	("Accel     [%3.2f, %3.2f,
           %3.2f]",VPUSH(NET_SavedAccel)); UI().Font().pFontStat->OutNext	("V         [%3.2f,
