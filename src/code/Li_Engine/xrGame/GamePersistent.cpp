...
//{ return Memory.mem_alloc(size,"ODE");			} 	static	void *
//ode_realloc	(void *ptr, size_t oldsize, size_t newsize)	{ return
//Memory.mem_realloc(ptr,newsize,"ODE");	} 	static	void	ode_free	(void
//*ptr, size_t size)					{ return xr_free(ptr);
//} #else // DEBUG_MEMORY_MANAGER 	static	void *	ode_alloc	(size_t size)
//{ return xr_malloc(size);			} 	static	void *	ode_realloc
//(void *ptr, size_t oldsize, size_t newsize)	{ return xr_realloc(ptr,newsize);	}
//	static	void	ode_free	(void *ptr, size_t size)					{
//return xr_free(ptr);				} #endif // DEBUG_MEMORY_MANAGER
...
void CGamePersistent::WeathersUpdate() {
    if (g_pGameLevel) {
        ...
        if (env_amb) {
            ...
            for (u32 idx = 0; I != E; ++I, ++idx) {
                ...
                if (ambient_sound_next_time[idx] == 0) // first
                {
                    ...
                    ambient_sound_next_time[idx] =
                        Device.dwTimeGlobal + _length_ms + ch.get_rnd_sound_time();
            ...
            /*
                                    ...
                                    {
                                            ref_sound* snd			=
               env_amb->get_rnd_sound(); ambient_sound_next_time	= Device.dwTimeGlobal +
               env_amb->get_rnd_sound_time(); if (snd)
                                            {
                                                    Fvector	pos;
                                                    float	angle		=
               ::Random.randF(PI_MUL_2); pos.x				= _cos(angle);
                                                    pos.y				= 0;
                                                    pos.z				=
               _sin(angle); pos.normalize
               ().mul(env_amb->get_rnd_sound_dist()).add(Device.vCameraPosition); pos.y
               += 10.f; snd->play_at_pos	(0,pos);
                                            }
                                    }
            */
            // start effect
        ...
        if (Device.fTimeGlobal >= ambient_effect_wind_start &&
            Device.fTimeGlobal <= ambient_effect_wind_in_time && ambient_effect_wind_on) {
...
void CGamePersistent::OnFrame() {
        ...
        if (Level().CurrentViewEntity() && IsGameTypeSingle()) {
            if (!g_actor || (g_actor->ID() != Level().CurrentViewEntity()->ID())) {
                CCustomMonster* custom_monster =
                    smart_cast<CCustomMonster*>(Level().CurrentViewEntity());
    ...
    if (0 != pDemoFile) {
        if (Device.dwTimeGlobal > uTime2Change) {
            ...
            Engine.Event.Defer("KERNEL:start", size_t(xr_strdup(_Trim(o_server))),
...
void CGamePersistent::LoadTitleOld(LPCSTR str)
{
	string512			buff;
	sprintf_s(buff, "%s", CStringTable().translate(str).c_str());
	pApp->LoadTitleIntOld(buff);
}
...
void CGamePersistent::UpdateDof() {
    ...
    m_dof[1].add(diff);
    (m_dof[0].x < m_dof[2].x) ? clamp(m_dof[1].x, m_dof[0].x, m_dof[2].x)
                              : clamp(m_dof[1].x, m_dof[2].x, m_dof[0].x);
    (m_dof[0].y < m_dof[2].y) ? clamp(m_dof[1].y, m_dof[0].y, m_dof[2].y)
