...
BOOL CHangingLamp::net_Spawn(CSE_Abstract* DC) {
    ...
    //	R_ASSERT3				(pUserData,"Empty HangingLamp user
    //data!",lamp->get_visual());
    ...
    //.	if (lamp->flags.is(CSE_ALifeObjectHangingLamp::flPhysic)&&Visual()&&!guid_physic_bone)
    //fHealth=0.f;
    if (Alive() && m_bState)
        TurnOn();
    else {
...
void CHangingLamp::TurnOff() {
    ...
    if (!PPhysicsShell()) // if we have physiccs_shell it will call processing deactivate when
                          // disable
...
//					   Fvector p_in_object_space, float impulse, ALife::EHitType
//hit_type)
void CHangingLamp::Hit(SHit* pHDS) {
    ...
    if (m_pPhysicsShell)
        m_pPhysicsShell->applyHit(pHDS->p_in_bone_space, pHDS->dir, pHDS->impulse, pHDS->boneID,
                                  pHDS->hit_type);
