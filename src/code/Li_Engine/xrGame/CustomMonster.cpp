...
void CCustomMonster::shedule_Update(u32 DT) {
    ...
    if (Remote()) {
        ...
        if (temp > 0) {
            ...
            // R = Radius();
            //////////////////////////////////////
            /// #pragma todo("Oles to all AI guys: perf/logical problem: Only few objects needs
            /// 'feel_touch' why to call update for everybody?")
...
void CCustomMonster::net_update::lerp(CCustomMonster::net_update& A, CCustomMonster::net_update& B,
                                      float f) {
    //
...
void CCustomMonster::UpdateCL() {
    ...
    m_client_update_delta =
        (u32)std::min(Device.dwTimeGlobal - m_last_client_update_time, u32(100));
    m_last_client_update_time = Device.dwTimeGlobal;
    ...
    */
    ...
    if (g_mt_config.test(mtSoundPlayer))
        Device.seqParallel.push_back(
            fastdelegate::FastDelegate0<>(this, &CCustomMonster::update_sound_player));
    else {
...
void CCustomMonster::UpdateCamera() {
    ...
    if (g_Alive())
        update_range_fov(new_range, new_fov,
                         memory().visual().current_state().m_max_view_distance * eye_range,
                         eye_fov);
    g_pGameLevel->Cameras().Update(eye_matrix.c, eye_matrix.k, eye_matrix.j, new_fov, .75f,
                                   new_range, 0);
}
...
BOOL CCustomMonster::net_Spawn(CSE_Abstract* DC) {
    ...
    if (ai().get_level_graph() && UsedAI_Locations() && (e->ID_Parent == 0xffff)) {
        ...
        if (ai().game_graph().valid_vertex_id(E->m_tNextGraphID) &&
            ...
            movement().restrictions().accessible(
                ai().game_graph().vertex(E->m_tNextGraphID)->level_vertex_id()))
    ...
    // Eyes
    eye_bone = smart_cast<IKinematics*>(Visual())->LL_BoneID(
        pSettings->r_string(cNameSect(), "bone_head"));
...
// void CCustomMonster::Hit(float P, Fvector &dir,CObject* who, s16 element,Fvector
...
bool CCustomMonster::is_special_killer(CObject* obj) {
    return (obj && (std::find(m_killer_clsids.begin(), m_killer_clsids.end(), obj->CLS_ID) !=
                    m_killer_clsids.end()));
}
...
void CCustomMonster::feel_sound_new(CObject* who, int type, CSound_UserDataPtr user_data,
...
bool CCustomMonster::update_critical_wounded(const u16& bone_id, const float& power) {
    ...
    float time_delta =
        m_last_hit_time ? float(Device.dwTimeGlobal - m_last_hit_time) / 1000.f : 0.f;
...
void CCustomMonster::OnRender() {
...

    {
        ...
        for (; i != e; ++i)
            Level().debug_renderer().draw_aabb(ai().level_graph().vertex_position(*i), radius,
                                               radius, radius, D3DCOLOR_XRGB(255, 22, 255));
    }
    ...
    for (int i = 0; i < 1; ++i) {
        ...
        float radius1 = !i ? .2f : .3f;
        {
            ...
            for (u32 I = 1; I < keys.size(); ++I) {
                ...
                temp = keys[I - 1];
                Fvector P1;
                P1.set(temp.position.x, ai().level_graph().vertex_plane_y(temp.vertex_id),
                       temp.position.y);
                P1.y += 0.1f;

                temp = keys[I];
                Fvector P2;
                P2.set(temp.position.x, ai().level_graph().vertex_plane_y(temp.vertex_id),
                       temp.position.y);
                P2.y += 0.1f;
    ...
    for (; I != E; ++I) {
        ...
        Level().debug_renderer().draw_line(Fidentity, traj_start, traj_start + z_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));
        Level().debug_renderer().draw_line(Fidentity, traj_end, traj_end - z_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));

        for (u32 i = 0; i < sizeof(z_offs) / sizeof(z_offs[0]); ++i)
            Level().debug_renderer().draw_line(Fidentity, traj_start + z_offs[i],
                                               traj_end + z_offs[i], D3DCOLOR_XRGB(255, 255, 128));
        ...
        Level().debug_renderer().draw_line(Fidentity, hor_start, hor_start + x_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));
        Level().debug_renderer().draw_line(Fidentity, hor_end, hor_end - x_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));

        for (u32 i = 0; i < sizeof(x_offs) / sizeof(x_offs[0]); ++i)
            Level().debug_renderer().draw_line(Fidentity, hor_start + x_offs[i],
                                               hor_end + x_offs[i], D3DCOLOR_XRGB(255, 255, 128));
        ...
        Level().debug_renderer().draw_line(Fidentity, ver_start, ver_start + y_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));
        Level().debug_renderer().draw_line(Fidentity, ver_end, ver_end - y_normal,
                                           D3DCOLOR_XRGB(128, 255, 128));

        for (u32 i = 0; i < sizeof(y_offs) / sizeof(y_offs[0]); ++i)
            Level().debug_renderer().draw_line(Fidentity, ver_start + y_offs[i],
                                               ver_end + y_offs[i], D3DCOLOR_XRGB(255, 255, 128));
