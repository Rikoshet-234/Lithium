...
IC void play_cycle(IKinematicsAnimated* CA, const MotionID& m, u8 channel, CBlend*& blend_block,
                   float base_power) {
...
void character_hit_animation_controller::PlayHitMotion(const Fvector& dir, const Fvector& bone_pos,
                                                       u16 bi, CEntityAlive& ea) const {
    ...
    float rotational_ammount = hit_point.magnitude() * g_params.power_factor *
                               g_params.rotational_power_factor; //_abs(torqu.x)
