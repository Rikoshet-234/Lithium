...
//								(для просчета столкновений и их
//визуализации)
//////////////////////////////////////////////////////////////////////
        ...
        if (entity && entity->g_Alive() && (entity->ID() != bullet->parent_id)) {
            ...
            if ((NULL != cform) && (cftObject == cform->Type())) {
                ...
                if (actor && IsGameTypeSingle() /**/ || stalker /**/) {
                    ...
                    if (Fsphere::rpNone !=
                        S.intersect_full(bullet->bullet_pos, bullet->dir, dist)) {
                        ...
                        if (actor) {
                            ...
                            if (weapon_object) {
                                ...
                                if (weapon) {
                                    ...
                                    float fly_dist = bullet->fly_dist + dist;
                                    dist_factor = std::min(
                                        1.f, fly_dist / Level().BulletManager().m_fHPMaxDist);
                                }
                            }
                            ...
                            // если стрелял сталкер, учитываем - hit_probability_factor сталкерa
                            // иначе - 1.0
                            if (i_stalker) {
                                hpf = i_stalker->SpecificCharacter().hit_probability_factor();
                                float fly_dist = bullet->fly_dist + dist;
                                float dist_factor =
                                    _min(1.f, fly_dist / Level().BulletManager().m_fHPMaxDist);
...
void CBulletManager::FireShotmark(SBullet* bullet, const Fvector& vDir, const Fvector& vEnd,
                                  collide::rq_result& R, u16 target_material,
                                  const Fvector& vNormal, bool ShowMark) {
    ...
    if (R.O) {
        ...
        */
        ...
        if (mtl_pair && !mtl_pair->m_pCollideMarks->empty() && ShowMark) {
            //добавить отметку на материале
            ::Render->add_StaticWallmark(&*mtl_pair->m_pCollideMarks, vEnd, bullet->wallmark_size,
                                         pTri, pVerts);
        }
    }

    ref_sound* pSound =
        (!mtl_pair || mtl_pair->CollideSounds.empty())
            ? NULL
    ...
    LPCSTR ps_name =
        (!mtl_pair || mtl_pair->CollideParticles.empty())
            ? NULL
    ...
    if ((ps_name && ShowMark) || (bullet->flags.explosive && bStatic)) {
        VERIFY2((particle_dir.x * particle_dir.x + particle_dir.y * particle_dir.y +
...
void CBulletManager::StaticObjectHit(CBulletManager::_event& E) {
    ...
    //	ObjectHit	(&E.bullet,					E.point, E.R, E.tgt_material,
    //hit_normal);
}
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event& E) {
    ...
    if (V) {
        VERIFY3(V->LL_GetBoneVisible(u16(E.R.element)), *E.R.O->cNameVisual(),
    ...
    if (E.bullet.flags.allow_sendhit && !E.Repeated) {
        ...
        if (GameID() != eGameIDSingle && E.bullet.flags.allow_sendhit &&
            smart_cast<CActor*>(E.R.O) && Game().m_WeaponUsageStatistic->CollectData()) {
            CActor* pActor = smart_cast<CActor*>(E.R.O);
            if (pActor) // && pActor->g_Alive())
            {
                Game().m_WeaponUsageStatistic->OnBullet_Hit(&E.bullet, E.R.O->ID(),
                                                            (s16)E.R.element, E.point);
                AddStatistic = true;
            };
        };

        SHit Hit = SHit(hit_param.power, original_dir, NULL, u16(E.R.element),
...
bool CBulletManager::ObjectHit(SBullet_Hit* hit_res, SBullet* bullet, const Fvector& end_point,
    ...
    if (R.O) {
        ...
        if (DOT(hit_normal, bullet->dir) < 0) {
            if (bullet->density_mode) {
                //				Log("WARNING: Material in material found while bullet
                //tracing. Incorrect behaviour of shooting is possible.");
            }
    ...
    *hit_res = bullet->hit_param; // default param
    ...
    if ((f < ricoshet_factor) && !mtl->Flags.test(SGameMtl::flNoRicoshet) &&
