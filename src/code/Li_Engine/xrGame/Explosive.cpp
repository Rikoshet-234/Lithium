...
const float exp_dist_extinction_factor = 3.f; //(>1.f, 1.f -means no dist change of exp effect)
                                              //on the dist of m_fBlastRadius exp. wave effect in
...
void CExplosive::Load(CInifile const* ini, LPCSTR section) {
    ...
    sscanf(ini->r_string(section, "light_color"), "%f,%f,%f", &m_LightColor.r, &m_LightColor.g,
           &m_LightColor.b);
...
float CExplosive::ExplosionEffect(collide::rq_results& storage, CExplosive* exp_obj,
                                  CPhysicsShellHolder* blasted_obj, const Fvector& expl_centre,
...
float CExplosive::TestPassEffect(const Fvector& source_p, const Fvector& dir, float range,
                                 float ef_radius, collide::rq_results& storage,
                                 CObject* blasted_obj) {
    float sq_ef_radius = ef_radius * ef_radius;
    float dist_factor =
        sq_ef_radius / (range * range * (exp_dist_extinction_factor - 1.f) + sq_ef_radius);
    ...
    if (range > EPS_L) {
        ...
        SExpQParams ep;
#endif
        g_pGameLevel->ObjectSpace.RayQuery(storage, RD, grenade_hit_callback, &ep, NULL,
                                           blasted_obj);
...
void CExplosive::Explode() {
    ...
    for (int i = 0; i < m_iFragsNum; ++i) {
        ...
        Level().BulletManager().AddBullet(pos, frag_dir, m_fFragmentSpeed, m_fFragHit,
                                          m_fFragHitImpulse, Initiator(), cast_game_object()->ID(),
    ...
    if (m_fExplodeDuration < 0.f && m_blasted_objects.empty()) {
        ...
        if (m_pLight && m_pLight->get_active() && m_fLightTime > 0) {
            if (m_fExplodeDuration > (m_fExplodeDurationMax - m_fLightTime)) {
                float scale =
                    (m_fExplodeDuration - (m_fExplodeDurationMax - m_fLightTime)) / m_fLightTime;
...
void CExplosive::OnAfterExplosion() {
    ...
    //	//		Msg					("ge_destroy: [%d] -
    //%s",ID(),*cName()); 	if (cast_game_object()->Local()) cast_game_object()->u_EventSend
    //(P);
}
void CExplosive::OnBeforeExplosion() {
    ...
    if (m_bHideInExplosion) {
        HideExplosive();
        //	Msg("---------CExplosive OnBeforeExplosion setVisible(false) [%d]
        //frame[%d]",cast_game_object()->ID(), Device.dwFrame);
    }
}
...
void CExplosive::ExplodeWaveProcessObject(collide::rq_results& storage,
    ...
    if (l_pGO->Visual())
        ...
        return; //мне непонятно зачем наносить хит от взрыва по объектам не имеющим вижуал - поэтому
                //игнорируем
    ...
    if (l_impuls > .001f || l_hit > 0.001) {
        ...
        HS.weaponID = cast_game_object()
                          ->ID();     //		P.w_u16			(cast_game_object()->ID());
        ...
        HS.hit_type =
            (m_eHitTypeBlast); //		P.w_u16			(u16(m_eHitTypeBlast));
...
void CExplosive::net_Relcase(CObject* O) {
    ...
    auto I = std::find(m_blasted_objects.begin(), m_blasted_objects.end(),
