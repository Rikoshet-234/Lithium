...
void ui_core::OnDeviceReset() {
    ...
    m_2DFrustum.CreateFromRect(
        Frect().set(0.0f, 0.0f, float(Device.dwWidth), float(Device.dwHeight)));
}
...
void ui_core::PushScissor(const Frect& r_tgt, bool overlapped) {
    ...
    if (!(result.x1 >= 0 && result.y1 >= 0 && result.x2 <= UI_BASE_WIDTH &&
        ...
        VERIFY(result.x1 >= 0 && result.y1 >= 0 && result.x2 <= UI_BASE_WIDTH &&
               result.y2 <= UI_BASE_HEIGHT);
    }
...
#ifdef DEBUG
        LogMsg("[16-9] get_xml_name for[{0}] returns [{1}]", fn, writer.str());
#endif // #ifdef DEBUG
    }
