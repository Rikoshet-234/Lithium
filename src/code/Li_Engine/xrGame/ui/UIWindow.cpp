...
CUIWindow::CUIWindow()
    : m_pParentWnd(NULL), m_pMouseCapturer(NULL), m_pMessageTarget(NULL), m_pKeyboardCapturer(NULL),
      m_bAutoDelete(false), m_bCursorOverWindow(false), m_bPP(false), m_dwFocusReceiveTime(0),
      m_bCustomDraw(false) {
    Show(true);
    Enable(true);
#ifdef LOG_ALL_WNDS
...
bool CUIWindow::IsChild(CUIWindow* pPossibleChild) const {
    WINDOW_LIST::const_iterator it =
        std::find(m_ChildWndList.begin(), m_ChildWndList.end(), pPossibleChild);
...
bool fit_in_rect(CUIWindow* w, Frect const& vis_rect, float border,
