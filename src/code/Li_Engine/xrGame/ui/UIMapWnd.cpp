...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from) {
    ...
    if (m_scroll_mode) {
        ...
        AddCallback(m_UIMainScrollH, SCROLLBAR_HSCROLL,
        ...
        AddCallback(m_UIMainScrollV, SCROLLBAR_VSCROLL,
...
bool CUIMapWnd::OnMouseAction(float x, float y, EUIMessages mouse_action) {
    if (inherited::OnMouseAction(
            x, y, mouse_action) /*|| m_btn_nav_parent->OnMouseAction(x,y,mouse_action)*/) {
        return true;
    }
