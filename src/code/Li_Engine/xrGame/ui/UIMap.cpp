...
Fvector2
CUICustomMap::ConvertRealToLocal(const Fvector2& src,
...
bool CUICustomMap::GetPointerTo(const Fvector2& src, float item_radius, Fvector2& pos,
                                float& heading) {
...
Fvector2
CUIGlobalMap::ConvertRealToLocal(const Fvector2& src,
...
float CUIGlobalMap::CalcOpenRect(const Fvector2& center_point, Frect& map_desired_rect,
                                 float tgt_zoom) {
...
void CUIMiniMap::Draw() {
    ...
    for (u32 idx = 0; idx < segments_count - 2; ++idx) {
        ...
        UIRender->PushPoint(S[idx + 2].pt.x, S[idx + 2].pt.y, 0, color, S[idx + 2].uv.x,
                            S[idx + 2].uv.y);
        UIRender->PushPoint(S[idx + 1].pt.x, S[idx + 1].pt.y, 0, color, S[idx + 1].uv.x,
                            S[idx + 1].uv.y);
    }
...
bool CUIMiniMap::GetPointerTo(const Fvector2& src, float item_radius, Fvector2& pos,
                              float& heading) {
