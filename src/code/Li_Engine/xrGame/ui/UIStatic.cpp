...
CUIStatic::CUIStatic()
    : m_bTextureEnable(true), m_bStretchTexture(false), m_bHeading(false), m_bConstHeading(false),
      m_fHeading(0.0f), m_pTextControl(NULL) {
...
void CUIStatic::Update() {
    ...
    if (m_lanim_xform.m_lanim) {
        ...
        if (m_lanim_xform.m_lanimFlags.test(LA_CYCLIC) ||
            ...
            int frame;
            u32 clr =
                m_lanim_xform.m_lanim->CalculateRGB(t - m_lanim_xform.m_lanim_start_time, frame);
...
void CUIStatic::ColorAnimationSetTextColor(u32 color, bool only_alpha) {
    TextItemControl()->SetTextColor(
        (only_alpha) ? subst_alpha(TextItemControl()->GetTextColor(), color) : color);
}
