...
void Weapon_Statistic::net_save(NET_Packet* P, victims_table const& vt, bone_table const& bt) {
    ...
    public:
        CompleteFilter(u32& hits_count_result, NET_Packet* P, victims_table const& vt,
                       bone_table const& bt)
...
bool bone_table::add_bone(shared_str const& bone_name, s16 bone_id) {
    ...
    bone_table_t::const_iterator tmp_iter =
        std::find_if(m_data.begin(), m_data.end(), bone_name_searcher(bone_name));
    if (tmp_iter != m_data.end())
        return false;
...
void Player_Statistic::net_save(NET_Packet* P) {
    ...
    bone_table::bone_table_t bt_storage(_alloca(sizeof(bone_table::bone_table_t::value_type) * 65),
                                        65);
...
void WeaponUsageStatistic::OnBullet_Fire(SBullet* pBullet, const CCartridge& cartridge) {
    ...
    //-----------------------------------------------------------------------------------
    ActiveBullets.push_back(
        BulletData(object_parent->cName(), object_weapon->cNameSect(), pBullet));

    //	Msg("! OnBullet Fire ID[%d]", pBullet->m_dwID);
}

void WeaponUsageStatistic::OnBullet_Hit(SBullet* pBullet, u16 TargetID, s16 element,
                                        Fvector HitLocation) {
    std::lock_guard<decltype(m_mutex)> syncg(m_mutex);
    if (!pBullet || !pBullet->flags.allow_sendhit)
        return;
...
void WeaponUsageStatistic::Send_Check_Respond() {
    ...
    for (u32 i = 0; i < m_Requests.size(); i++) {
        ...
        //		Msg("%d-%d || %d-%d", NumFalse, BChA_Request.NumFalse, NumTrue,
        //BChA_Request.NumTrue);
...
void WeaponUsageStatistic::OnExplosionKill(game_PlayerState* ps, const SHit& hit) {
    ...
    WEAPON_STATS_it WeaponIt =
        PlayerStatKiller.FindPlayersWeapon(weapon_object->cNameSect().c_str());
    ++WeaponIt->m_dwHitsScored;
    ++WeaponIt->m_dwKillsScored;
...
void WeaponUsageStatistic::OnBleedKill(game_PlayerState* killer_ps, game_PlayerState* victim_ps,
                                       u16 weapon_id) {
    std::lock_guard<decltype(m_mutex)> syncg(m_mutex);
...
u8 WeaponUsageStatistic::ConvertToTeamIndex(s16) { return 1; }
...
void WeaponUsageStatistic::SVUpdateAliveTimes() {
    ...
    for (u32 player_index = 0, max_players = aPlayersStatistic.size(); player_index < max_players;
         ++player_index) {
