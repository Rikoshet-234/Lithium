...
void interactive_motion_diagnostic(LPCSTR message, const MotionID& m, CPhysicsShell* s) {
    ...
    Msg("death anims - interactive_motion:- %s, motion: %s, obj: %s, model:  %s ", message,
        motion_name, O->cName().c_str(), O->cNameVisual().c_str());
#endif
}
...
void interactive_motion::setup(const MotionID& m, CPhysicsShell* s, float _angle) {
    ...
    VERIFY2(MD->StopAtEnd(), make_string("can not use cyclic anim in death animth motion: %s",
...
void interactive_motion::switch_to_free() {
    ...
    CPhysicsShellHolder* obj =
        smart_cast<CPhysicsShellHolder*>(shell->get_ElementByStoreOrder(0)->PhysicsRefObject());
    VERIFY(obj);
