...
#include "phdebug.h"
#endif
moving_bones_snd_player::moving_bones_snd_player(IKinematics* K, CInifile* ini, LPCSTR section,
                                                 const Fmatrix& object)
    : bone_id(BI_NONE), min_factor(-FLT_MAX), max_factor(-FLT_MAX), base_velocity(-FLT_MAX),
      smothed_velocity(-FLT_MAX), previous_position(Fidentity), kinematics(K) {
    VERIFY(K);
    VERIFY(ini);
...
void moving_bones_snd_player::update(float time_delta, CGameObject& object) {
    ...
    if (dbg_moving_bones_snd_player) {
        DBG_OutText(
            "smoothed angular speed							 : %f",
            smothed_velocity);
        // DBG_OutText( "angular speed         : %f",  aw_speed );
        DBG_OutText("velocity factor = base_speed/smoothed speed=       : %f", factor);
        DBG_OutText("min_factor=								"
                    "		 : %f",
                    min_factor);
        DBG_OutText("max_factor=								"
                    "		 : %f",
                    max_factor);
        DBG_OutText("frequency_factor=								"
                    "	 : %f",
                    frequency_factor);
        // DBG_OutText( "time delta            : %f", time_delta );
        // Msg( "smoothed angular speed: %f, angular speed: %f, factor: %f time delta: %f ",
        // smothed_velocity, aw_speed, factor, time_delta );
    }
#endif
...
moving_bones_snd_player* create_moving_bones_snd_player(CInifile* ini, IKinematics& K,
                                                        const Fmatrix& obj) {
...
void moving_bones_snd_player::load(IKinematics& K, CInifile& ini, LPCSTR section,
                                   const Fmatrix& object) {
    ...
    VERIFY2(min_factor > 0.f,
            make_string("moving_bones_snd_player: bad params: min_factor: %f < 0 ", min_factor));
    VERIFY2(max_factor > 0.f,
            make_string("moving_bones_snd_player: bad params: max_factor: %f < 0 ", max_factor));
    VERIFY2(
        max_factor > 0.f,
        make_string("moving_bones_snd_player: bad params: base_velocity: %f < 0 ", base_velocity));
    VERIFY2(min_factor <= max_factor,
            make_string("moving_bones_snd_player: bad params: min_factor %f > max_factor %f, ",
