    ...
    for (; I != E; ++I) {
        xr_vector<COperatorCondition>::const_iterator J =
            std::lower_bound(this->current_state().conditions().begin(),
								this->current_state().conditions().end(), CWorldProperty((*I).first, false));
        char temp = '?';
...
IC void CPlanner::save(NET_Packet& packet) {
    ...
    {
        packet.w_u32(this->m_storage.m_storage.size());
