...
public:
    ...
    bool in_fov(shared_str const& cover_id, shared_str const& loophole_id,
                Fvector const& position) const;
    bool in_range(shared_str const& cover_id, shared_str const& loophole_id,
                  Fvector const& position) const;
...
private:
    ...
    // fov range checks
private:
    bool in_min_acceptable_range(shared_str const& cover_id, shared_str const& loophole_id,
                                 Fvector const& position, float const& min_range) const;
...
private:
    void loophole_path(smart_cover::cover const& cover, shared_str const& source,
                       shared_str const& target, LoopholePath& path) const;
    void build_enter_path();
    void build_exit_path();
    void build_exit_path_to_cover();
    void actualize_path();
    void try_actualize_path();
    transition_action const& action(smart_cover::cover const& cover, shared_str const& loophole_id0,
                                    shared_str const& loophole_id1) const;
    transition_action const& nearest_action(smart_cover::cover const& cover,
                                            shared_str const& loophole_id0,
                                            shared_str const& loophole_id1, Fvector const& position,
                                            Fvector& result_position, u32& result_vertex_id,
                                            EBodyState* target_body_state) const;
    bool fill_enemy_position(Fvector& position) const;
    bool update_script_cover();
    float exit_path_weight(u32 const& source_node, Fvector const& source_position,
