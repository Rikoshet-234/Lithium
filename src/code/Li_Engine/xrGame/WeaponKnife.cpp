...
void CWeaponKnife::KnifeStrike(const Fvector& pos, const Fvector& dir) {
    ...
    if (real_victim) {
        float new_khit = m_eHitType == m_eHitType_1 ? float(m_Splash1PerVictimsHCount)
    ...
    if (SelectHitsToShot(dest_hits, pos)) {
        ...
        for (shot_targets_t::const_iterator i = dest_hits.begin(), ie = dest_hits.end(); i != ie;
             ++i) {
            Fvector shot_dir;
...
void CWeaponKnife::MakeShot(Fvector const& pos, Fvector const& dir, float const k_hit) {
    ...
    Level().BulletManager().AddBullet(pos, dir, m_fStartBulletSpeed, fCurrentHit, fHitImpulse_cur,
                                      H_Parent()->ID(), ID(), m_eHitType, fireDistance, cartridge,
                                      1.f, SendHit);
}
...
void CWeaponKnife::LoadFireParams(LPCSTR section) {
    ...
    // fHitPower_2			= pSettings->r_float	(section,strconcat(full_name, prefix,
    // "hit_power_2"));
    ...
    fvHitPower_2[egdMaster] = (float)atof(
        _GetItem(*s_sHitPower_2, 0, buffer)); //первый параметр - это хит для уровня игры мастер
    fvHitPowerCritical_2[egdMaster] = (float)atof(_GetItem(
        *s_sHitPowerCritical_2, 0, buffer)); //первый параметр - это хит для уровня игры мастер
    ...
    fvHitPowerCritical_2[egdNovice] = fvHitPowerCritical_2[egdStalker] =
        fvHitPowerCritical_2[egdVeteran] = fvHitPowerCritical_2[egdMaster]; //изначально параметры
                                                                            //для других уровней
                                                                            //сложности такие же

    int num_game_diff_param =
        _GetItemCount(*s_sHitPower_2); //узнаём колличество параметров для хитов
    if (num_game_diff_param > 1)       //если задан второй параметр хита
    {
        fvHitPower_2[egdVeteran] = (float)atof(
            _GetItem(*s_sHitPower_2, 1, buffer)); //то вычитываем его для уровня ветерана
    }
    if (num_game_diff_param > 2) //если задан третий параметр хита
    {
        fvHitPower_2[egdStalker] = (float)atof(
            _GetItem(*s_sHitPower_2, 2, buffer)); //то вычитываем его для уровня сталкера
    }
    ...
    if (num_game_diff_param > 1)                                 //если задан второй параметр хита
    {
        fvHitPowerCritical_2[egdVeteran] = (float)atof(
            _GetItem(*s_sHitPowerCritical_2, 1, buffer)); //то вычитываем его для уровня ветерана
    }
    if (num_game_diff_param > 2) //если задан третий параметр хита
    {
        fvHitPowerCritical_2[egdStalker] = (float)atof(
            _GetItem(*s_sHitPowerCritical_2, 2, buffer)); //то вычитываем его для уровня сталкера
    }
    if (num_game_diff_param > 3) //если задан четвёртый параметр хита
    {
        fvHitPowerCritical_2[egdNovice] = (float)atof(
            _GetItem(*s_sHitPowerCritical_2, 3, buffer)); //то вычитываем его для уровня новичка
    }
...
void CWeaponKnife::OnRender() {
    ...
    if (g_bDrawBulletHit) {
        ...
        // renderer.draw_line			(Fidentity, m_dbg_data.m_pos, victim_end,
...
void CWeaponKnife::fill_shapes_list(CEntityAlive const* entity, Fvector const& camera_endpos,
    ...
    for (CCF_Skeleton::ElementVec::const_iterator i = elems_vec.begin(), ie = elems_vec.end();
         i != ie; ++i) {
        Fvector tmp_pos;
        i->center(tmp_pos);
...
void CWeaponKnife::fill_shots_list(victims_shapes_list_t& victims_shapres, Fsphere const& query,
                                   shot_targets_t& dest_shots) {
    m_victims_hits_count.clear();
    for (victims_shapes_list_t::iterator i = victims_shapres.begin(), ie = victims_shapres.end();
         i != ie; ++i) {
...
void CWeaponKnife::create_victims_list(spartial_base_t spartial_result,
    ...
    for (spartial_base_t::const_iterator i = spartial_result.begin(), ie = spartial_result.end();
         i != ie; ++i) {
...
u32 CWeaponKnife::SelectHitsToShot(shot_targets_t& dst_dirs, Fvector const& f_pos) {
    ...
    victims_list_t tmp_victims_list(_alloca(m_spartial_query_res.size() * sizeof(CEntityAlive*)),
                                    m_spartial_query_res.size());

    create_victims_list(m_spartial_query_res, tmp_victims_list);

    u32 summ_shapes_count = 0;
    for (victims_list_t::const_iterator i = tmp_victims_list.begin(), ie = tmp_victims_list.end();
         i != ie; ++i) {
...
bool CWeaponKnife::SelectBestHitVictim(Fvector const& f_pos, Fmatrix& parent_xform,
                                       Fvector& fendpos_dest, Fsphere& query_sphere) {
    ...
    g_SpatialSpace->q_sphere(m_spartial_query_res, 0, STYPE_COLLIDEABLE, fendpos_dest,
                             m_splash_radius);

    if ((m_eHitType == m_eHitType_2) && (!m_spartial_query_res.empty())) {
        spartial_base_t::value_type tmp_best_victim = NULL;
        best_victim_selector tmp_selector(tmp_parent->ID(), fendpos_dest, spartial_prefetch_radius,
                                          tmp_best_victim);
...
CObject* CWeaponKnife::TryPick(Fvector const& start_pos, Fvector const& dir, float const dist) {
    ...
    Level().ObjectSpace.RayQuery(m_ray_query_results, tmp_rdefs, &CWeaponKnife::RayQueryCallback,
...
CWeaponKnife::victim_filter::victim_filter(victim_filter const& copy)
    : m_except_id(copy.m_except_id), m_start_pos(copy.m_start_pos),
...
CWeaponKnife::best_victim_selector::best_victim_selector(u16 except_id, Fvector const& pos,
