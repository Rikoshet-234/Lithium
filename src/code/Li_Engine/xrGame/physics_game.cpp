...
const float mass_limit = 10000.f; // some conventional value used as evaluative param (there is no
                                  // code restriction on mass)
//////////////////////////////////////////////////////////////////////////////////
...
//CPHReqComparerV* v)					const	{return v->compare(this);}
//	virtual bool			compare							(const
//CPHOnesConditionSelfCmpTrue* v)		const	{return true;}
//
//};
...
template <class Pars>
void play_particles(float vel_cret, dxGeomUserData* data, const dContactGeom* c,
                    bool b_invert_normal, const SGameMtl* static_mtl, LPCSTR ps_name) {
    VERIFY(c);
    VERIFY(static_mtl);
                    ...
                    if (vel_cret > Pars::vel_cret_sound) {
                        if (!mtl_pair->CollideSounds.empty()) {
                            ...
                            GET_RANDOM(mtl_pair->CollideSounds)
                                .play_no_feedback(0, 0, 0, ((Fvector*)c->pos), &volume);
                        }
                    }
                } else {
