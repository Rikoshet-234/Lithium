...
bool CHelicopter::isObjectVisible(CObject* O) {
    ...
    BOOL res = Level().ObjectSpace.RayTest(from_point, dir_to_object, ray_length,
...
void CHelicopter::UpdateHeliParticles() {
    ...
    if (m_light_render->get_active()) {
        ...
        if (m_lanim) {
            ...
            Fcolor fclr;
            fclr.set((float)color_get_B(clr), (float)color_get_G(clr), (float)color_get_R(clr),
                     1.f);
...
void CHelicopter::Hit(SHit* pHDS) {
    ...
    if (pHDS->who && (smart_cast<CActor*>(pHDS->who) || smart_cast<CAI_Stalker*>(pHDS->who) ||
                      smart_cast<CCustomZone*>(pHDS->who))) {
        callback(GameObject::eHelicopterOnHit)(pHDS->damage(), pHDS->impulse, pHDS->hit_type,
                                               pHDS->who->ID());
    }
...
void CollisionCallbackDead(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1,
                           SGameMtl* material_2) {
    do_colide = true;

    CHelicopter* l_this =
        bo1 ? smart_cast<CHelicopter*>(PHRetrieveGeomUserData(c.geom.g1)->ph_ref_object)
