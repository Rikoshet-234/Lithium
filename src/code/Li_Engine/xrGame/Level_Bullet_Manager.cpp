...
void SBullet::Init(const Fvector& position, const Fvector& direction, float starting_speed,
                   float power,
                   //.				   float power_critical,
                   float impulse, u16 sender_id, u16 sendersweapon_id, ALife::EHitType e_hit_type,
                   float maximum_distance, const CCartridge& cartridge,
...
void CBulletManager::Load() {
    ...
    if (pSettings->line_exist(bullet_manager_sect, "bullet_velocity_time_factor")) {
        g_bullet_time_factor =
            pSettings->r_float(bullet_manager_sect, "bullet_velocity_time_factor");
    }
...
void CBulletManager::PlayExplodePS(const Fmatrix& xf) {
    ...
    const auto& ps_name = m_ExplodeParticles[Random.randI(0, m_ExplodeParticles.size())];
...
void CBulletManager::AddBullet(
    const Fvector& position, const Fvector& direction, float starting_speed, float power,
    //.							   float power_critical,
    float impulse, u16 sender_id, u16 sendersweapon_id, ALife::EHitType e_hit_type,
    float maximum_distance, const CCartridge& cartridge, float const air_resistance_factor,
    bool SendHit, bool AimBullet) {
    ...
    bullet.Init(position, direction, starting_speed, power, /*power_critical,*/ impulse, sender_id,
                sendersweapon_id, e_hit_type, maximum_distance, cartridge, air_resistance_factor,
                SendHit);
...
static Fvector parabolic_velocity(Fvector const& start_velocity, Fvector const& gravity,
                                  float const air_resistance, float const time) {
    return (
        Fvector(start_velocity).mul(std::max(0.f, 1.f - air_resistance * time)).mad(gravity, time));
}

static Fvector trajectory_velocity(Fvector const& start_velocity, Fvector const& gravity,
                                   float const air_resistance, float const time) {
...
static Fvector trajectory_position(Fvector const& start_position,
    ...
    float const& air_resistance = base_air_resistance; // g_use_new_ballistics ?
                                                       ...
                                                       // base_air_resistance;
    Fvector const& start_velocity = base_start_velocity; // g_use_new_ballistics ?
                                                         // Fvector(base_start_velocity).mul( factor
                                                         // ) : base_start_velocity;
...
static float trajectory_pick_error(float const low, float const high, Fvector const& position,
                                   Fvector const& velocity, Fvector const& gravity,
    ...
    Fvector const max_error =
        trajectory_position(position, velocity, gravity, air_resistance, max_error_time);
...
static float trajectory_select_pick_gravity(SBullet& bullet, float start_low, float const high,
                                            Fvector const& gravity, float const air_resistance) {
    ...
    VERIFY2(result <= high, make_string("result[%f], high[%f], start_low[%f], air_resistance[%f]",
                                        result, high, start_low, air_resistance));
    return (result);
}

static float trajectory_select_pick_parabolic(SBullet& bullet, float const start_low, float high,
                                              Fvector const& gravity, float const air_resistance) {
    float const max_test_distance = bullet.max_dist - bullet.fly_dist;
    ...
    while (!fsimilar(low, high)) {
        Fvector const intermediate =
            trajectory_position(bullet.start_position, bullet.start_velocity, gravity,
                                air_resistance, start_low + (check_time - start_low) * .5f);
        Fvector const target = trajectory_position(bullet.start_position, bullet.start_velocity,
...
static bool trajectory_select_pick_ranges(float& result, SBullet& bullet, float const low,
                                          float const high, Fvector const& gravity,
                                          float const air_resistance) {
    float const max_test_distance = bullet.max_dist - bullet.fly_dist;
    ...
    if (!fsimilar(fall_down_time, low)) {
        result =
            trajectory_select_pick_parabolic(bullet, low, fall_down_time, gravity, air_resistance);
        return (false);
    }
...
static float trajectory_select_pick_time(SBullet& bullet, float const start_low, float high,
                                         Fvector const& gravity, float const air_resistance) {
    ...
    while (!fsimilar(low, high)) {
        float distance = trajectory_pick_error(start_low, check_time, bullet.start_position,
...
void CBulletManager::add_bullet_point(Fvector const& start_position, Fvector& previous_position,
                                      Fvector const& start_velocity, Fvector const& gravity,
                                      float const air_resistance, float const current_time) {
#ifdef DEBUG
    Fvector const temp =
        trajectory_position(start_position, start_velocity, gravity, air_resistance, current_time);
...
static void update_bullet_parabolic(SBullet& bullet, bullet_test_callback_data& data,
                                    Fvector const& gravity, float const air_resistance) {
    ...
    data.collide_position = trajectory_position(bullet.start_position, bullet.start_velocity,
                                                gravity, air_resistance, data.collide_time);
    Fvector const new_velocity =
        trajectory_velocity(bullet.start_velocity, gravity, air_resistance, data.collide_time);
    bullet.speed = new_velocity.magnitude();
    bullet.dir = Fvector(new_velocity).normalize_safe();
}

static void update_bullet_gravitation(SBullet& bullet, bullet_test_callback_data& data,
                                      Fvector const& gravity, float const air_resistance,
                                      float const fall_down_time) {
    Fvector const fall_down_position = trajectory_position(
        bullet.start_position, bullet.start_velocity, gravity, air_resistance, fall_down_time);
...
static void update_bullet(SBullet& bullet, bullet_test_callback_data& data, Fvector const& gravity,
                          float const air_resistance) {
    if (air_resistance * (bullet.life_time + air_resistance_epsilon) >= 1.f) {
        update_bullet_gravitation(bullet, data, gravity, air_resistance,
                                  std::max(0.f, 1.f / air_resistance - air_resistance_epsilon));
        return;
    }
...
BOOL CBulletManager::firetrace_callback(collide::rq_result& result, LPVOID params) {
    ...
    if (!result.O) {
        ...
        bullet_manager.RegisterEvent(EVENT_HIT, FALSE, &bullet, collide_position, result,
                                     triangle.material);
        return (FALSE);
    }
    ...
    bullet_manager.RegisterEvent(EVENT_HIT, TRUE, &bullet, collide_position, result,
                                 bone_data.game_mtl_idx);
    return (FALSE);
}

bool CBulletManager::trajectory_check_error(Fvector& previous_position,
                                            collide::rq_results& storage, SBullet& bullet,
                                            float& low, float& high, Fvector const& gravity,
                                            float const air_resistance) {
    ...
    BOOL const result =
        Level().ObjectSpace.RayQuery(storage, RD, CBulletManager::firetrace_callback, &data,
                                     CBulletManager::test_callback, NULL);
    if (!result || (data.collide_time == 0.f)) {
        add_bullet_point(bullet.start_position, previous_position, bullet.start_velocity, gravity,
                         air_resistance, high);
        return (true);
    }

    add_bullet_point(bullet.start_position, previous_position, bullet.start_velocity, gravity,
                     air_resistance, data.collide_time);
...
static bool try_update_bullet(SBullet& bullet, Fvector const& gravity, float const air_resistance,
                              float const time) {
    Fvector const new_position = trajectory_position(bullet.start_position, bullet.start_velocity,
    ...
    Fvector const new_velocity =
        trajectory_velocity(bullet.start_velocity, gravity, air_resistance, bullet.life_time);
    bullet.speed = new_velocity.magnitude();
    if (fis_zero(bullet.speed))
        return (false);
...
bool CBulletManager::process_bullet(collide::rq_results& storage, SBullet& bullet,
    ...
    float const air_resistance =
        (GameID() == eGameIDSingle) ? m_fAirResistanceK : bullet.air_resistance;
    bullet.tracer_start_position = bullet.bullet_pos;
    ...
    //	Msg							("process_bullet0: low[%f], high[%f]", low,
    //high);
    ...
    for (;;) {
        for (;;) {
            ...
            if (!trajectory_check_error(previous_position, storage, bullet, low, time, gravity,
                                        air_resistance)) {
...
void CBulletManager::Render() {
    ...
    for (auto it = m_BulletsRendered.begin(); it != m_BulletsRendered.end(); it++) {
        ...
        float dist2segSqr =
            SqrDistancePointToSegment(Device.vCameraPosition, bullet->bullet_pos, tracer);
        //---------------------------------------------
        ...
        tracers.Render(bullet->bullet_pos, center, tracer_direction, length, width,
                       bullet->m_u8ColorID, bullet->speed, bActor);
    }
...
void CBulletManager::CommitEvents() // @ the start of frame
{
    ...
    clearAndReserve(m_Events);
}

void CBulletManager::RegisterEvent(EventType Type, BOOL _dynamic, SBullet* bullet,
                                   const Fvector& end_point, collide::rq_result& R,
                                   u16 tgt_material) {
#if 0  // def DEBUG
	if (m_Events.size() > 1000) {
