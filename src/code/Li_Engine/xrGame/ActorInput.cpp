    ...
    case kQUICK_USE_4: {
        ...
        if (item_name.size()) {
            ...
            if (itm) {
                ...
                string1024 str;
                strconcat(sizeof(str), str, *CStringTable().translate("st_item_used"), ": ",
                          itm->NameItem());
...
void CActor::IR_OnKeyboardRelease(int cmd) {
    ...
    if (g_Alive()) {
		...
		if (g_b_OldStylePickupMode == TRUE)
		{
			m_bPickupMode = false;
		}
...
#ifdef DEBUG
    if (psActorFlags.test(AF_NO_CLIP) && (cmd == kFWD || cmd == kBACK || cmd == kL_STRAFE ||
                    ...
                    if (pGameSP) {
                        if (!m_pPersonWeLookingAt->deadbody_closed_status()) {
                            if (pEntityAliveWeLookingAt->AlreadyDie() &&
                                pEntityAliveWeLookingAt->GetLevelDeathTime() + 3000 <
                                    Device.dwTimeGlobal)
                                // 99.9% dead
