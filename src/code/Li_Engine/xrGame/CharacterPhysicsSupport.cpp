...
//	if( !body1 || !body2 || ( dGeomUserDataHasCallback( c.geom.g1,NodynamicsCollide )&&
//dGeomUserDataHasCallback( c.geom.g2, NodynamicsCollide ) ) ) 		return; 	do_colide =
//false;
//}
...
void CCharacterPhysicsSupport::in_Load(LPCSTR section) {
    ...
    if (pSettings->line_exist(section, "bonce_damage_factor"))
        ...
        m_BonceDamageFactor =
            pSettings->r_float("collision_damage", "bonce_damage_factor_for_objects");
...
void CCharacterPhysicsSupport::SpawnInitPhysics(CSE_Abstract* e) {
    if (m_EntityAlife.g_Alive()) {
        ...
        if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
            ...
            Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s before collision correction "
                "%f,%f,%f",
        ...
        if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) &&
            stricmp(PH_DBG_ObjectTrackName(), *m_EntityAlife.cName()) == 0) {
            Msg("CCharacterPhysicsSupport::SpawnInitPhysics obj %s after collision correction "
                "%f,%f,%f",
                PH_DBG_ObjectTrackName(), m_EntityAlife.Position().x, m_EntityAlife.Position().y,
                m_EntityAlife.Position().z);
        }
...
#ifdef DEBUG
        Msg("! saved bones %d , current bones %d, object :%s", saved_bones.size(),
            m_EntityAlife.PHGetSyncItemsNumber(), m_EntityAlife.cName().c_str());
#endif
...
void CCharacterPhysicsSupport::KillHit(SHit& H) {
    ...
    m_character_shell_control.TestForWounded(m_EntityAlife.XFORM(),
                                             m_EntityAlife.Visual()->dcast_PKinematics());
    Fmatrix prev_pose;
...
void CCharacterPhysicsSupport::in_Hit(SHit& H, bool is_killing) {
    ...
    if (m_EntityAlife.g_Alive() && is_killing && H.type() == ALife::eHitTypeExplosion &&
        H.damage() > 70.f)
    ...
    {
        m_hit_animations.PlayHitMotion(H.direction(), H.bone_space_position(), H.bone(),
                                       m_EntityAlife);
    }
    ...
    if (!(m_pPhysicsShell && m_pPhysicsShell->isActive())) {
        ...
        if (is_killing && death_anim_debug && !is_imotion(m_interactive_motion)) {
            Msg("death anim: applied fatal impulse dir: (%f,%f,%f), value: (%f) ", H.dir.x, H.dir.y,
                H.dir.z, H.impulse);
        }
#endif
        m_pPhysicsShell->applyHit(H.bone_space_position(), H.direction(), H.phys_impulse(),
                                  H.bone(), H.type());
    }
}
...
void CCharacterPhysicsSupport::in_UpdateCL() {
    ...
    if (Type() == etStalker && ph_dbg_draw_mask1.test(phDbgHitAnims)) {
        ...
        /*
           ...
           K->LL_BoneID("bip01_neck"); u16 eb = K->LL_BoneID("eye_right"); Fmatrix &mh  =
           K->LL_GetTransform(hb); Fmatrix &mp  = K->LL_GetTransform(pb); Fmatrix &me	 =
           K->LL_GetTransform(eb); Fmatrix &mn	 = K->LL_GetTransform(nb); float d = DET(mh);
...
#ifdef DEBUG
    Msg("shell for %s[%d] created in %f ms", *m_EntityAlife.cName(), m_EntityAlife.ID(),
...
bool CCharacterPhysicsSupport::CollisionCorrectObjPos(const Fvector& start_from,
    ...
    bool ret =
        ActivateShapeCharacterPhysicsSupport(activation_res, vbox, activation_pos, mXFORM,
...
physics_root_bone_corrected_pos ) );
        ...
        //physics_root_element->TransformPosition( Fmatrix().mul_43( Fmatrix().invert(
K->LL_GetTransform( animation_root_bone_id ) ), physics_root_bone_corrected_pos ) );
}
*/
...
void CCharacterPhysicsSupport::AddActiveWeaponCollision() {
    ...
    CPhysicsShell* weapon_shell =
        P_build_Shell(&active_weapon_item->object(), true, (BONE_P_MAP*)(0), true);
...
void CCharacterPhysicsSupport::CreateShell(CObject* who, Fvector& dp, Fvector& velocity) {
    ...
    if (IsGameTypeSingle()) {
        m_pPhysicsShell
            ->SetPrefereExactIntegration(); // use exact integration for ragdolls in single
...
void CCharacterPhysicsSupport::EndActivateFreeShell(CObject* who,
...
#ifdef DEBUG
    if (death_anim_debug) {
        Msg("death anim: ragdoll velocity picked from char controller =(%f,%f,%f), velocity "
            "applied to ragdoll =(%f,%f,%f)  ",
    ...
    //	anim_mov_blend->timeCurrent  += Device.fTimeDelta * anim_mov_blend->speed;
    //	m_pPhysicsShell->AnimToVelocityState( Device.fTimeDelta, 2 * default_l_limit, 10.f *
    //default_w_limit ); 	mXFORM.set( sv_xform );
    //}
    IKinematics* K = smart_cast<IKinematics*>(m_EntityAlife.Visual());
