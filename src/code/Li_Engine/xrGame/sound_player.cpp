...
u32 CSoundPlayer::add(LPCSTR prefix, u32 max_count, ESoundTypes type, u32 priority, u32 mask,
                      u32 internal_type, LPCSTR bone_name, CSound_UserDataPtr data) {
...
bool CSoundPlayer::check_sound_legacy(u32 internal_type) const {
    ...
    if (m_sounds.end() == J) {
        ...
        ai().script_engine().script_log(
            eLuaMessageTypeMessage, "Can't find sound with internal type %d (sound_script = %d)",
...
void CSoundPlayer::remove_inappropriate_sounds(u32 sound_mask) {
    m_playing_sounds.erase(std::remove_if(m_playing_sounds.begin(), m_playing_sounds.end(),
...
void CSoundPlayer::play(u32 internal_type, u32 max_start_time, u32 min_start_time,
                        u32 max_stop_time, u32 min_stop_time, u32 id) {
...
#ifdef DEBUG
        Msg("- There are no sounds in sound collection \"%s\" with internal type %d (sound_script "
            "= %d)",
    ...
    sound_single.m_stop_time = sound_single.m_start_time +
...
IC Fvector CSoundPlayer::compute_sound_point(const CSoundSingle& sound) {
    Fmatrix l_tMatrix;
    l_tMatrix.mul_43(m_object->XFORM(), smart_cast<IKinematics*>(m_object->Visual())
