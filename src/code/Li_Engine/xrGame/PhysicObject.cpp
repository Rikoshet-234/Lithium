...
void CPhysicObject::create_collision_model() {
    ...
    if (ini && ini->section_exist("collide") && ini->line_exist("collide", "mesh") &&
...
void CPhysicObject::play_bones_sound() {
    if (!bones_snd_player) {
        Msg("! no sound loaded for obj: %s, model :%s - can not play", cName().c_str(),
            cNameVisual().c_str());
        return;
    }
...
static void door_ignore(bool& do_collide, bool bo1, dContact& c, SGameMtl* /*material_1*/,
                        SGameMtl* /*material_2*/) {
            ...
            VERIFY2((!!PKinematicsAnimated->LL_MotionID(visual->startup_animation.c_str()).valid()),
                    (make_string(" animation %s not faund ", visual->startup_animation.c_str()) +
                     dbg_object_base_dump_string(this))
                        .c_str());
            m_anim_blend =
                m_anim_script_callback.play_cycle(PKinematicsAnimated, visual->startup_animation);
        }
...
#ifdef DEBUG
    if (!b)
        Msg(" ! can not control anim - model is not animated name[%s] sect[%s] visual[%s]", name,
            sect, visual);
#endif
    return !!b;
}
...
#ifdef DEBUG
        Msg(" ! can not set blend time %f - it must be in range 0 - %f(timeTotal) obj: %s, model: "
            "%s, anim: %s",
            time, m_anim_blend->timeTotal, cName().c_str(), cNameVisual().c_str(),
...
void CPhysicObject::CreateBody(CSE_ALifeObjectPhysic* po) {
    ...
    case epotBox: {
        m_pPhysicsShell =
            P_build_SimpleShell(this, m_mass, !po->_flags.test(CSE_ALifeObjectPhysic::flActive));
    } break;
    case epotFixedChain:
    case epotFreeChain: {
...
void CPhysicObject::net_Export_PH_Params(NET_Packet& P, SPHNetState& State,
                                         mask_num_items& num_items) {
...
void CPhysicObject::net_Import_PH_Params(NET_Packet& P, net_update_PItem& N,
                                         mask_num_items& num_items) {
