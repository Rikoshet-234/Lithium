...
animation_movement_controller::animation_movement_controller(Fmatrix* _pObjXForm,
    ...
    : m_startObjXForm(inital_pose), m_pObjXForm(*_pObjXForm), m_pKinematicsC(_pKinematicsC),
      ...
      inital_position_blending(true), stopped(false), blend_linear_speed(0), blend_angular_speed(0),
      m_control_blend(b), m_poses_blending(Fidentity, Fidentity, -1.f)
#ifdef DEBUG
      ,
...
static void get_animation_root_position(Fmatrix& pos, IKinematics* K, IKinematicsAnimated* KA,
...
void animation_movement_controller::NewBlend(CBlend* B, const Fmatrix& new_matrix,
                                             bool local_animation) {
    /*
#ifdef	DEBUG
    LPCSTR old_anim_name	= m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg(
ControlBlend( )->motionID ).first; LPCSTR old_anim_set		=
m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg( ControlBlend( )->motionID
...
started anim: %s anim set: %s", new_anim_name,new_anim_set,old_anim_name,old_anim_set
    ...
    if( !ControlBlend( )->stop_at_end )
            Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but fallowed in chain
by another obj movement anim: %s anim set: %s",
                    old_anim_name,old_anim_set,new_anim_name,new_anim_set
                    );
    if( !B->stop_at_end )
            Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but fallowing after
another obj movement anim: %s anim set: %s", new_anim_name,new_anim_set,old_anim_name,old_anim_set
                    );
#endif
    //	VERIFY(  );
...
//
//	if(  pos_sq_delta > change_pos_sq_delta || sphere.P.square_magnitude() + change_pos_sq_delta
//+ pos_sq_delta > sphere.R*sphere.R )
//	{
