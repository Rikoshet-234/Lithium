...
void CEntityAlive::shedule_Update(u32 dt) {
    ...
    if (Local() && !g_Alive() && !AlreadyDie()) {
        if (conditions().GetWhoHitLastTime()) {
            //			Msg			("%6d : KillEntity from CEntityAlive (using who hit last time) for
            //object %s",Device.dwTimeGlobal,*cName());
            KillEntity(conditions().GetWhoHitLastTimeID());
        } else {
            //			Msg			("%6d : KillEntity from CEntityAlive for object
...
BOOL CEntityAlive::net_Spawn(CSE_Abstract* DC) {
    ...
    for (WOUND_VECTOR::const_iterator it = conditions().wounds().begin();
         conditions().wounds().end() != it; ++it) {
        CWound* pWound = *it;
...
void CEntityAlive::HitImpulse(float /**amount/**/, Fvector& /**vWorldDir/**/,
                              Fvector& /**vLocalDir/**/) {
    //	float Q					=
...
void CEntityAlive::Hit(SHit* pHDS) {
    ...
    if (g_Alive() && IsGameTypeSingle()) {
        ...
        if (EA && EA->g_Alive() && EA->ID() != ID()) {
            RELATION_REGISTRY().FightRegister(EA->ID(), ID(), this->tfGetRelationType(EA),
                                              HDS.damage());
...
void CEntityAlive::PlaceBloodWallmark(const Fvector& dir, const Fvector& start_pos,
    ...
    BOOL reach_wall =
        Level().ObjectSpace.RayPick(start_pos, dir, trace_dist, collide::rqtBoth, result, this) &&
        !result.O;
    ...
    if (reach_wall) {
        ...
        if (pMaterial->Flags.is(SGameMtl::flBloodmark)) {
            ...
            VERIFY(!pwallmarks_vector->empty());
            {
                ...
                //::Render->add_StaticWallmark(wallmarkShader, end_point, wallmark_size, pTri,
                //pVerts);
                ::Render->add_StaticWallmark(pwallmarks_vector, end_point, wallmark_size, pTri,
                                             pVerts);
            }
        }
    }
...
void CEntityAlive::StartFireParticles(CWound* pWound) {
    if (pWound->TypeSize(ALife::eHitTypeBurn) > m_fStartBurnWoundSize) {
        if (std::find(m_ParticleWounds.begin(), m_ParticleWounds.end(), pWound) ==
        ...
        pWound->SetParticleName(
            (*m_pFireParticlesVector)[::Random.randI(0, m_pFireParticlesVector->size())]);

        if (BI_NONE != particle_bone) {
            CParticlesPlayer::StartParticles(
                pWound->GetParticleName(), pWound->GetParticleBoneNum(), Fvector().set(0, 1, 0),
                ID(), u32(float(m_dwMinBurnTime) * ::Random.randF(0.5f, 1.5f)), false);
        } else {
            CParticlesPlayer::StartParticles(
...
void CEntityAlive::UpdateBloodDrops() {
    ...
    for (auto it = m_BloodWounds.begin(); it != m_BloodWounds.end();) {
        ...
        if (pWound->m_fDropTime < Device.fTimeGlobal) {
            ...
            pWound->m_fDropTime =
                Device.fTimeGlobal +
                (m_fBloodDropTimeMax - (m_fBloodDropTimeMax - m_fBloodDropTimeMin) * size_k) *
            ...
            if (pWound->GetBoneNum() != BI_NONE) {
                ...
                CParticlesPlayer::GetBonePos(this, pWound->GetBoneNum(), Fvector().set(0, 0, 0),
                                             pos);
                pos.add(pos_distort);
                PlaceBloodWallmark(Fvector().set(0.f, -1.f, 0.f), pos, m_fBloodMarkDistance,
...
DLL_Pure* CEntityAlive::_construct() {
    ...
    if (character_physics_support())
        m_material_manager =
            xr_new<CMaterialManager>(this, character_physics_support()->movement());
    return (this);
}
...
void CEntityAlive::fill_hit_bone_surface_areas() const {
    ...
    for (u16 i = 0, n = kinematics->LL_BoneCount(); i < n; ++i) {
        ...
        case SBoneShape::stBox: {
            ...
            surface_area =
                2.f * (half_size.x * (half_size.y + half_size.z) + half_size.y * half_size.z);
            break;
        }
        ...
        case SBoneShape::stCylinder: {
            surface_area = 2.f * PI * shape.cylinder.m_radius *
...
Fvector CEntityAlive::get_new_local_point_on_mesh(u16& bone_id) const {
    ...
    VERIFY2(hit_bones_surface_area > 0.f,
            make_string("m_hit_bone_surface_areas[%d]", m_hit_bone_surface_areas.size()));
    ...
    case SBoneShape::stBox: {
        ...
        Fvector random = { (random_value & 1) ? -1.f : 1.f, ::Random.randF(2.f) - 1.f,
    ...
    case SBoneShape::stCylinder: {
        ...
        if (random_value < shape.cylinder.m_height) {
            ...
            height = shape.cylinder.m_height / 2.f *
                     ((normalized_value < shape.cylinder.m_radius / 2.f) ? -1.f : 1.f);
            radius =
                height > 0.f ? normalized_value - shape.cylinder.m_radius / 2.f : normalized_value;
        }
