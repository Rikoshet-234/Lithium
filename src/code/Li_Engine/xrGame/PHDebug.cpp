...
struct SPHContactDBGDraw : public SPHDBGDrawAbsract {
    ...
    void render() {
        ...
        Level().debug_renderer().draw_aabb(pos, .01f, .01f, .01f,
                                           D3DCOLOR_XRGB(255 * is_cyl, 0, 255 * !is_cyl));
        Fvector dir;
        dir.set(norm);
        dir.mul(depth * 100.f);
        dir.add(pos);
        Level().debug_renderer().draw_line(Fidentity, pos, dir,
                                           D3DCOLOR_XRGB(255 * is_cyl, 0, 255 * !is_cyl));
    }
};
...
template <int ax>
void DBG_DrawRotation(float ang0, float ang1, const Fmatrix& m, const Fvector& l, float size,
                      u32 ac, bool solid, u32 tessel) {
    Fvector from;
    from.set(m.c);
    Fvector ln;
    ln.set(l);
...
void DBG_DrawRotationX(const Fmatrix& m, float ang0, float ang1, float size, u32 ac, bool solid,
                       u32 tessel) {
    DBG_DrawRotation<0>(ang0, ang1, m, Fvector().set(0, 0, 1), size, ac, solid, tessel);
}

void DBG_DrawRotationY(const Fmatrix& m, float ang0, float ang1, float size, u32 ac, bool solid,
                       u32 tessel) {
    DBG_DrawRotation<1>(ang0, ang1, m, Fvector().set(1, 0, 0), size, ac, solid, tessel);
}

void DBG_DrawRotationZ(const Fmatrix& m, float ang0, float ang1, float size, u32 ac, bool solid,
                       u32 tessel) {
...
struct SPHDBGOutText : public SPHDBGDrawAbsract {
    ...
    virtual void render() {
        ...
        if (!fsimilar(dbg_text_current_height_scale, dbg_text_height_scale)) {
            UI().Font().pFontStat->SetHeight(UI().Font().pFontStat->GetHeight() *
                                             dbg_text_height_scale / dbg_text_current_height_scale);
...
void DBG_RenderUpdate() {
    ...
    dbg_draw_cashed.insert(dbg_draw_cashed.end(), dbg_draw_cashed_secondary.begin(),
...
void DBG_DrawStatBeforeFrameStep() {
    if (ph_dbg_draw_mask.test(phDbgDrawObjectStatistics)) {
        ...
        update_obj_count =
            update_obj_count * 0.9f + float(physics_world()->UpdateObjectsNumber()) * 0.1f;
...
void DBG_DrawStatAfterFrameStep() {
    ...
    if (ph_dbg_draw_mask.test(phDbgDrawCashedTriesStat)) {
        ...
        fdbg_saved_tries_for_active_objects = 0.9f * fdbg_saved_tries_for_active_objects +
                                              0.1f * float(dbg_saved_tries_for_active_objects);
        fdbg_total_saved_tries =
            0.9f * fdbg_total_saved_tries + 0.1f * float(dbg_total_saved_tries);
...
void CFunctionGraph::Init(type_function fun, float x0, float x1, int l, int t, int w, int h,
                          int points_num /*=500*/, u32 color /*=*/, u32 bk_color) {
    x_min = x0;
    x_max = x1;
...
void DBG_AnimBlend(IKinematicsAnimated& ka, const CBlend& B) {
    ...
    if (dbg_track_obj_flags.test(dbg_track_obj_blends_time))
        DBG_OutText("time current: %f, time total: %f, frame %d ", B.timeCurrent, B.timeTotal,
                    B.dwFrame);
    ...
    if (dbg_track_obj_flags.test(dbg_track_obj_blends_flags))
        DBG_OutText("bonepart: %d, channel: %d, stop_at_end: %s, fall_at_end: %s ", B.bone_or_part,
                    B.channel, name_bool(B.stop_at_end), name_bool(B.fall_at_end));
    ...
    for (u16 i = 0; i < nbb; ++i) {
        ...
        if (BI_NONE != bp) {
            ...
            DBG_DrawLine(Fmatrix().mul_43(xform, bone_pos).c,
                         Fmatrix().mul_43(xform, pbi.mTransform).c, D3DCOLOR_XRGB(255, 255, 0));
        }
    }
    ...
    for (u16 i = 0; i < nbb; ++i) {
        ...
        if (BI_NONE != bp) {
            ...
            DBG_DrawLine(Fmatrix().mul_43(O.XFORM(), binds[i]).c,
                         Fmatrix().mul_43(O.XFORM(), binds[bp]).c, D3DCOLOR_XRGB(0, 255, 255));
        }
    }
...
void DBG_ObjBeforeStep(CPHObject* obj) {
    if (is_trace_obj(obj)) {
        ...
        if (obj->ref_object()->ObjectPPhysicsShell()) {
            obj->ref_object()->ObjectPPhysicsShell()->dbg_draw_force(0.1f,
...
void DBG_ObjAfterStep(CPHObject* obj) {
    if (is_trace_obj(obj)) {
        ...
        if (obj->ref_object()->ObjectPPhysicsShell()) {
            obj->ref_object()->ObjectPPhysicsShell()->dbg_draw_velocity(0.1f,
                                                                        D3DCOLOR_XRGB(255, 0, 0));
        }
...
class CPHDebugOutput : public IDebugOutput {
    ...
    // virtual	void DBG_DrawRotationX( const Fmatrix &m, float ang0, float ang1, float size, u32 ac,
    // bool solid = false, u32 tessel = 7 ) = 0;  virtual	void DBG_DrawRotationY( const Fmatrix
    // &m, float ang0, float ang1, float size, u32 ac, bool solid = false, u32 tessel = 7 ) = 0;
    // virtual	void DBG_DrawRotationZ( const Fmatrix &m, float ang0, float ang1, float size, u32 ac,
    // bool solid = false, u32 tessel = 7 ) = 0;
    ...
    virtual u32& dbg_tries_num() {
        ...
        //	make_string( "%s, _14_=%f \n", dump_string( make_string( "%s.i, ", name ).c_str(),
        //form.i ).c_str( ) , form._14_ )	+ 	make_string( "%s, _24_=%f \n", dump_string(
    ...
    virtual float dbg_vel_collid_damage_to_display() {
        ...
        // Msg( "%s, _14_=%f ", dump_string( make_string( "%s.i, ", name ).c_str(), form.i ).c_str(
        // ) , form._14_ );  Msg( "%s, _24_=%f ", dump_string( make_string( "%s.j, ", name ).c_str(),
        // form.j ).c_str( ) , form._24_ );  Msg( "%s, _34_=%f ", dump_string( make_string( "%s.k, ",
        // name ).c_str(), form.k ).c_str( ) , form._34_  );  Msg( "%s, _44_=%f ", dump_string(
        // make_string( "%s.c, ", name ).c_str(), form.c ).c_str( ) , form._44_ );
    }
