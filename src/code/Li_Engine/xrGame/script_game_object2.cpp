...
CScriptGameObject* CScriptGameObject::best_weapon() {
    ...
    if (!object_handler) {
        ...
        CGameObject* game_object =
            object_handler->best_weapon() ? &object_handler->best_weapon()->object() : 0;
...
void CScriptGameObject::set_item(MonsterSpace::EObjectAction object_action,
                                 CScriptGameObject* lua_game_object, u32 queue_size) {
    CObjectHandler* object_handler = smart_cast<CAI_Stalker*>(&object());
    if (!object_handler)
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "CObjectHandler : cannot access class member set_item!");
...
void CScriptGameObject::Hit(CScriptHit* tpLuaHit) {
    ...
    THROW2(tLuaHit.m_tpDraftsman, "Where is hit initiator??!"); //	THROW2
                                                                //(tLuaHit.m_tpDraftsman,"Where is
                                                                //hit initiator??!");
    HS.whoID = u16(tLuaHit.m_tpDraftsman
                       ->ID());    //	P.w_u16			(u16(tLuaHit.m_tpDraftsman->ID()));
    ...
    IKinematics* V =
        smart_cast<IKinematics*>(object().Visual()); //	IKinematics		*V =
    ...
    HS.p_in_bone_space =
        Fvector().set(0, 0, 0);      //	P.w_vec3		(Fvector().set(0,0,0));
    HS.impulse = tLuaHit.m_fImpulse; //	P.w_float		(tLuaHit.m_fImpulse);
    HS.hit_type = (ALife::EHitType)(
        tLuaHit.m_tHitType); //	P.w_u16			(u16(tLuaHit.m_tHitType));
    HS.Write_Packet(P);
...
void CScriptGameObject::SetNpcPosition(Fvector pos) {
    ...
    if (obj) {
        ...
        ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
                                        "ScriptGameObject : attempt to call SetActorPosition "
                                        "method for non-CCustomMonster object");
}
