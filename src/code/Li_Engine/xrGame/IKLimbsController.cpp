...
void CIKLimbsController::LimbCalculate(SCalculateData& cd) {
    cd.do_collide = m_legs_blend && !cd.m_limb->KinematicsAnimated()
...
float CIKLimbsController::LegLengthShiftLimit(float current_shift,
...
void CIKLimbsController::Calculate() {
    ...
    for (u16 j = 0; sz > j; ++j) {
        ...
        if (ph_dbg_draw_mask1.test(phDbgDrawIKPredict)) {
            ...
            DBG_DrawLine(toe, Fvector().add(toe, Fvector().set(0, -_object_shift.shift(), 0)),
    ...
    root_bi.set_callback(root_bi.callback_type(), sv_root_cb, root_bi.callback_param(),
                         sv_root_cb_ovwr);
}
...
void CIKLimbsController::PlayLegs(CBlend* b) {
    ...
    if (MD.marks.empty() && (MD.flags & esmUseFootSteps))
        Msg("! No foot stseps for animation: animation name: %s, animation set: %s ", anim_name,
            anim_set_name);
#endif
}
