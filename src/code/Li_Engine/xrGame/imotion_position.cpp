...
imotion_position::imotion_position()
    : interactive_motion(), time_to_end(0.f), saved_visual_callback(0), blend(0),
      shell_motion_has_history(false){

      };

static void interactive_motion_diag(LPCSTR message, const CBlend& b, CPhysicsShell* s,
                                    float time_left) {
#ifdef DEBUG
    if (!death_anim_debug)
        return;
    ...
    VERIFY(KA);
    CPhysicsShellHolder* O =
        smart_cast<CPhysicsShellHolder*>(s->get_ElementByStoreOrder(0)->PhysicsRefObject());
    VERIFY(O);
    LPCSTR motion_name = KA->LL_MotionDefName_dbg(m).first;
    Msg("death anims - interactive_motion:- %s, motion: %s, blend time %f , total blend time %f , "
        "time left: %f , obj: %s, model:  %s ",
        message, motion_name, b.timeCurrent, b.timeTotal, time_left, O->cName().c_str(),
        O->cNameVisual().c_str());
#endif
}
...
static void get_depth(bool& do_colide, bool bo1, dContact& c, SGameMtl* /*material_1*/,
                      SGameMtl* /*material_2*/) {
...
void imotion_position::state_start() {
    ...
    VERIFY2(B.stop_at_end, make_string("can not use cyclic anim in death animth motion: %s",
...
void imotion_position::collide_not_move(IKinematicsAnimated& KA) {
    ...
    buffer_vector<sblend_save> saved_blends(_alloca(sv_blends_num * sizeof(sblend_save)),
                                            sv_blends_num);
        ...
        if (!!flags.test(fl_switch_dm_toragdoll)) {
            ...
            if (dbg_imotion_collide_debug) {
                ...
                interactive_motion_diagnostic(
                    make_string(" move (to ragdoll): deppth= %f", depth).c_str());
                DBG_OpenCashedDraw();
...
float imotion_position::motion_collide(float dt, IKinematicsAnimated& KA) {
    ...
    if (time_to_end < (max_collide_timedelta + end_delta)) {
        interactive_motion_diagnostic(
            make_string("motion_collide 0: stoped: time out, time delta %f", dt).c_str());
    ...
    if (depth > depth_resolve) {
...
#ifdef DEBUG
        if (dbg_imotion_collide_debug) {
            // interactive_motion_diagnostic( make_string( " motion_collide collided0: deppth= %f",
            // depth ).c_str() );
            interactive_motion_diagnostic(
        ...
        if (dbg_imotion_collide_debug) {
            interactive_motion_diagnostic(
                make_string(" motion_collide collided1: deppth= %f", depth).c_str());
            DBG_OpenCashedDraw();
        ...
        if (depth > depth0) {
            ...
            if (dbg_imotion_collide_debug) {
                interactive_motion_diagnostic(
                    make_string(" motion_collide collided2: deppth= %f", depth).c_str());
                DBG_OpenCashedDraw();
                shell->dbg_draw_geometry(0.02, D3DCOLOR_ARGB(255, 0, 255, 0));
                DBG_ClosedCashedDraw(50000);
            }
            ...
            if (depth > depth_resolve) {
                interactive_motion_diagnostic(
                    make_string("motion_collide 2: stoped: colide: %s, depth %f",
                                collide_diag().c_str(), depth)
                        .c_str());
                flags.set(fl_switch_dm_toragdoll, TRUE);
            }
