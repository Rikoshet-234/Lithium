...
#define FASTMODE_DISTANCE \
    (50.f) // distance to camera from sphere, when zone switches to fast update sequence
...
void CCustomZone::Load(LPCSTR section) {
    ...
    if (pSettings->line_exist(section, "idle_particles_dont_stop"))
        m_zone_flags.set(eIdleObjectParticlesDontStop,
    ...
    if (m_zone_flags.test(eBlowoutLight)) {
        sscanf(pSettings->r_string(section, "light_color"), "%f,%f,%f", &m_LightColor.r,
               &m_LightColor.g, &m_LightColor.b);
...
void CCustomZone::UpdateWorkload(u32 dt) {
    ...
    if (m_zone_flags.test(eUseSecondaryHit) && m_eZoneState != eZoneStateIdle &&
...
void CCustomZone::UpdateIdleLight() {
    ...
    Fcolor fclr;
    fclr.set((float)color_get_B(clr) / 255.f, (float)color_get_G(clr) / 255.f,
...
void CCustomZone::CreateHit(u16 id_to, u16 id_from, const Fvector& hit_dir, float hit_power,
                            s16 bone_id, const Fvector& pos_in_bone, float hit_impulse,
    ...
    if (OnServer()) {
        ...
        SHit Hit =
            SHit(hit_power, hdir, this, bone_id, pos_in_bone, hit_impulse, hit_type, 0.0f, false);
