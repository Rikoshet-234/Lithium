...
void CPHMovementControl::Calculate(Fvector& vAccel, const Fvector& camDir, float /**ang_speed/**/,
                                   float jump, float /**dt/**/, bool /**bLight/**/) {
    ...
    /*
            ...
            {
                    const SGameMtl
       *last_material=GMLib.GetMaterialByIdx(m_character->LastMaterialIDX()); if(
       last_material->Flags.test(SGameMtl::flInjurious) ) mat_injurios =
       m_character->LastMaterialIDX();
            }
       ...
       ); gcontact_HealthLost+=Device.fTimeDelta*GMLib.GetMaterialByIdx( mat_injurios
       )->fInjuriousSpeed;
            }
    ...
    */
    ...
    if (cdi->HitCallback())
        cdi->HitCallback()->call((m_character->PhysicsRefObject()), fMinCrashSpeed, fMaxCrashSpeed,
                                 fContactSpeed, gcontact_HealthLost, CollisionDamageInfo());
...
bool CPHMovementControl::MakeJumpPath(xr_vector<DetailPathManager::STravelPathPoint>& out_path,
                                      u32& travel_point, Fvector& out_deviation) {
...
void CPHMovementControl::Calculate(const xr_vector<DetailPathManager::STravelPathPoint>& in_path,
                                   float speed, u32& travel_point, float& precision) {
#ifdef DEBUG
    if (debug_output().ph_dbg_draw_mask1().test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        stricmp(debug_output().PH_DBG_ObjectTrackName(), *pObject->cName()) == 0) {
...
void CPHMovementControl::PathNearestPoint(
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        ...
        Msg("CPHMovementControl::Calculate out %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::Calculate out %s (CPHMovementControl::vPosition) %f,%f,%f",
...
void CPHMovementControl::PathNearestPointFindDown(
    ...
    for (; i > 1; --i) {
        ...
        if (from_second_dir > 0.f) // befor this line
        {
            ...
            if (after_line) // after previous line && befor this line = near second point (going
                            // down)
            {
                if (temp < m_path_distance) {
                    m_path_distance = temp;
...
void CPHMovementControl::PathDIrLine(const xr_vector<DetailPathManager::STravelPathPoint>& path,
                                     int index, float distance, float precesition, Fvector& dir) {
...
void CPHMovementControl::PathDIrPoint(const xr_vector<DetailPathManager::STravelPathPoint>& path,
                                      int index, float distance, float precesition, Fvector& dir) {
    Fvector to_path_point;
    Fvector corrected_path_dir;
    CorrectPathDir(GetPathDir(), path, index, corrected_path_dir);
...
void CPHMovementControl::Load(LPCSTR section) {
    ...
    if (pSettings->line_exist(section, "actor_restrictor"))
        SetRestrictionType(
            ERestrictionType(pSettings->r_token(section, "actor_restrictor", retrictor_types)));
    fCollisionDamageFactor = READ_IF_EXISTS(pSettings, r_float, section,
    ...
    // );  gf					= pSettings->r_float
    // (section,"ph_friction_ground");
...
void CPHMovementControl::SetPosition(const Fvector& P) {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        ...
        Msg("CPHMovementControl::SetPosition %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::SetPosition %s (CPHMovementControl::vPosition) %f,%f,%f",
            PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
    }
#endif
    vPosition.set(P);
...
bool CPHMovementControl::TryPosition(Fvector& pos) {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        ...
        Msg("CPHMovementControl::TryPosition %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::TryPosition %s (CPHMovementControl::vPosition) %f,%f,%f",
            PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
    }
#endif
    if (m_character->b_exist) {
...
void CPHMovementControl::GetPosition(Fvector& P) {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        ...
        Msg("CPHMovementControl::GetPosition %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::GetPosition %s (CPHMovementControl::vPosition) %f,%f,%f",
            PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
    }
#endif
    P.set(vPosition);
...
void CPHMovementControl::AllocateCharacterObject(CharacterType type) {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        stricmp(PH_DBG_ObjectTrackName(), *pObject->cName()) == 0) {
        Msg("CPHMovementControl::AllocateCharacterObject %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::AllocateCharacterObject %s (CPHMovementControl::vPosition) "
            "%f,%f,%f",
            PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
    }
#endif
...
void CPHMovementControl::GetJumpMinVelParam(Fvector& min_vel, float& time, JumpType& type,
...
float CPHMovementControl::JumpMinVelTime(const Fvector& end_point) {
    return ThrowMinVelTime(
        Fvector().sub(end_point,
                      smart_cast<CGameObject*>(m_character->PhysicsRefObject())->Position()),
        physics_world()->Gravity());
}

void CPHMovementControl::GetJumpParam(Fvector& velocity, JumpType& type, const Fvector& end_point,
                                      float time) {
    Fvector velosity;
...
void CPHMovementControl::CreateCharacter() {
    ...
    if (ph_dbg_draw_mask1.test(ph_m1_DbgTrackObject) && (!!pObject->cName()) &&
        stricmp(PH_DBG_ObjectTrackName(), *pObject->cName()) == 0) {
        Msg("CPHMovementControl::CreateCharacter %s (Object Position) %f,%f,%f",
            PH_DBG_ObjectTrackName(), pObject->Position().x, pObject->Position().y,
            pObject->Position().z);
        Msg("CPHMovementControl::CreateCharacter %s (CPHMovementControl::vPosition) %f,%f,%f",
            PH_DBG_ObjectTrackName(), vPosition.x, vPosition.y, vPosition.z);
    }
#endif
    m_character->SetPosition(vPosition);
...
void CPHMovementControl::InterpolateBox(DWORD id, float k) {
    ...
    if (!m_character || !m_character->b_exist)
        return;
    dVector3 size = { aabb.x2 - aabb.x1, aabb.y2 - aabb.y1, aabb.z2 - aabb.z1 };
    dVector3 to_size = { boxes[id].x2 - boxes[id].x1, boxes[id].y2 - boxes[id].y1,
...
void CPHMovementControl::ApplyHit(const Fvector& dir, const float P, ALife::EHitType hit_type) {
    ...
    if ((Environment() == CPHMovementControl::peOnGround ||
        ...
        //			case ALife::eHitTypePhysicStrike:	SetVelocity(Fvector().set(0,0,0))
        //;break;//stop
        default:
            NODEFAULT;
        }
...
//		return;
//
//	dxGeomUserData	*my_data			=	PHRetrieveGeomUserData(	bo1 ?
//c.geom.g1 : c.geom.g2 );
//	//dxGeomUserData	*oposite_data		=	PHRetrieveGeomUserData( bo1 ?
//c.geom.g2 : c.geom.g1 ) ; 	VERIFY( my_data );
//
...
//dJointCreateContact(0, ContactGroup, &c);// 	CPHObject* obj = (CPHObject*)my_data->ph_object;
//	VERIFY( obj );
...
void CPHMovementControl::actor_calculate(Fvector& vAccel, const Fvector& camDir, float ang_speed,
                                         float jump, float dt, bool bLight) {
